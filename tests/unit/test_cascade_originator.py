...
`chart/values.yaml` `medallion.stageRunners[].author` — and `lance-medallion/embed_features` failed
...
the stage runner either — `enforce_author` would overwrite a human there anyway. The sanctioned mechanism is
...
`medallion.bronze` trigger. The bronze→silver stage runner then threads `trigger.originator` onto every one
of its four FAIL emits. The break is the TIER BOUNDARY. Under one door a stage runner does not publish the
...
a cascade publish is the stage runner's own service identity (`service-bronze-to-silver`, the sub the service
door mints). So the silver→gold FAIL run named a SERVICE as the person it was for: an inbox actor
named after a stage runner, which looks delivered and is not (trap 4 — an address must identify a person).

THE FIX IS THE `cascade_id` SHAPE, because that field lost the same hop for the same reason and its
carrier is already in these three files: the stage runner puts the human on the publish body, the catalog
...
#: What the bronze→silver stage runner authenticates to the catalog AS (`chart/values.yaml`
#: `medallion.stageRunners[].serviceIdentity`). This is the string that was reaching the inbox.
STAGE_RUNNER_IDENTITY = "service-bronze-to-silver"
...
# ── link 3: the stage runner hands the human to the catalog with its publish ─────────────────────────────
...
def test_the_stage_runner_puts_the_human_on_the_publish_it_sends_the_catalog() -> None:
    """The stage runner is the last holder of the batch's human before the tag move. It carried `cascade_id`
...
def test_a_stage_runner_with_no_human_behind_it_publishes_no_identity() -> None:
...
def test_the_catalog_prefers_the_carried_human_over_the_service_that_published() -> None:
    """THE DEFECT, at its choke point. A cascade publish authenticates as the stage runner, so the actor is
...
def test_a_claim_that_names_no_person_is_refused(claimed: str) -> None:
    ...
    assert publication_originator(claimed, _service_token(STAGE_RUNNER_IDENTITY)) == ""
...
def test_an_fga_spelled_principal_is_read_as_the_person_it_names() -> None:
    ...
    assert publication_originator(f"user:{HUMAN}", _service_token(STAGE_RUNNER_IDENTITY)) == HUMAN
...
def test_the_next_tiers_trigger_names_the_human_not_the_stage_runner_that_published() -> None:
    """THE ROW'S DEFECT, at the hop where it happened. The head used to read the event's ACTOR, so a
    cascade publish handed the silver→gold stage runner `originator: service-bronze-to-silver` — and every
    FAIL that stage runner emitted addressed an inbox named after a service."""
    trigger = _next_trigger(extra={"originator": HUMAN}, actor=f"user:{STAGE_RUNNER_IDENTITY}")
...
def test_a_publication_with_no_human_carries_no_originator_at_all() -> None:
    ...
    trigger = _next_trigger(extra={}, actor=f"user:{STAGE_RUNNER_IDENTITY}")
...
def _gold_stage_fail(trigger: dict[str, Any]) -> dict[str, Any]:
    """The FAIL RunEvent the silver→gold stage runner emits, built exactly as `transform._emit_fail_run` does.
...
def test_a_personless_cascade_failure_reaches_only_its_author() -> None:
    ...
    trigger = _next_trigger(extra={"cascade_id": "tok1"}, actor=f"user:{STAGE_RUNNER_IDENTITY}")
