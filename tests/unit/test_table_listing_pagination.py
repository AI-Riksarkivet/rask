...
from catalog.api.v1.endpoints.tables import _paginate, list_all_tables
...
def test_no_limit_returns_everything_and_no_cursor() -> None:
    assert _paginate(list(NAMES), None, None) == (NAMES, None)
...
def test_pages_walk_the_whole_list_without_overlap_or_loss() -> None:
    ...
    for _ in range(10):  # bounded so a broken cursor cannot hang the test
        page, token = _paginate(list(NAMES), token, 2)
...
def test_final_page_has_no_cursor() -> None:
    page, token = _paginate(list(NAMES), "d$four", 2)
    assert page == ["e$five"]
...
def test_exact_fit_ends_pagination() -> None:
    ...
    page, token = _paginate(list(NAMES), None, 5)
    assert page == NAMES
    assert token is None


def test_stale_cursor_past_the_end_is_empty_and_done() -> None:
    assert _paginate(list(NAMES), "zzz", 2) == ([], None)
...
def test_native_call_is_unpaginated_and_pagination_runs_after_fga() -> None:
    ...
    only the root ingredient), and _paginate must run AFTER the FGA filter so pages count only
    ...
    assert src.index("can_read_data") < src.index("_paginate(")
