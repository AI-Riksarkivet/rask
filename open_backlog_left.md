...
- *Closes when:* the run reaches a terminal SUCCESS against the deployed estate. **TWO OF THREE OBSTACLES ARE FIXED 2026-09-10 and the third is a new finding.** (1) `_auth()` was assembled and DROPPED — `post()` built `headers={'Content-Type': ...}` and never applied it, so every control-plane door answered 401 whatever credential was supplied; a helper nobody calls reads as authentication being handled. Fixed, and `provision` now answers 200/200/200. (2) It provisioned the bare PROJECT namespace while a bronze write lands in `<project>-<tier>` (`ingest.naming.bronze_namespace_for`), so it created a namespace nothing writes to. Fixed. (3) FIXED — the script's `authorize` step set an env var and wrote NO tuple, so the identity could not see the tier namespace an admin had just created; it now grants `writer` on `namespace:<project>-<tier>` and the run passes the namespace check. (4) FIXED — `ingest_pod` selected on `status.phase=Running`, and a TERMINATING pod is still Running (it only gains a `deletionTimestamp`), while `items[-1:]` took the last match; right after `provision` — whose `set env` rolls the deployment — every step answered `container not found`. It now skips pods carrying a deletion timestamp. **WHERE IT STOPS NOW (new, not the script):** the run now reaches Activity #10, having CREATED and registered its table (`lane-bronze$a11-…` lists in the catalog), and fails reading it back: *"could not read the `id` column of s3://lane-wh/…, so this run cannot tell which objects bronze already holds ... Failed to get AWS credentials: CredentialsNotLoaded"*. Not a 403 — NO credential was loaded at all, on a warehouse bucket minted minutes earlier. The dedupe read reaches the store by a different path than `ensure` does, and only `ensure` asks the catalog to vend. That is the next thing to measure. (The 403-vs-404 half of this row is fixed separately: an existence probe answered 403 now names the missing grant instead of reporting the namespace absent.) Next: give the lane's service identity the rung it needs on the tier namespace (the script's own `authorize the ingest service token` step grants on `project:lane` and evidently not on the namespace), and separate the two answers in the ingest activity's message.
