...
- *Closes when:* Decide whether ingest's bronze output is a governed tier. If it is, the lander's schema carries all three columns (`stage`, `lineage`, `source_rowid`) from creation — they cannot be added later for the same reason the A14 flags cannot. If it is not, it carries NONE of them and the publish is not attempted. Then re-run `scripts/ingest-lane.sh run` and assert `published: true`.
