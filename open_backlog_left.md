...
**LH-125 · The cascade HEAD's own write event never reaches the graph, so every silver run has no bronze parent**
...
- *Why open:* Measured 2026-09-10 minutes apart: `GET /admin/dlq` answers `{"depth": 0, "oldest_age_seconds": 0.0, "events": []}` while `nats stream ls` reports the DLQ stream at 4,758 messages / 13 MiB with its most recent message 9 seconds old. The endpoint exists precisely so an operator can see the backlog without a NATS client, and it reports the one number that means "nothing is wrong". Either it reads a different subject than the dead-letter component writes, or it reads a queue that is drained on read while the stream retains — either way an operator checking the DLQ sees clean.
- *Closes when:* Drive one event to the dead-letter topic and assert `/admin/dlq` reports depth 1 with that event in `events`; then reconcile the number the endpoint reports with the stream's own depth, and say in the response which of the two it is answering.
