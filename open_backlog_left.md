...
| Phase | Items | High |
| --- | --- | --- |
| **1 · Lakehouse** (catalog, lineage, medallion, maintenance) | 120 | 26 |
...
**1. 20+ governed silver/gold tables still declare their PARENT's `lance.dataset_id`**
...
**2. The reconcile sweep warns every tick on 32 `storage_loss` + 2 `unreadable` datasets that are all test residue**
...
**3. The `parent`, `processingEngine` and engine-version run facets are neither emitted nor stored, and the graph has no version/branch/tag/clone/base nodes**
...
**4. Four OpenLineage emit kernels and three `RunEvent` builders, all swallowing transport failures, with no outbox**
...
**5. `create_table` accepts no `source`/`source_version`/`X-Lance-Run-Facets`, so a derived table's first write emits lineage with no reproducibility pin**
`catalog, lineage` · **HIGH**

- *Why open:* An asymmetry that exists today: `merge_insert` (data.py:445) takes a version-pinned `source` and a custom run facet while `create` takes only `mode`, `properties`, `data_base`, `authorization` — so a Ray job's output, an export or a publish is created unpinned. Specified down to the test and file, nothing written.
- *Closes when:* Add `source`, `source_version` and the `X-Lance-Run-Facets` header to `create_table` reusing `_merge_source_pin` (data.py:383) and `_parse_run_facets` (data.py:401); RED test first (input_version on the READ edge, custom facet survives `shape_run_facets`, `source_version` without `source` raises `InvalidInputError`, a `_RESERVED_RUN_FACETS` name rejected); regenerate `docs/catalog-openapi.json` with `make openapi`.

**6. `UPSTREAM`/`DOWNSTREAM`/column-lineage Cypher is unbounded `*1..`, and Dataset nodes carry no `latest_version`**
`lineage` · med
...
**7. `ray_stage_job.py:642-649` re-creates its target with `mode="overwrite"` every run, re-minting `_rowid` for the whole tier**
...
**8. Publication deltas are insert-only: neither in-place updates nor deletions reach a consumer**
...
**9. The lineage graph keys a dataset version by `(dataset, N)` where Lance's identity is `(branch, N)`, so a branch write reconciles as `storage_loss`**
...
**10. HTR-lane cascade residuals: the P7b re-cut, the bronze→silver geometry stage runners, and populating the in-dataset `lineage` column**
...
**11. `runRetentionDays: 0`, `compaction.lineageEmit: false` and `freshnessBudgetHours: 0` ship in the prod render**
...
**12. `scripts/medallion_demo.py::write_gold` silently falls back to a `pa.string()` lineage column when `pa.json_()` raises**
`medallion` · low
...
**13. OPEN-WORK C3 still cites a stale 'gold does not embed lineage' verdict that the code contradicts**
`medallion, lineage` · low

- *Why open:* The 2026-07-28 re-check banner says the §1 verdict is out of date: `medallion/services/compute.py:43-50` defines `_LINEAGE_COLUMN = 'lineage'`, stamps it on every stage via `_RESTAMPED_COLUMNS` and builds a `lineage_run_id_idx` index on the JSON path `run_id`. So the row tracks work that may already be done, against evidence rather than code.
- *Closes when:* Re-derive OPEN-WORK C3 against `medallion/services/compute.py` and `UpstreamFacts.chain`, rewrite or strike the row, and confirm the `lineage` column's scalar index uses the same JSON path literal the queries use.

**14. The DIY provenance recipe (`stamp_stage`, `source_rowid`, the tier contract) is written down nowhere**
`medallion, lineage` · low
...
**15. medallion `transform.py` composes `{project_root}/medallion/{namespace}` for its write TARGET and registers nothing, so silver/gold tables are invisible to the catalog**
...
**16. `bronze-media` and `silver-media` still hijack medallion namespaces inside `lakehouse-wh`, and unbind is refused because they hold real tables**
...
**17. `CatalogServiceClient.ensure` never calls `assert_creation_contract` — not on its short-circuit return, not after `_create_empty`**
...
**18. The governed commit door is the non-spec `/commit`; `CreateTableVersion`/`BatchCommitTables` carry no lineage, gate, protection or replay marker**
...
**19. rask-only governance side effects still run inside spec handlers (warehouse-scoped namespace refusal, trash soft-delete, protection 409, lineage keys in schema metadata, implicit BTREE, insert pre-coercion, maintenance 503 on POST reads)**
...
**20. Two of the three stock Lance clients still do not drive the deployed catalog: lancedb cannot address a nested namespace, lance-ray is untested**
...
**21. 25 rask-only route groups still sit on the spec prefixes instead of a versioned management API**
...
**22. The joint `lance-namespace` 0.12.0 + pylance bump is unmade, so `merge_insert`'s `on` stays a single string and a composite merge key is inexpressible**
...
**23. A client-supplied `delimiter` is refused 400 rather than honoured on all 153 catalog ops**
`catalog` · med
...
**24. Every Q13 `LANCE CLAIM` verdict was measured on pylance 10.0.0 and is unverified against the locked 11.0.0**
...
**25. `create_table`'s `properties` land only in the namespace declare and the response echo, never on the dataset's schema metadata**
`catalog` · med
...
**26. Schema and data evolution is neither exposed nor governed — no add/alter/drop column door, no compatibility check, no owner gate, no schema history**
`catalog` · med
...
**27. `register_table`'s door enforces neither location containment nor stable row ids — both live only below it**
...
**28. Three container-tier deletion paths were never driven live: warehouse delete, project delete, cascade DETACH + plural undrop, and bucket-purge sole-ownership**
`catalog` · med
...
**29. `batch_commit_tables` cannot converge: a retry re-runs the atomic native commit, hits `TableAlreadyExists`, and never reaches the ownership seeds**
...
**30. A partially-failed warehouse delete does not report which parts failed**
`catalog` · med
...
**31. Root `ListNamespaces` asks only the default namespace, so a spec client discovers no warehouse-bound data from the root**
`catalog` · med
...
**32. `handle_validation_error` hardcodes 422 while emitting `ErrorCode.INVALID_INPUT`, which maps to 400 — the vendored spec contains zero 422s**
...
**33. Nine catalog operations answer 501: branch-scoped `query`/`explain_plan`/`analyze_plan`, `create_index`/`create_scalar_index`, `stats`, `index/list`, `index/{n}/stats`**
`catalog` · med
...
**34. Compression is never configured anywhere and there is no decision record**
...
**35. The query door serves only the blob DESCRIPTOR shape with no `all_binary` opt-in, and `read_blob_ranges` is undocumented as the batched byte-fetch path**
...
**36. Body-id reconciliation (A1) is missing on four catalog routes**
`catalog` · med
...
**37. `POST /v1/namespace/{id}/create` and `POST /v1/table/{id}/register` accept `mode` and answer 409 whatever it says**
`catalog` · med
...
**38. `POST /v1/table/{id}/version/list` accepts `page_token` and ignores it**
`catalog` · med
...
**39. `POST /produce` accepts a governed-tier claim in `settings` and disregards it**
`medallion` · med
...
**40. Put-if-not-exists is verified only on RustFS, so Lance's CAS commit model is untested on any other store**
`catalog, storage` · med
...
**41. Branch/tag writes are unconditional at every layer including pylance's `Tags::update` — a lost update in waiting**
`catalog` · med
...
**42. Blob v2 default thresholds disagree three ways (64 KB/4 MB vs 16 KiB/2 MiB vs rask's measured 64 KiB/4 MiB)**
`catalog, service-kit` · med
...
**43. Unknown whether MemWAL server-id sharding fits append-only bronze landing (coordinator-free ingest)**
...
**44. `tags/create` drops `branch`, `branches/create` drops `from_branch`+`from_version`, `branches/delete` drops `name`**
`catalog` · low
...
**45. rename / backfill / alter_transaction / MV create+refresh / batch-create + batch-commit versions return 501 from the native backend**
...
**46. A malformed BRANCH name on the S3-backed catalog answers `Internal 18` instead of `InvalidInput 13`**
...
**47. `lance_docs/` is 79 lines behind upstream and carries no manifest, pinned commit or provenance line**
`catalog` · low
...
**48. Two upstream defects are unfiled: pylance's GET routes (A3) and the 0.12.0 `header.` vs `headers.` prefix in the bundled client**
...
**49. Whether descriptions bind to the catalog object is undecided**
...
**50. TRIPWIRE: no query store for listings, deliberately, until interactive-frequency listing load appears**
...
**51. `rask-catalog` is the last service presenting the RustFS root key `rustfsadmin` as its S3 identity**
...
**52. `LANCE_FGA_CASCADE_WRITERS` grants every stage runner `owner` on EVERY tenant warehouse, and the bounding control is off by default**
...
**53. Bucket claims are keyed by warehouse ID, so two warehouse IDs can both claim the SAME bucket — and the four control-root JSON stores it must live in are not collapsed**
...
**54. The credential-isolation e2e SKIPS against the shipped stack, so cross-tenant credential refusal is proven only by rask's own offline policy evaluator**
...
**55. The FGA model has no `branch`/`column`/`base`/`estate` type, `can_set_protection` collapses onto `can_drop`, and `project` has no security_admin/data_admin/role_creator split or machine identity**
...
**56. Branch-scoped governance is missing: no FGA `branch` type, vending/protection/trash are branch-blind, and branch/tag creation emits no control event**
...
**57. Per-base credential vending is unimplemented — the vendor refuses any table whose fragments carry a `base_id` instead of vending a union of bases**
...
**58. No column-level classification or policy exists: `columns.py` has no FGA check and `pii` survives only as a key in seed data**
...
**59. The bronze write doors never self-check `can_create_table` on `namespace:bronze` — the FGA model describes a rung nothing enforces**
`medallion, catalog` · med
...
**60. Nothing detects a table stranded between the Lance write and the ownership grant — a catalog object with ZERO FGA tuples is invisible to the drift report**
...
**61. There is no write-capable reconcile: stranded objects cannot be listed, repaired or dropped, and FGA tuples cannot be rebuilt from the catalog**
...
**62. `type role` in `model.fga` has no `project` edge, so a global role name can hold a rung on any tenant's namespaces**
...
**63. Every FGA grant is keyed on a Dex-CONNECTOR-scoped protobuf subject, with no subject-identity migration story**
...
**64. Lineage-bus producers are unauthenticated: no Dapr `accessControl` on `dapr-caller-app-id` and no producer signature over the CloudEvent**
`lineage` · med
...
**65. Prod vending is `mode_b` everywhere, so the tenant-isolation machinery is dormant on every shipped estate (and no `/refresh-credentials` door exists for the alternative)**
...
**66. The maintenance identity is one key across every warehouse rather than a per-warehouse scoped credential**
...
**67. Warehouse storage cannot be expressed as bases: one endpoint and one key for the whole estate, and a warehouse-rooted connection swaps only `root`**
...
**68. Legacy data that predates warehouses has no migration path**
`catalog, maintenance` · med
...
**69. Pre-registry 'ghost' project ids were never migrated, which is why the consume-side id rule stays looser than the mint rule**
...
**70. No versioned authz-model migration (`ACTIVE_MODEL_VERSION` + an idempotent `migrate()`) — the 3-axis model shipped without it**
...
**71. Tuple helpers were never split into `tuples.py` and there are no golden tuple tests**
`catalog, service-kit` · med
...
**72. The vended response still mixes credentials and config in one dict**
`catalog, storage` · med
...
**73. Right to erasure is a Lance row delete only — it reaches no blob sidecar, clone/branch or version-pinning tag**
...
**74. No quotas or storage accounting per project/warehouse**
`catalog, maintenance` · med
...
**75. The read audit log has no retention policy and no index on dataset in GreptimeDB**
`catalog, chart` · med
...
**76. `can_observe_events` is the estate-admin bar under a name that says 'read the feed', and its comment names one of its four consumers**
...
**77. `alter_transaction` gates a whole `AlterTransactionRequest` at one committer-tier check while the model claims a per-action distinction**
...
**78. 8 credential vends per tick still 403, so 8 rewrites sign with the ambient root key, and no counter or alert surfaces it**
...
**79. Two standing answers on the `x-api-key` principal contradict each other, and its key store and rotation model are undesigned**
...
**80. `can_promote` buys nothing on `table` because `validator ⊇ owner`**
`catalog, openfga` · low
...
**81. Nothing proves `_authorize_transaction`'s two branches gate equivalent privilege**
`catalog` · low
...
**82. The gateway proxies the catalog's full all-method write surface to the public ingress and nothing says whether that is intended**
...
**83. `engine_registry.executor_for` has ZERO callers — the deployed stage lane still calls `ray_submit` directly at `workflow.py:495`**
...
**84. `BAKED_JOBS_DIR`/`BAKED_CLUSTER_JOBS` live in the shared library and the catalog enforces them, so a non-Ray lane cannot be declared and the word 'Ray' reaches every API client via the published OpenAPI**
...
**85. The multimodal write lane is single-driver even though `lance_ray` 0.5.0 does not strip blob typing**
`medallion` · med
...
**86. `test_outbox_e2e::test_reconcile_sweep_drains_a_staged_outbox_event` still fails — a staged lineage event strands instead of draining**
`lineage` · **HIGH**
...
**87. The bronze-arrival cascade head fired on `create_table` but not on a second INSERT into the same bronze table**
...
**88. Every lineage restart replays the retained stream and logs ~125 `dapr_dead_letter_parked` ERRORs claiming provenance was lost**
...
**89. Neither `medallion.bronze` trigger head can be retired: no remaining writer can publish, so disabling either strands a lane**
...
**90. `POST /v1/table/{id}/publish` emits the `table_published` control event carrying the WRONG `to_version`**
...
**91. The change feed has no control-lane EVENT, so a BYO consumer must poll `POST /v1/table/{id}/changes`**
...
**92. The ingest-lane slice proves the TRIGGER chain but not the DATA chain — `MEDALLION_FROM_URI`/`TO_URI` are unset there**
`medallion` · med
...
**93. The event actor is one `author.sub` string where it should be a closed union (Anonymous | Principal | Role)**
...
**94. The orphan scan refuses every flag-16 dataset wholesale — 490 of 611 reconcile scans incomplete — because there is no per-base managed vs reference-only policy**
...
**95. The maintenance sweep takes no lease per tick or per dataset, and refusal state (`attempts`/`last_refusal`) is never persisted on the trash record**
...
**97. Tiers re-materialise managed blob bytes per tier instead of silver being a shallow clone of bronze@N plus `add_columns`**
...
**98. Maintenance's reclamation trail carries no attempt count, no duration and no per-object fact a TENANT can query**
...
**99. A sweep that deleted a terabyte and one that deleted nothing produce the same-shaped report — no bytes-reclaimed anywhere, and no control event for what was rewritten**
...
**100. 49 `models/<run>/<id>/` prefixes still file as IncompleteScan coverage gaps, and the `discoveryMaxDepth` lever never reached the running pod**
...
**101. The sweep has no per-tick budget and no rotated bucket order, so at estate scale the tail is maintained only if the tick has time left**
`maintenance` · med
...
**102. Storage reclamation has never been run live — trash purge must go first, and it is gated on a clean drift report**
...
**103. The maintenance sweep has never been run against a real S3 — its e2e is env-gated and skips**
`maintenance` · med
...
**104. No index is ever built on a governed table — search tunes `nprobes` for one that is not there and `maintenance.indexAckWait` is a 3600s placeholder**
...
**105. There is no reindex-from-scratch operation in `services/maintenance`**
`maintenance` · med
...
**106. Resilience rows exist only inline in `RESILIENCE.md`: the chaos harness was never automated, DLQ sidecar parking was never driven live, and lineage scale-0 restart-replay was never re-verified**
`lineage, chart` · med
...
**107. The catalog is correct only at `replicas=1` because `controlEmit`'s ring buffer and cursor are per-replica, and every stage runner calls it**
...
**108. The lineage + OpenFGA store is still the hand-rolled `rask-age` StatefulSet; the CNPG cutover is built but off**
...
**109. Eleven stable live-suite failures are still unclassified after the `/runs?limit=1000` drift repair**
...
**110. No documented, exercised restore of the control root (projects, warehouses, bindings, trash)**
`catalog, chart` · med
...
**111. No in-flight blob-byte admission budget — the catalog counts requests, not bytes**
`catalog` · med
...
**112. Unknown whether Lance honours a tag that pins a BRANCH version during main cleanup**
...
**113. One 340-line catalog `Settings` class carries every domain's configuration**
`catalog` · med
...
**114. Multi-base (`base_paths`) is implemented but never exercised, and no test proves cleanup on a shared non-root base spares its sibling**
`maintenance, catalog` · low
...
**115. Manifest flags 16/64 are refused only by the orphan pass, not by the rest of the maintenance surface**
`maintenance` · low
...
**116. Fragment sizing is left at Lance defaults with no per-table lever**
`maintenance` · low
...
**117. The orphan scan has no chart toggle — `MAINTENANCE_ORPHAN_SCAN_ENABLED` is env-only**
...
**118. An unparseable `lance-catalog/_projects/_probe.json` that nothing writes keeps `registry:projects` in the reconcile report's `incomplete: 61`**
...
**119. The dropped-parameter sweep's coverage is unassessed — 16 verify calls and the completeness critic failed on the weekly subagent limit**
...
**120. Both medallion entrypoints read settings at import time — `producer.py:170`, `producer.py:202` and `stage_runner.py:46`**
`medallion` · low
...
**121. No `checksum/secret` pod-template annotation anywhere, so a rotated Secret is never re-read — six of seven zones polled lineage with a dead token and 46% of its traffic was 401**
...
**122. The chart fix moving the Ray head's S3 credential onto the sanctioned ESO path is committed and never deployed**
...
**123. `lance.audit` shares `opentelemetry_logs` with all telemetry: 14-day TTL, and an unauthenticated in-cluster `DELETE` on the audit stream is accepted**
...
**124. 43 secret refs still arrive through env (APP_API_TOKEN ×10, the zones' OIDC/session/lineage tokens, `ray-lance-head`) while ESO is built, provisioned and switched off**
...
**125. The OpenBao seed Job and the three ExternalSecrets carry no `helm.sh/hook`, so adding one property to an ExternalSecret destroys the whole Secret for ~12 minutes**
...
**126. No auto-unseal for OpenBao — every pod restart or node drain leaves it sealed and the whole fleet hangs at 'waiting for application startup'**
...
**127. Every store the fleet dials is plaintext — 171 `http://` store URLs on the live Deployments, RustFS S3 + STS, OpenFGA, the AGE DSN, OpenBao, the NATS monitor and OTLP**
...
**128. `rask-age` serves TLS-off Postgres for the lineage graph and OpenFGA, and the fix — the AGE→CNPG cutover — is built and unblocked but never taken**
...
**129. No Dapr `accessControl` policy in any chart template, and the actor + workflow invocation planes were never characterised**
...
**130. OpenFGA subjects are raw-interpolated (`f"user:{user}"` in `service_kit/governed/fga.py`) — user IDs are never URL-encoded, and OIDC subjects here are emails**
`service-kit, catalog` · med
...
**131. Estate bootstrap is still check-then-write, and `fga.provision()` rewrites the authorization model on every unpinned boot**
...
**132. The dev cluster runs `auth.enabled=false`, so all seven rendered user doors sit in their dev-open state**
...
**133. pg-dump backups land in the same RustFS bucket the VolumeSnapshot protects, nothing prunes `_backups/` or old snapshots, and `snapshotClassName` is empty**
...
**134. Bootstrap on a fresh machine is not chart-complete: the Ray head is hand-applied `deploy/ray-lance-demo.yaml` and OpenBao's k8s auth backend/policy/role + KV values are seeded by runbook**
...
**135. Residual duplicated storage seams: ingest hand-maps its own 409, a dead line in `storage/client.py`, and three boto3 constructors outside `s3_client`**
...
**136. OpenFGA's pgxpool defaults (MaxOpenConns 30 / MaxIdleConns 10) are untuned against the AGE Postgres running on the default `max_connections=100`**
...
**137. The zero-trust posture is a claim, not a measured target — 1 control missing and 8 partial with nothing re-deriving them**
...
**138. `model.fga.yaml` has no `check` case with a userset subject or the self-referential userset — exactly the behaviours `weighted_graph_check` alters**
...
**139. The Lance writer seam has no create verb, so a fresh estate's first annotation save has nothing to write into**
...
**140. `transaction.can_set_property` and `transaction.can_cancel` are defined in `model.fga` and referenced by no relation and no string in `services/` or `packages/`**
...
**141. The OpenFGA subchart is pinned at 0.3.9 while the running image is v1.18.3, which is 0.3.12's appVersion**
`openfga, chart` · low
...
**142. NATS HA, a nack operator under GitOps, and a query engine are owner-parked with no ruling**
...
**143. The Dapr retreat (D5) has not started: 40/480 source files import the SDK, 2,481 LOC of actors, 3,407 LOC of workflows, 36/53 chart templates**
...
**144. With `frontend.oidc.enabled: false`, `locals.authEnabled` is false and `@rask/api/serve-proxy`'s 401 guard never fires, so anonymous callers reach GPU inference**
...
**145. `chart/templates/dex.yaml` still ships in-memory storage, bcrypt('password') static users and a plaintext client secret, and `values-prod.yaml` has no dex stanza at all**
...
**146. Every actor method accepts a caller-supplied `actor` field; the only guard is pod-network topology, not the app**
...
**147. `chart/values-prod.yaml` sets `ingress.enabled/className/host` but no `tls:` block, so OIDC tokens and vended S3 credentials traverse plaintext at the edge**
...
**148. `security.serviceAccounts`, `security.infraContexts` and `dapr.sidecarRestricted` are all false by default and `values-prod` flips none of them**
`chart` · med
...
**149. The ingress carries no `nginx.ingress.kubernetes.io/proxy-read-timeout`, so the controller's 60 s cut severs every idle `query.live` SSE feed**
`chart, gateway` · med
...
**150. Images carry no signature, and cosign alone would be decoration because the estate runs zero signature verifiers**
...
**151. No ordered prod install runbook exists — the FGA seed / OpenBao unseal / PSA-label ordering footguns are documented only as warnings in values-prod**
`chart` · med
...
**152. `imagePullSecrets` is plumbed into only 3 of 56 chart templates and no first-party image is digest-pinned in values-prod**
`chart` · med
...
**153. Two `test_lineage_e2e.py` cases fail, and nothing routinely points the 111 e2e functions at k3s**
`lineage, e2e` · med
...
**154. Seven seams have no test at all: `objectfs.py`, `lakehouse/blobs.py`, `lancekit/store.py`, `lancekit/reader.py`'s REST path, `audit.py`, `middleware.py`, and `submit_or_reattach`'s delete branch**
...
**155. 111 single-component test files sit in `tests/unit` instead of their service's own testpath, so a catalog change cannot be verified against catalog tests**
...
**156. A subchart names `{{ .Release.Name }}-x` while this chart's Secrets use `lance.fullname` — they agree only when the release is named `rask`**
`chart` · low
...
**157. `pytest-xdist` is still not a dependency and 17 test files roll their own `subprocess.run` helm render**
...
**158. The infra subcharts (GreptimeDB, NATS, Perses, the Dapr control plane) still get no requests/limits from values-prod**
`chart` · low
...
**159. Three live e2e legs skip on a 5 s `/livez` timeout while the medallion producer is up and serving the cascade**
`medallion, e2e` · low
...
**160. The dangling-locator gate checks pointers INTO a file but nothing gates a register's SIDECAR (e.g. an orphaned `*.findings.json`)**
`e2e` · low
...
**161. `make seed-dev` chmods the whole corpus root, is pinned to one release name/node/ports, and seeds doc ids with an already-wrong `dataset_version`**
`scripts, chart` · low
...
**162. `scripts/dev-micro.sh` never starts `:8103`, the annotations plane, and `/capi/v1/me` 502s without a catalog**
...
**163. The P7a/P7b dead-name docs sweep: six nav-served docs still describe the orchestrator, `core_api`/`search_api`/`volumes_api`, `packages/htr` or `/default/<zone>` bases**
`docs` · low
...
**164. `fga model validate` is not in the `ms-authz` CI job, so weighted-graph compatibility stays a hand audit**
...
**165. Decide whether the deferred infra/app chart split becomes the home for rask-operator's chart**
...
**166. Remote branch `claude/flyte-2-dapr-audit-19cyc2` is still not deleted**
...
**167. OpenFGA exports no traces or metrics: `telemetry.trace.otlp.endpoint` is unset and the estate scrapes no Prometheus endpoint**
...
**168. `request_id` + actor propagation has zero hits in `services/`, and the verdict on whether OTel plus the audit trail supersede it was never recorded**
...
**169. Two Kueue controllers reconcile one set of CRDs — the resulting `kueue-ca` handshake spam runs ~450/min, plus two failing otel-collector scrape targets**
...
**170. `chart/values-prod.yaml` never sets `observability.environment`, so every OTel resource attribute labels prod telemetry with the chart default**
`chart` · low
...
**171. Undecided whether the estate shares one GreptimeDB or runs one per workload**
`chart` · low · **blocked:** owner decision
...
**172. The Ray/compute S3 identity enumerates 105 buckets, and one shared dashboard token reads every job's `runtime_env` past the vended credential's 900s TTL**
...
**173. `services/compute` still runs an image without `DiagnosticFormatter`, so its `extra=` diagnostics are dropped**
...
**174. The compute zone's actual Ray Serve / dashboard call set is unknown, so the dashboard exposure cannot be narrowed**
...
**175. Nothing in the FGA model governs execution — no zone, compute-job or run type — and whether that boundary is deliberate is undecided**
...
**176. `ensure_dataset` runs before enumeration, so a mis-pointed source leaves a registered empty bronze table and the run reports COMPLETE**
...
**177. `drain_chunk` batches all redeliveries in one fetch regardless of origin batch, so two crashed batches' remainders merge into an undecidable fragment**
`ingest` · med
...
**178. Ingest's reads of the estate-default store and of secretless registered stores have no scoped credential now that the ambient root pair is withdrawn**
...
**179. Enumeration has no BYTE ceiling (the unit and time ceilings landed and gate A15's enforcement half now exists)**
`ingest, chart` · low
...
**180. CLOSED IN CODE: `runs.py`'s outcome-status promotion now accepts terminal FAILED and TERMINATED**
`ingest` · low
...
**181. dev-kuberay.ra.se's job-submission API answers 200 with no token and with a wrong token**
...
**182. `runners/htr` is not re-cut as stage runners: the prefetch pipeline and the loader/writer endcaps remain, and `main.py` has no `stage` subcommand**
...
**183. The RayJob CRD path is built but off the deployed path — submission goes through the Jobs API, zero RayJob/RayCluster/RayService CRs exist, and Kueue admits nothing**
...
**184. `RayJobExecutor.status()` maps only `status.jobStatus`, so a RayJob whose cluster never came up reports PENDING forever and can never be resubmitted**
...
**185. `RayJobExecutor` treats any 409 as REATTACHED without reading the CR, and the CR name omits `code_version`**
`medallion, ray-kit` · med
...
**186. `POST /train` on the medallion producer does not resolve the `$n` form of `features[].dataset`**
`medallion` · med
...
**187. `tests/unit/test_ray_auth.py` asserts only `helm template` output, never that a token is honoured**
...
**188. Nothing scrapes the external Ray cluster — zero `ray_*` / `ray_serve_*` / `ray_data_*` / `autoscaler_*` series reach GreptimeDB**
...
**189. Ray core logs never leave the pod — driver/task/actor logs are files under `/tmp/ray` that nothing mounts and nothing tails**
`external-kuberay` · med · **blocked:** the operators of the KubeRay cluster at dev-kuberay.ra.se
...
**190. Serve-plane tracing is switched on nowhere external and no proxy/router/replica span has ever been observed**
...
**191. The chart ships Ray telemetry wiring that no install renders — decide between rendering `rayservice.yaml`, adopting the orphaned `rask-ray`, or dropping it**
...
**192. Ray GCS is not fault-tolerant — a head restart kills in-flight jobs, and the only supported fix needs an external Redis the estate forbids**
...
**193. `ray-kit` imports the heavy `ray` SDK solely for `JobSubmissionClient`, forcing `compute` onto its own 1536Mi tier**
...
**194. `RAY_LOGGING_CONFIG_ENCODING` / `RAY_SERVE_LOG_ENCODING=JSON` are set on no cluster that runs, and no Serve replica line has been seen to land**
...
**195. Confirm `ray_gcs_*` survives token auth before any GCS alert rule is written**
...
**196. Dapr Workflow has no versioning seam — two replay divergences already shipped and "drain before deploying" is the only safe answer**
...
**197. `GET/POST /stage-runners/{stage runner}/stages/{instance_id}` ignores both path parameters, so the wrong stage runner answers**
`medallion` · med
...
**198. The workflow status metric reports SUCCESS on a dying path**
`medallion` · med
...
**199. 1,367 orphan rows in `daprstate` with no TTL and no alert**
...
**200. `compute` is an introspection shell — no submit door with vended credentials, no idempotent outcome door, no plan document on a control lane**
...
**201. The `Transform` CRD is deferred to `rask-operator`, so a lane declaration cannot live in git as a CR with the catalog record as a projection**
...
**202. A stage runner row still carries `stageJob` / `ray_entrypoint` / `ray_job_params` beside the declaration that supersedes them, with `engine_choice` arbitrating**
...
**203. The edge exposes sidecar-only paths — invert lineage_sidecar_guard's blocklist to a per-row allowlist**
`gateway, chart` · **HIGH**
...
**204. Decide how the north-south path authenticates once the edge calls Services directly instead of via dapr-api-token**
...
**205. Nothing replaces the dapr-sentry mTLS that an edge->Service backendRef drops**
...
**206. Dapr invocation resiliency (retries, timeouts, invokeBreaker) leaves with the gateway's sidecar and has no edge equivalent**
`gateway, chart` · **HIGH** · **blocked:** gateway/P2-dapr-token; gateway/P1-edge browser-proven
...
**207. `make dev-micro` loses its stable /api origin when the gateway dissolves — derive a dev proxy from the chart's HTTPRoutes**
...
**208. The edge has no body cap, rate limit, X-Forwarded handling, access log or coded errors**
...
**209. No Gateway/HTTPRoute exists — install kgateway behind a values toggle and render the edge one rule per future backend (nginx stays default)**
`gateway, chart` · med
...
**210. kgateway has no equivalent of nginx's 3600s proxy-read-timeout, so query.live streams would be cut**
...
**211. Move longest-prefix /api routing out of gateway/__init__.py::_routes() into per-service HTTPRoute rules**
...
**212. Zones reach the gateway server-side through two different env vars (RASK_GATEWAY_URL vs LANCE_GATEWAY_URL)**
...
**213. Deleting the gateway removes the north-south OTLP span the Perses 'Fleet — RED' panels read**
...
**214. Envoy path-normalization parity with _normalize_path (merge_slashes, `..` segments) is assumed, not checked**
`gateway, chart` · med · **blocked:** gateway/P2-routes
...
**215. services/gateway, its dockerfile, chart Deployment and dev-micro.sh entry all still exist**
...
**216. The Dapr helper comments' "routes become HTTPRoutes" note is neither acted on nor deferred**
...
**217. Name the 502-with-detail -> Envoy 503 change for unreachable upstreams**
...
**218. Decide the fate of the merged /docs and fleet-wide openapi.json**
`gateway` · low
...
**219. The "no CRD without its controller" ruling is enforced by nothing but the absence of a file**
...
**220. controlplane ProjectStatus carries only `phase` and `namespace` — no conditions[], observedGeneration or catalogProjectId**
...
**221. No managed surfaces for roles and identities over the FGA model**
...
**222. The models registry has no MLflow-parity feature set**
...
**223. notifications cannot hold a dedicated service credential — daprd overwrites dapr-api-token on service invocation**
...
**224. No subject-erasure door, no TTL on watches/prefs/cursor, and no reverse index for a subject**
`notifications` · med
...
**225. Sidecar proxies return bare 500s and block per call instead of reusing a lifespan-built proxy**
`notifications` · low
...
**226. .claude/skills/rask-notifications/SKILL.md contradicts the code in eight named places**
`notifications` · low
...
**227. No in-tab browser memo for `/api/atlas/points` — 6,679,228 bytes refetched on every mount and every Text/Visual toggle**
...
**228. 13 hand-rolled poll/loading/401/offline fetch triples instead of SvelteKit `load`/`query` — two lineage pages keep rendering governed rows after the session dies**
...
**229. Nothing subscribes to lineage's per-subject `GET /events` keyset cursor — 8 of 13 lakehouse pollers still poll and four admin surfaces make zero requests ever**
...
**230. A lineage jobs render does not pass `summary: true` — 464,318 bytes per render instead of 46,980**
`lineage, lakehouse-zone` · med
...
**231. No UI reaches `undrop` on either rung — neither `POST /v1/table/{id}/undrop` nor the plural `POST /v1/namespace/{id}/undrop`**
...
**232. The project-scoped maintenance policy has an API but no UI**
`catalog, lakehouse-zone` · med
...
**233. The chart ships only the ingress-nginx proxy-read-timeout annotation, inert on k3s's Traefik — the zones' live SSE bell has no edge-level guard there**
`chart, gateway` · low
...
**234. Six mutation sites still do a trailing `await load()` after every write instead of `form` + single-flight + `withOverride`**
`lineage, viewer` · low
...
**235. The lakehouse zone's `/governance` route was removed and nothing decided whether the catalog layer should have one**
...
**236. Nobody has decided what `/projects/<id>` is FOR, or whether the hierarchy graph belongs on it**
...
**237. There is no Estate Settings zone — estate-level settings have no home in the UI**
...
**238. Kill the zones' VIEWER_BACKEND :8888 vs LANCE_BACKEND :8001 dev-proxy split**
...
**239. The WebGPU atlas is still zone-local in explorer instead of hoisted to `frontend/packages/atlas`**
...
**240. services/annotator keys ownership on a client-settable `X-User` header — no OIDCVerifier, `get_author` defaults to "anon"**
...
**241. Consensus replicas mint unaddressable `{gid}-r{k}` task ids that wedge publish, and all N replicas share one per-task draft doc**
`annotator` · med
...
**242. S10 — the media-plane Lance write path is still the annotator's live write: `annotations/{save,tags,versions}.py` + `commit.py:check_base_version_value`, and the canvas still writes it (615 versions / 9.8 MB for 3 rows)**
...
**243. S1 — `services/annotator/projects/{__init__,schema,machine}.py`: the entities and the pure `apply(entity, event, *, actor, rungs, now)` do not exist**
`annotator` · med
...
**244. S2 — the `annotation_project` FGA type and `can_create_annotation_project` are absent from `service_kit/governed/auth/model.fga`**
`service-kit, annotator` · med
...
**245. S3 — `services/annotator/projects/publish.py`: `PUBLISHED_LABELS_SCHEMA` (34 columns) and `build_published_table()` do not exist**
`annotator` · med
...
**246. S6 — `ProjectActor` (project doc, claimable queue, tenant index) and `TaskActor` (task state + lease-expiry reminder) are unwritten**
...
**247. S7 — no HTTP surface for annotation projects/tasks/drafts behind the §6.1 FGA doors**
...
**248. S8 — the publish workflow (freeze → snapshot accepted drafts → Arrow → exists/create → tag → record → emit `annotation.project.published`) does not exist**
...
**249. `publish.py:435` stamps the PROJECT template instead of each task's captured one, invents `template_kind`, and publishes client-supplied shape provenance under a server-stamped comment**
`annotator` · low
...
**250. Batch-labeling submit has no runner behind it — `runners.jobsUrl: ""` in every default deploy**
...
**251. The annotator's wire and state still say `project` where the owner ruled the unit of work is a labeling TASK**
...
**252. S9 — the annotator zone still lands on the refused `DataSelection.svelte` dataset→document→chunk gallery**
...
**253. No character-span tool, no audio item has ever gone through the loop, no relation/reading-order tools**
...
**254. `GET /projects/{project_id}/tasks` ignores `limit` and `cursor`**
...
**255. Export serializers (COCO/YOLO/CSV/HF) and a managed label taxonomy are unscheduled, and the export half belongs to the P7c exporter service**
...
**256. `proxyServeInfer` does `await request.arrayBuffer()`, buffering every uploaded byte in a SvelteKit pod sized for rendering HTML**
...
**257. The studio node library marks every Ray-dashboard-reported Serve app RUNNING, so an app with no ingress route appears callable**
...
**258. flows runs still take the inline executor; the Dapr Workflow path has never been observed end to end**
`flows` · low
...
**259. `services/flows` declares the node catalog and the studio zone still ships its own registry**
`flows, frontend` · low
...
**260. No IIIF loader node, no bucket attach, and no streaming — a long run shows nothing until it completes**
...
**261. `https://dev-kuberay.ra.se/htr/transcribe` answers a bare `text/plain 404` — the Serve HTTP ingress exposes no `/htr` route**
...
**262. D2b — no lines FTS surface at all; the governed lines table was never built**
...
**263. D2d — the EAD `archive_catalog` Lance table is frozen and unserved, with no governed re-land**
...
**264. `GET /api/search` (search :8102) silently ignores `dataset` and `mode`**
`search` · low
...
**265. Search binds to ONE declared `row_table` keyed by the identity triple — no table chooser, no non-identity joins, no external-pointer declaration**
...
**266. The media/explorer corpus is read off a mounted volume — `explorer.yaml:109-111` still derives `MEDIA_DB_ROOT`/`MEDIA_DB`/`MEDIA_DESCRIPTOR_DIR` from `explorer.corpusMountPath`**
...
**267. K-F9 — no generic/opaque asset rung; the model registry squats on the `table` FGA type**
