...
**LH-095 · A repeatedly-refused trash record is indistinguishable from a transient one — `attempts`/`last_refusal` are never persisted**
`maintenance, service-kit` · med

- *Why open:* **THE LEASE CLAUSE IS OVER-ASKING and is struck (re-measured 2026-09-10).** It is true that `bindings.cron` fires on every replica and that the service holds no distributed lease — but a pin IS the estate's sanctioned answer for this service (`rask-dapr`: the estate answers cron duplication four different ways, and maintenance's is `replicas: 1`), and here that answer is chosen, written down and ENFORCED rather than assumed. Measured live: `replicas=1 strategy=Recreate`, `replicas: 1` is HARDCODED in `chart/templates/maintenance.yaml` and not reachable through values at all, and `tests/unit/test_invariants.py::test_the_sweep_lock_is_only_correct_while_maintenance_CANNOT_scale` fails the moment `replicas` stops being a literal 1 while the lock is still an `asyncio.Lock`. So parameterising the deployment FORCES the distributed-lock conversation instead of silently starting a second sweep. Adding a lease now buys nothing the gate does not already guarantee.
  **The refusal-state clause is REAL and untouched**: `attempts` and `last_refusal` appear nowhere in `service_kit/lakehouse/trash.py` or `maintenance/services/purge.py` (grepped at HEAD). The purge reports refusals per tick through `RefusedRecord`, and nothing carries across ticks — so a record refused every five minutes for thirty days reads exactly like one refused once, and a permanent exclusion is invisible as permanent.
- *Closes when:* Persist `attempts` and `last_refusal` on the trash record (`service_kit/lakehouse/trash.py`, beside `expires_at`) and consume them in the purge so a permanent exclusion reads as permanent. Note the cost this adds and decide it deliberately: the trash record becomes MUTABLE and the purge gains a write per refused record per tick, where today it only reads — so the read-modify-write wants the conditional `records.mutate_json` primitive rather than a plain overwrite, for the same reason the warehouse registry does. The lease half is struck; see above.
...
- *A LIVE OOM, 2026-09-10, on the one service that ALREADY has the bounded session.* `rask-maintenance` was OOMKilled (exit 137, `reason: OOMKilled`) against its 512Mi limit and restarted; steady-state is 153Mi. It happened while three manual `/maintenance-reconcile-cron` invocations overlapped the 5-minute scheduled ticks — self-inflicted, and the pod recovered unaided — so this is a data point about HEADROOM, not a standing outage.
  ...
  It also means the reconcile route is re-entrant enough to be dangerous by hand: `_reconcile_lock` is documented to SKIP rather than queue, so three overlapping calls should not have run three full scans. Whether they did, and whether the OOM is that skip failing, is unmeasured and worth one look before any sizing work.
...
- *Why open:* Two of three landed and were observed (`_lineage_outbox` added to `_CONTROL_PREFIXES`; a concurrent delete below the walk root tolerated), dropping incomplete 71 → 61. The walk stops at depth 3 and cannot tell 'no dataset here' from 'did not look deep enough'; the lever exists (`maintenance.discoveryMaxDepth`, bounded 1..16) but the default is still 3. **The "absent from the running pod" half is FALSIFIED (re-measured 2026-09-10): `MAINTENANCE_DISCOVERY_MAX_DEPTH=3` IS set in `rask-maintenance`** — the setting reached the pod, it is simply the default.
  Re-measured the same day by invoking the reconcile route in-pod and reading the reasons the log line does not carry: **64 incomplete units, ALL of them `depth limit reached`**, the majority on `s3://lance-catalog/models/<model>/<hash>` with the rest in tenant warehouses. Combined with this row's own measurement that depth 6 finds no extra datasets, that makes them FALSE COVERAGE GAPS: the walk stops where nothing scannable exists and reports that it did not look deep enough. Raising the depth is therefore the wrong lever — the walk needs to tell 'no dataset here' from 'did not look deep enough', which is what this row already says.
- *Closes when:* **THE DECISION IS NO LONGER ABOUT THE DEPTH.** Re-measured 2026-09-10, the lever is already deployed (`=3`) and this row's own note says depth 6 finds no extra datasets — so raising it costs a wider scan across 93 buckets and buys nothing, while the 64 gaps stay. The gaps are FALSE: `optimize.py:206-209` records a truncation for any non-dataset directory at the depth bound without asking whether anything could be under it, and `models/<model>/<hash>/` holds artifacts, not datasets.
  The exact cheap test is one listing per truncated prefix: **a directory whose children are all FILES cannot hide a dataset**, because a Lance dataset is a directory with a `_versions/` child (`objectfs.is_lance_dataset_root`, the estate's one definition). Record a truncation only when a subdirectory actually exists below the bound. 64 extra listings per tick against 93 buckets, and no coverage is lost — the gaps it stops reporting are prefixes it has just proven empty of directories.
  **THE OWNER DECISION IS WHAT THAT UNBLOCKS, not the scan cost.** `report_is_clean` gates the #79 expired-trash purge, so a report that stops carrying 64 permanent false gaps may start certifying an estate the purge was previously refusing to touch. That is the intended outcome — LH-094 records that nothing is being reclaimed — but it is a DESTRUCTION posture change and must be chosen, not arrived at. `MAINTENANCE_TRASH_PURGE_ENABLED` is report-only by default, which bounds it. RED test first, pinning that a directory of files at the bound is not a coverage gap and a directory of directories still is.
...
- *Closes when:* PATH 1 FIRST and on its own: give `_outbox_storage_options` a real credential for `s3://lance-catalog/_lineage_outbox`. The prefix is a CONSTANT, so it needs no per-table vending — but ingest holds no static S3 key by design (STS-only, which is the stronger posture), and the catalog vends only per table (`POST /v1/table/{id}/credentials`), so this needs either a prefix-scoped vending door or the medallion's precedent (a scoped identity read from the Dapr secret store, never env). Then prove it by staging an event and watching the reconcile cron drain it — which also unblocks LH-086. THEN paths 2 and 3: register the source buckets as stores that DECLARE a `secret` naming a scoped identity in the Dapr secret store, so `objectstore._own_store_for` stops falling back to `without_credentials`; the machinery landed 2026-09-08 (`06f0ab21`) and is inert until an operator aims it.
