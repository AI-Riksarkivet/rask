...
**13 pages + a dock** (measured 2026-08-07 — `find src/routes -name '+page.svelte'`): `/` (overview), `/jobs`, `/jobs/[id]`, `/actors`, `/cluster`, `/serve`, `/gpu`, `/logviewer`, `/api-docs` (iframes `/api/docs`), `/etl` (the ingest form, BUILT from the source-kind registry rather than restating it: it reads the kinds and their fields from `GET /api/ingest/sources`, so a registered kind such as `s3-prefix`, built on `service_kit.lakehouse.sources.S3FileSystemSource`, reaches the product without a page edit), `/ingest` (the run board, read from LINEAGE), `/ingest/[run_id]` (that run's detail), `/query` (scaffold — no query backend yet), and `/workbench`. The views themselves live in `$lib/boards/*` per the panel-is-the-page rule, so the routes stay thin and the dock's panels render the page's component.
