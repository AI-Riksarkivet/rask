...
| | |
|---|---|
| **Populated data is per zone** | Mocks answer 404 until seeded (by design — a mock with baked-in fixtures cannot tell a live surface from a dead one), so a zone renders EMPTY unless it ships `e2e/dev-seed.ts`. **Only `lakehouse` has one** (4 catalog routes, 2 observability, 3 lineage runs — verified rendering as real rows). The launcher announces a zone with none rather than leaving blank ambiguous between "no data" and "broken". |
