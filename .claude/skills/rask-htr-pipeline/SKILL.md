...
| Retargeting GPUs = edit **all three** files | `pipeline.py` + `transcribe_service.py` + `htrflow_service.py` | GPU sizing is hardcoded for a 3-GPU node (Serve fracs are env-overridable; pipeline pool sizes are literals). Changing target hardware means touching every one. |
...
- **`co-reside on a sub-3-GPU pool` is the *Serve* default.** A full 3-GPU node can run more — but raise `RASK_SERVE_REPLICAS`/`RASK_SERVE_GPU_FRAC` deliberately and re-check the fractional-sum ≤ physical-GPUs invariant and host-RAM headroom.
