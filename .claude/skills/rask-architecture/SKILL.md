...
Current Python services (11): `gateway`, `compute`, `controlplane`, `ingest` — plus the lance plane (`catalog`, `lineage`, `medallion`, `maintenance`, `viewer`, `search`, `annotator`). (`compaction` was renamed `maintenance` in 06cc7579 — it compacts, optimizes indices, cleans up old versions *and* reconciles cross-store drift, so it is named for all four, not one. `core`/`core_api`/`search_api`/`volumes_api` died in the R6/R20 media wave.)
