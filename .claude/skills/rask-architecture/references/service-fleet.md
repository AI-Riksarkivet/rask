...
monolith — single-process dev convenience over the same `core` package.
...
| `orchestrator` | 8810 | `core` (same package) | `core.lifespan.make_lifespan` | yes | health + orchestrator endpoints; loop **ON** via `RASK_ORCHESTRATOR_AUTOSTART` |
...
Both compose the **same `core` package** with the **same `make_lifespan`**. They
...
set. They run as **two processes over one package, sharing the `batches` table
...
factory. Heavy deps live in the package that needs them (`core` → sqlmodel,
...
down on exit. This is the seam that lets one package (`core`) back two processes
