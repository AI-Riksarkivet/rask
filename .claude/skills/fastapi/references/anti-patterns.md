...
| `BackgroundTasks` for anything you'd page on                          | No retry, dies with the worker.                                 | Use a real queue: **NATS JetStream** (background-jobs.md) or **DBOS workflows** for sagas (durable-workflows.md). Both in `python-infrastructure`. |
