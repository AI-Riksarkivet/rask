...
# The RayCluster head renders `.Values.ray.image` (templates/_ray-cluster-config.tpl), not `image.tags`,
# so its pin is written where the chart reads it; under `image.tags` it would restore nothing.
ray_tag = tags.pop("ray-cluster", None)
ray_block = f"ray:\n  image:\n    tag: {json.dumps(ray_tag)}\n" if ray_tag else ""
sys.stdout.write(
    ...
    "image:\n" + block("tags", tags) + block("digests", digests) + ray_block
)
' > "$OUT.tmp"
