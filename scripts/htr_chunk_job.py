...
def s3_client() -> Any:  # noqa: ANN401 — boto3 has no public client type
    kwargs: dict = {
...
def list_keys(s3: Any, bucket: str, prefix: str, suffix_ok) -> list[str]:  # noqa: ANN401 — boto3 client, see s3_client()
    out: list[str] = []
...
def transcribe(endpoint: str, img: bytes) -> str:
    # S310: `endpoint` is the operator-supplied Ray Serve URL for this job, never
    # user input, and always http(s) — there is no scheme to smuggle.
    req = urllib.request.Request(endpoint, data=img, headers={"Content-Type": "application/octet-stream"}, method="POST")  # noqa: S310
