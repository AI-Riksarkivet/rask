<script lang="ts">
	...
	function onKey(e: KeyboardEvent) {
		...
		if ((e.key === 'ArrowLeft' || e.key === 'h' || e.key === 'k') && prevPage) {
			...
			e.preventDefault();
			goto(`${base}/viewer/${encodeURIComponent(volume)}/${encodeURIComponent(nextPage.key)}`, {
				replaceState: false,
				noScroll: true,
			});
		} else if (e.key === 'f') {
			...
			>
				<Button
					...
					onclick={() =>
						prevPage &&
						goto(
							`${base}/viewer/${encodeURIComponent(volume)}/${encodeURIComponent(prevPage.key)}`,
							{
								noScroll: true,
							},
						)}
				...
				<Button
					...
					onclick={() =>
						nextPage &&
						goto(
							`${base}/viewer/${encodeURIComponent(volume)}/${encodeURIComponent(nextPage.key)}`,
							{
								noScroll: true,
							},
						)}
