...
RASK_VIEWER_INPUT=s3://images-batch RASK_VIEWER_OUTPUT=s3://images-batch-alto \
  uv run uvicorn viewer.main:app --host 0.0.0.0 --port 8888
```

Or via the FastAPI CLI:

```bash
uv run fastapi run viewer.main:app --port 8888
```

### Against postgres

```bash
uv pip install -e components/services/viewer[postgres]    # adds asyncpg
...
```bash
uv run pytest components/services/viewer/tests
```
