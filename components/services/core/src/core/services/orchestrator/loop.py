"""Periodic orchestrator tick — runs inside the core process via lifespan.
...
# consumer so the orchestrator survives core restarts and scales horizontally.
...
from core.services.orchestrator.derive import derive_state
from core.services.submission import submit_chunk
...
async def run_loop(
    *,
    ...
    If Ray was unreachable at core boot, `ray_client` is None and stays that way for
