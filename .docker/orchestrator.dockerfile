...
# rask orchestrator image — FastAPI on python:3.13-slim-bookworm.
# Build from repo root:
#   docker buildx build -f .docker/orchestrator.dockerfile -t orchestrator:dev .
...
RUN --mount=type=cache,target=/root/.cache/uv \
    ...
    --mount=type=bind,source=projects/orchestrator/pyproject.toml,target=projects/orchestrator/pyproject.toml \
...
COPY projects/orchestrator projects/orchestrator
RUN --mount=type=cache,target=/root/.cache/uv \
...
LABEL org.opencontainers.image.source="https://github.com/AI-Riksarkivet/rask" \
      org.opencontainers.image.title="rask-orchestrator" \
      org.opencontainers.image.description="rask orchestrator on :8810"
...
HEALTHCHECK --interval=15s --timeout=3s --start-period=20s \
  CMD curl -fsS http://127.0.0.1:8810/api/v1/health || exit 1

ENTRYPOINT ["/usr/bin/tini", "--"]
CMD ["uvicorn", "orchestrator:app", "--host", "0.0.0.0", "--port", "8810", \
