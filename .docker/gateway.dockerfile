...
# rask gateway image — FastAPI on python:3.13-slim-bookworm.
# Build from repo root:
#   docker buildx build -f .docker/gateway.dockerfile -t gateway:dev .
...
RUN --mount=type=cache,target=/root/.cache/uv \
    ...
    --mount=type=bind,source=projects/gateway/pyproject.toml,target=projects/gateway/pyproject.toml \
...
LABEL org.opencontainers.image.source="https://github.com/AI-Riksarkivet/rask" \
      org.opencontainers.image.title="rask-gateway" \
      org.opencontainers.image.description="rask gateway — reverse proxy on :8888"
...
HEALTHCHECK --interval=15s --timeout=3s --start-period=20s \
  CMD curl -fsS http://127.0.0.1:8888/api/v1/docs || exit 1
