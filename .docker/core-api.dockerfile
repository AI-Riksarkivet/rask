...
# rask core-api image — FastAPI on python:3.13-slim-bookworm.
# Build from repo root:
#   docker buildx build -f .docker/core-api.dockerfile -t core-api:dev .
...
RUN --mount=type=cache,target=/root/.cache/uv \
    ...
    --mount=type=bind,source=projects/core-api/pyproject.toml,target=projects/core-api/pyproject.toml \
...
COPY projects/core-api projects/core-api
RUN --mount=type=cache,target=/root/.cache/uv \
...
LABEL org.opencontainers.image.source="https://github.com/AI-Riksarkivet/rask" \
      ...
      org.opencontainers.image.description="rask core-api — batches/chunks/catalog on :8801"
...
HEALTHCHECK --interval=15s --timeout=3s --start-period=20s \
  CMD curl -fsS http://127.0.0.1:8801/api/v1/health || exit 1

ENTRYPOINT ["/usr/bin/tini", "--"]
CMD ["uvicorn", "core_api:app", "--host", "0.0.0.0", "--port", "8801", \
