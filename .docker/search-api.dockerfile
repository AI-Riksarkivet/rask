...
# rask search-api image — FastAPI on python:3.13-slim-bookworm.
# Build from repo root:
#   docker buildx build -f .docker/search-api.dockerfile -t search-api:dev .
...
RUN --mount=type=cache,target=/root/.cache/uv \
    ...
    --mount=type=bind,source=projects/search-api/pyproject.toml,target=projects/search-api/pyproject.toml \
    --mount=type=bind,source=packages,target=packages \
    --mount=type=bind,source=components,target=components \
    uv sync --frozen --no-install-workspace --package search-api --no-editable

# Step 2: COPY real sources and resolve workspace deps (locked).
COPY pyproject.toml uv.lock ./
COPY packages    packages
COPY components  components
COPY projects/search-api projects/search-api
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --locked --package search-api --no-editable
...
LABEL org.opencontainers.image.source="https://github.com/AI-Riksarkivet/rask" \
      org.opencontainers.image.title="rask-search-api" \
      org.opencontainers.image.description="rask search-api on :8802"
...
HEALTHCHECK --interval=15s --timeout=3s --start-period=20s \
  CMD curl -fsS http://127.0.0.1:8802/api/v1/health || exit 1

ENTRYPOINT ["/usr/bin/tini", "--"]
CMD ["uvicorn", "search_api:app", "--host", "0.0.0.0", "--port", "8802", \
