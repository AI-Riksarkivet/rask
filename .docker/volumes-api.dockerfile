...
# rask volumes-api image — FastAPI on python:3.13-slim-bookworm.
# Build from repo root:
#   docker buildx build -f .docker/volumes-api.dockerfile -t volumes-api:dev .
...
RUN --mount=type=cache,target=/root/.cache/uv \
    ...
    --mount=type=bind,source=projects/volumes-api/pyproject.toml,target=projects/volumes-api/pyproject.toml \
    --mount=type=bind,source=packages,target=packages \
    --mount=type=bind,source=components,target=components \
    uv sync --frozen --no-install-workspace --package volumes-api --no-editable

# Step 2: COPY real sources and resolve workspace deps (locked).
COPY pyproject.toml uv.lock ./
COPY packages    packages
COPY components  components
COPY projects/volumes-api projects/volumes-api
RUN --mount=type=cache,target=/root/.cache/uv \
    uv sync --locked --package volumes-api --no-editable
...
LABEL org.opencontainers.image.source="https://github.com/AI-Riksarkivet/rask" \
      org.opencontainers.image.title="rask-volumes-api" \
      org.opencontainers.image.description="rask volumes-api on :8803"
...
HEALTHCHECK --interval=15s --timeout=3s --start-period=20s \
  CMD curl -fsS http://127.0.0.1:8803/api/v1/health || exit 1

ENTRYPOINT ["/usr/bin/tini", "--"]
CMD ["uvicorn", "volumes_api:app", "--host", "0.0.0.0", "--port", "8803", \
