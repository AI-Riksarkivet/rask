...
# rask compute-frontend microfrontend — SvelteKit SSR (Ray/cluster domain) built with
# Bun and RUN by the Bun runtime (svelte-adapter-bun). Build context = repo root.
#   docker buildx build -f .docker/compute-frontend.dockerfile \
#     --build-arg BUILD_DATE=$(date -u +%FT%TZ) \
#     --build-arg VCS_REF=$(git rev-parse HEAD) \
#     --build-arg VERSION=$(git describe --always) -t compute-frontend:dev .
#
...
# All JS workspace members so `bun install` + prepare scripts resolve. @rask/api is JIT
# (no build); @rask/ui is packaged below. Siblings are build-stage only — never shipped.
...
# hadolint ignore=DL3059
RUN --mount=type=cache,target=/root/.bun/install/cache \
    bun run --cwd components/apps/compute-frontend build
...
LABEL org.opencontainers.image.created="${BUILD_DATE}" \
      ...
      org.opencontainers.image.title="rask-compute-frontend" \
      org.opencontainers.image.description="rask Compute microfrontend (SvelteKit SSR, base /compute), Bun server"
...
COPY --from=builder --chown=10001:10001 /src/components/apps/compute-frontend ./components/apps/compute-frontend

WORKDIR /app/components/apps/compute-frontend
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s \
  CMD bun -e "fetch('http://127.0.0.1:3000/compute/overview').then(r=>process.exit(r.status<500?0:1)).catch(()=>process.exit(1))"
