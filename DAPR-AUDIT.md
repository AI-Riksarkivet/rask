...
No shipped path sets the toggle: `make k3s-up` (`Makefile:399`) passes only `hfToken`/rustfs creds, `chart/values-prod.yaml` does not set it, `Tiltfile:207` sets `explorer.enabled` but never `singleTenant`. Aggravators:
...
`chart/templates/dapr-resiliency.yaml:109-113` adds viewer/search/annotator to `targets.apps` behind `{{- if .Values.explorer.enabled }}`. `Tiltfile:166-190` renders a fixed `FLEET_TEMPLATES` list with `--set explorer.enabled=true` (`Tiltfile:207`) — and **`templates/dapr-resiliency.yaml` is not in that list**. The Resiliency CR therefore still comes from the `make k3s-up` release, rendered at the default `explorer.enabled: false` (`chart/values.yaml:663`).
...
Invocation is genuinely live: `RASK_DAPR_ENABLED=true` in the gateway pod, `__init__.py:87-97` routes `/api/explorer*` to viewer/search/annotator, `:152-158` sends them to `http://127.0.0.1:3500/v1.0/invoke/<app-id>/method`, and from inside the gateway `GET :8888/api/explorer/health → 200` (a bogus app-id control returns 500 `ERR_DIRECT_INVOKE`, so the 200s are real Dapr resolutions).
...
| `Tiltfile:287-288` | "Tilt does not override it" — `Tiltfile:207` does (`--set explorer.enabled=true`) | delete the stale comment |
...
- **"The merge disabled the estate's only actor host"** — chronologically impossible: `c0fde61` landed 2026-07-27, the first actor landed 2026-07-28 in `4716a1a`. `explorer.enabled: false` is a documented opt-in default; the actor host is running right now.
