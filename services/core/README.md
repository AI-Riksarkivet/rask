...
```bash
uv pip install -e services/core[postgres]    # adds asyncpg
...
The SvelteKit catch-all app lives at `frontend/microfrontends/home/`; the rest are the
per-domain microfrontends under `frontend/microfrontends/`. Dev flow:
