...
async def produce(
    ...
    *,
    ...
    # THE SPAN COVERS THE WHOLE OPERATION, not a prefix of it. It used to open around the Lance SEED
    # only, so it closed before `build_run_event` and before the outbox publish — and a run whose
    # cascade never fired left a `medallion.produce` span reporting SUCCESS. It also did not exist at
    # all when `compute_enabled` is false, which is exactly the configuration where the head emits a
    # SYNTHETIC event and is most worth watching.
    #
    # Not the anti-pattern observability.md warns about: that one is wrapping a ROUTE body, which
    # duplicates the auto-created HTTP span. This is the service function the route calls, and the
    ...
    with tracer.start_as_current_span("medallion.produce") as span:
        ...
        try:
            ...
            # HEAD did not, so the one operation whose failure stops the whole pipeline was the one
            # reporting UNSET. The route still answers 503 — this is about which span carries the fact.
            span.set_status(Status(StatusCode.ERROR, "publish_failed"))
