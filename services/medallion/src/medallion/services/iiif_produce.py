...
and lands them as the RAW page-image blob-v2 Lance dataset, then emits the ONE ``iiif -> raw`` OpenLineage
raw-write event. It **never publishes ``medallion.raw`` itself**: the producer's own ``/raw-arrival``
subscription reacts to the COMPLETE raw write and fires the cascade — publishing both would double-fire it
(pinned by ``tests/unit/test_medallion.py``). The raw dataset *is* the page cache now: versioned,
governed, and promoted to bronze by the raw→bronze mover's MEDIA path (stage stamp, ``source_rowid``,
thumbnail/embedding); HTR then runs as cascade compute (bronze→silver layout/lines, silver→gold
...
#: The bare input namespace on the raw-write event: ``(iiif, <volume/manifest id>)`` — the graph records
#: which manifest a raw page dataset was harvested from without pretending IIIF is a Lance dataset.
IIIF_INPUT_NAMESPACE = "iiif"


def iiif_head_enabled(settings: MedallionSettings) -> bool:
    """Whether the IIIF ingest head is configured — compute + a raw target URI.

    Unlike the media head no source bucket is needed (the source is the IIIF API itself), and a local-path
    ``iiif_raw_uri`` is a legitimate dev configuration — so only compute + the target gate it."""
    return bool(settings.compute_enabled and settings.iiif_raw_uri)
...
def _harvest_and_ingest(settings: MedallionSettings, raw_uri: str, volume_id: str, max_pages: int | None) -> IngestResult:
    """Blocking half (IIIF HTTP + Lance IO — callers threadpool it): harvest every page of the volume and
    land the raw blob-v2 page dataset in one atomic overwrite commit."""
...
def _measure_raw(raw_uri: str, storage_options: dict[str, str]) -> IngestResult:
    """Measure a raw dataset the RAY job wrote (blocking — threadpool it): the head still owns the ONE
    raw-write emit, so it reads version/rows/schema off the committed dataset, exactly like the movers'
    measure-after-Ray path. ``source_uris`` stays empty — the IIIF input is the bare manifest id."""
    dataset = lance.dataset(raw_uri, storage_options=storage_options or None)
    return IngestResult(
...
async def ingest_iiif(
    ...
    *,
    ...
    """Harvest one IIIF volume into the raw page-image dataset and emit its ONE raw-write event.
    ...
    cascade trigger is NOT published here — ``/raw-arrival`` fires it off this very event.

    ``project`` (#84 per-tenant routing) routes the volume to ``<project-root>/medallion/<raw_namespace>``
    and qualifies the run-id seed + lineage identities, so volumes/tenants never collide; fail closed on
    an unresolvable project (never the shared ``iiif_raw_uri``).
    """
    if not iiif_head_enabled(settings):
        return {"status": "iiif_disabled"}
    token = token or uuid.uuid4().hex[:12]
    raw_uri = settings.iiif_raw_uri
    if project:
    ...
    with tracer.start_as_current_span("medallion.ingest_iiif") as span:
        ...
        if settings.ray_enabled:
            ...
            await submit_iiif_ingest_job(settings, raw_uri=raw_uri, volume_id=volume_id, max_pages=max_pages, token=token)
            result = await run_in_threadpool(_measure_raw, raw_uri, settings.storage_options())
        else:
            span.set_attribute("lance.medallion.compute", "in_process")
            result = await run_in_threadpool(_harvest_and_ingest, settings, raw_uri, volume_id, max_pages)
    ...
    event = build_run_event(
        ...
        # ONE bare input — the harvested manifest. Provenance per page lives IN the data (source_uri,
        # volume_id, page_key columns), so the graph stays one node per volume, not one per page.
        inputs=[(IIIF_INPUT_NAMESPACE, volume_id)],
        output_namespace=project_namespace(project, settings.raw_namespace),
        output_name=project_namespace(project, settings.iiif_raw_dataset),
    ...
    try:
        ...
        # IS the cascade head — /raw-arrival fires the medallion.raw trigger off it — so a failed emit
        # surfaces (503, retryable: overwrite + deterministic run id make the retry converge).
    ...
    dataset = project_namespace(project, settings.iiif_raw_dataset)
