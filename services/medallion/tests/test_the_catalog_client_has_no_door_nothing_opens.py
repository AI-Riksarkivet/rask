...
`register_stage_output` lost its last production caller when the stage runner started ASKING the catalog
where to write (`ensure_stage_output`) instead of writing first and telling it afterwards — the
reordering `test_stage_runner_writes_where_the_catalog_says.py` pins. The function stayed, and with it a
...
and `test_stage_runner_writes_where_the_catalog_says.py` all `monkeypatch.setattr(transform.catalog_register,
...
def test_the_catalog_root_setting_has_a_reader() -> None:
    ...
    a REGISTER call needs. When the stage runners stopped registering (they ask, and write to the absolute
    ...
    the bronze->silver stage runner's `MEDALLION_FROM_URI` from one expression), so it tells the catalog rather
    ...
    assert "catalog_root" in MedallionSettings.model_fields, "the chart renders MEDALLION_CATALOG_ROOT onto the producer and both stage runners"
