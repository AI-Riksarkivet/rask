...
import pytest
from medallion.services.lane import UndeclaredLaneError, resolve_lane

from service_kit.lakehouse import transform_specs
from service_kit.lakehouse.transform_specs import TransformSpec


def _settings(tmp_path: Path, lane: str) -> Any:
    from types import SimpleNamespace

    return SimpleNamespace(lane=lane, control_root=str(tmp_path), storage_options=lambda: {})
...
def test_an_unset_lane_resolves_to_None_so_the_chart_settings_still_govern(tmp_path: Path) -> None:
    ...
    assert resolve_lane(_settings(tmp_path, lane=""), project="acme") is None


def test_a_declared_lane_resolves_to_its_record(tmp_path: Path) -> None:
    _declare(tmp_path)

    spec = resolve_lane(_settings(tmp_path, lane="dummy"), project="acme")
...
def test_a_NAMED_but_UNDECLARED_lane_REFUSES_rather_than_falling_back(tmp_path: Path) -> None:
    ...
    with pytest.raises(UndeclaredLaneError) as caught:
        resolve_lane(_settings(tmp_path, lane="dummy"), project="acme")
...
def test_a_lane_declared_for_ANOTHER_project_does_not_resolve(tmp_path: Path) -> None:
    ...
    with pytest.raises(UndeclaredLaneError):
        resolve_lane(_settings(tmp_path, lane="dummy"), project="acme")
...
def test_a_lane_with_no_project_in_scope_REFUSES(tmp_path: Path) -> None:
    ...
    with pytest.raises(UndeclaredLaneError):
        resolve_lane(_settings(tmp_path, lane="dummy"), project="")
...
def test_a_lane_with_no_control_root_REFUSES_rather_than_reading_nothing() -> None:
    ...
    settings: Any = SimpleNamespace(lane="dummy", control_root="", storage_options=lambda: {})

    with pytest.raises(UndeclaredLaneError, match="MEDALLION_CONTROL_ROOT"):
        resolve_lane(settings, project="acme")
