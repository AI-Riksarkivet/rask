...
`medallion.services.ray_jobs_api.submit_or_reattach(..., on_terminal_failure="report")`. A comment describing removed
...
def test_ray_submit_names_no_removed_terminal_constants() -> None:
    ...
    for dead in ("_TERMINAL_BAD", "_TERMINAL_OK"):
        assert dead not in source, (
            ...
            "medallion.services.ray_jobs_api.submit_or_reattach — a comment describing removed code must go with it"
        )
