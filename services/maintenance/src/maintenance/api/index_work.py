...
async def handle_index_unit(event: dict[str, Any], settings: MaintenanceSettings, emitter: MaintenanceEmitter) -> dict[str, str]:
    ...
    try:
        ...
        # SUCCESS, not RETRY: the catalog refused this identity a write credential for this table, or has no
        # table by this id, and that answer does not change on the next delivery — retrying would redeliver
        # forever while the index is never built. Building it anyway under `options`, the ambient key, is the
        # bypass the rewrite path refuses for the same reason: an index build writes files under the table's
        # own prefix, so it is a write. The unit is dropped loudly, with the door's answer.
