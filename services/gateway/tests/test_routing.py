...
def test_ray_rows_map_to_the_ray_service(gw) -> None:
    routes = gw._routes()
    ...
    assert gw._pick_route("/api/serve/status", routes)[2] == "ray"
...
def test_target_base_uses_sidecar_when_enabled(gw, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    base = gw._target_base("ray", "http://127.0.0.1:8804")
    assert base == "http://127.0.0.1:3500/v1.0/invoke/ray/method"
