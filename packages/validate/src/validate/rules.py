...
from validate.images import ValidationError
    ...
    *,
    ...
    def check(path: Path) -> None:
        ...
        if w < min_w or h < min_h:
            raise ValidationError(path, f"Image too small: {w}x{h} (minimum: {min_w}x{min_h})")
        if w > max_w or h > max_h:
            raise ValidationError(path, f"Image too large: {w}x{h} (maximum: {max_w}x{max_h})")
...
def allowed_extensions(*exts: str) -> Rule:
    ...
    return Rule(name=f"allowed_extensions({', '.join(sorted(normalized))})", check=check)
