...
| # | where | what |
| --- | --- | --- |
| M1 | `Makefile:45-50` | **CLOSED.** **`make test-slow` could not run one slow test.** Confirmed exactly, plus a third fault the audit missed. Leg 1 selects zero because the root workspace declares **no `slow` mark at all** — both live in `runners/htr`. Leg 2 omitted the `cd` and died at collection (`ModuleNotFoundError: lineage_kit`, reproduced). And **`runners/dummy` was absent from the target entirely** (10 tests), though `make test` runs it. Fixed to mirror `make test`'s shape; both legs now collect (50 + 10). Pinned by `tests/unit/test_runner_suites_are_invoked.py`, which asserts every `runners/*/tests` is named by BOTH targets, with the `cd`, and that `test-slow` never deselects `slow` — mutation-tested against the exact prior recipe, which reds on both counts. Verified here: `runners/dummy` 10/10 and `runners/htr` 48/48 non-slow pass; the 2 slow marks need real weights/a GPU and were NOT run on this box. |
...
| M3 | `tests/e2e/tests/mfe.spec.ts` | **CLOSED, with one sub-claim refuted and one confirmed the hard way.** *Deleted zones* — CONFIRMED: it listed `/media` and `/train`, neither a zone; `/explorer` and `/models`, which exist, were uncovered. Fixed to all seven roots (trailing-slash form: `/compute` is a 308, `/compute/` a direct 200). *Invoked by nothing* — **REFUTED**: `Makefile:749` defines `make e2e` and runs it. It is absent from **CI**, which is the real gap. *Vacuous* — **CONFIRMED, and I first mis-refuted it.** A curl of `/train` returns an honest 404, so I reported the spec as merely stale; a curl carries no session and gets the raw ingress answer. Driven through a browser against the live deploy, `/train`, `/media` AND `/this-is-invented` all returned `status=200 url=…/dex/auth/local/login` — `page.goto` returns the LAST hop, so the OIDC bounce satisfies `toBe(200)` for any path, and the `appAsset404`/`pageErrors` assertions go with it (the Dex page loads no `/_app/` asset and throws no error). Fixed with a land-check that turns an auth bounce into a SKIP naming the reason, never a pass. Against the auth-on deploy it now reports 7 skipped instead of 7 green. Wiring a signed-in `storageState` is the follow-up that gives it teeth — not attempted, because the redirect targets the OIDC ISSUER url (`localhost:8080`), whose reachability is environment-specific. |
| M4 | `transport-contract.test.ts:241` | **CLOSED.** The cross-zone schema gate **never compared `createWarehouse`** — the case the file was written for. Confirmed and WORSE than filed: only **2 of 70** keys were compared across zones at all, and **62** resolved an empty-or-`/` upstream path. Two independent faults in one line: the window (`slice(0, 600)`) is shorter than a documented remote function, so both zones' real `catalogJSON('/v1/warehouses', …)` sat at ~char 800 and was never reached; and the search ran on RAW source, so home's comment `` `query.live`/`command` `` matched as a path of `/`. Home keyed `createWarehouse -> /`, lakehouse keyed `createWarehouse -> `, and both guards passed on a split half. Fixed by bounding the search to the DECLARATION (balanced parens, comments and string bodies stepped over) — a syntactic unit has no window to outgrow. `createWarehouse -> /v1/warehouses` now unifies home+lakehouse; empty paths fell 62 → 7. The guard now asserts the pair is COMPARED, not merely present, and carries a canary against a total path-resolution collapse; restoring the old `upstreamPath` reds it. |
...
| M6 | `test_worker_queue.py:107` | **CLOSED.** The poison-park test never provisioned the DLQ stream its own subject needs, and never asserted the park either. Reproduced on a bare broker (`NoStreamResponseError`). The underlying defect is PRODUCTION, not test-only: `park_poison` is awaited BEFORE `msg.ack()` and is unwrapped, so a missing DLQ stream means the unit is never acked and the drain dies — the one mechanism meant to stop a poison unit stalling a run becomes the stall. Fixed with `WorkQueue.ensure_dlq_stream()` (config mirrored from the chart — `limits` retention, `dlq.>`; diverging would make the chart's `assert_retention` job exit 1 for the whole estate), called on the drain path only; the fixture asks for it rather than inheriting it; and the test now asserts the parked unit is ON the DLQ, filtered by `run_id` because that stream is estate-shared. Mutation-tested: a no-op `park_poison` reds it. Green against a bare NATS (brought up as a Dagger service) AND the live cluster NATS where the stream already exists. Also corrected the `dlq_present` warning, which said parked units are "DROPPED" — they raise. |
