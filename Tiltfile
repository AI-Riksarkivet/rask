...
IGNORE = [
    ...
    '**/__pycache__',
    '**/*.pyc',
    # Build OUTPUT. A zone's build/ is regenerated by any local `bun run build`, and .vite/ by any dev
    # server; neither is an input to anything Tilt syncs.
    '**/build',
    '**/.vite',
    '**/storybook-static',
    # Test output. These are what actually broke it in practice: a Playwright run in ONE zone left
    # `test-results/.playwright-artifacts-*` behind and Tilt stopped live-updating EVERY zone, because
    # each zone's deps is the whole `frontend/` tree — so one zone's stray files are every zone's
    # problem. The failure surfaces only as "Found file(s) not matching any sync", naming a path that
    # has nothing to do with the zone that stopped reloading.
    '**/test-results',
    '**/playwright-report',
