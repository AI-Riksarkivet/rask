...
itself as the open R3 item: add pylance/lance-ray + the job scripts to the unified image (or ship them as a
...
| `.docker/ray-lance.dockerfile` pins | `lance-ray==0.4.2`, `pylance==8.0.0`, `pyarrow==19.0.1` | **WRONG** — cannot read the fleet's blob v2 (above) | FIXED — `0.5.0` / `9.0.0` / `24.0.0`, verified to resolve and pass the null-blob matrix with `ray[data]==2.56.1`; pinned equal to the workspace by `test_ray_job_images.py` |
...
| `runners/htr` pipeline + `/transcribe`, `/htrflow` Serve | Ray Data actors, Ray Serve | out of the lance-ray surface (no Lance IO); GPU sizing pinned in `pipeline.py` | none |
...
1. **Distributed compaction (`services/compaction`).** `lance_ray.compact_files` plans `CompactionTask`s on
   ...
   `service-kit` + `pylance`), so distributing means either adding `ray[default]` + `lance-ray` to a fleet
...
- `tests/unit/test_ray_job_images.py` (5 tests) — every settings submit-entrypoint is baked into the Ray
  image, every COPYed path exists, and the image's `pylance`/`pyarrow`/`lance-ray` pins equal the
