...
on it. Root lock 200 → 145 packages; fleet suite ~32 min → ~6 s. It has its own
  ...
  resolves" was mine, not the owner's, and it is struck. P7 re-cuts it as stage runners anyway, from `runners/`.
...
**Gates**: ray-kit tests, medallion unit tests, `e2e-ray-ci` as a **dedicated** job (stage runners/OpenBao secret-race landmine — never inside e2e-stack). **Live-proof**: `MEDALLION_RAY_ENABLED=true` stage runner cascade submits to the KubeRay head, `ray_stage_job` completes, OpenLineage RunEvent with DatasetVersion facet lands in AGE; resubmit proves idempotent reattach; rask's `/htrflow` Serve route still answers.
