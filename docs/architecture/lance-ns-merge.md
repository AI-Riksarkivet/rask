...
- **Net-new**: ~~greptimedb-standalone + perses subcharts~~ (**rask already ships greptimedb-standalone 0.4.5, vector 0.56.0 and perses 0.22.0** as of the rebase — dedupe them like the other shared subcharts; lance-ns retired Vector for an in-chart OTel Collector, so that one IS a real decision), otel-collector, openbao, dex, alerting (+ `rules_test.yml`), perses-dashboards, external-secrets, security-sa, ha.yaml, merged `values-prod.yaml`; `prod_render_check.sh` adapted to `rask-` names.
