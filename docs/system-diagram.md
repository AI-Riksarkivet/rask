...
### 3. Promote (medallion) — `medallion-producer` producer + Dapr movers, bronze → silver → gold  *(built & deployed: the `medallion-producer` producer fires the cascade and the event-driven movers promote each stage, emitting OpenLineage at every hop — see [`FLOW.md`](FLOW.md). The **distributed** lance-ray Ray Data job is the rask future; the in-process fake-Ray compute fills the same contract today.)*
