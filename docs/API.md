...
| ★ **Client-direct write (#2)** | `POST /management/v1/table/{id}/credentials`, `POST /management/v1/table/{id}/commit` | vend expiring creds → client writes fragments to the store → catalog folds only the metadata commit (zero data bytes transit the catalog) |
...
| **Blobs** | `GET /management/v1/table/{id}/blobs` | credential-less blob read (data-reader gated) |
...
| ★ **Project registry / tenants** | `GET,POST /v1/projects`, `GET,DELETE /v1/projects/{id}` | A tenant EXISTS when its registry record does — `POST` writes the record AND the creator's `project#admin` tuple in ONE operation, so existence and permission cannot drift (create/list/get are estate-observer gated; a fresh project legitimately has zero warehouses). `DELETE` is gated on the project's OWN `can_administer`, refuses **409 naming its warehouses** while any remain, and has **no `cascade` parameter at all** — retiring a tenant must never destroy buckets transitively. `?force=true` overrides the `protected` flag ONLY, never the authz gate |
| ★ **Warehouse admin / physical multi-tenancy (#3-A)** | `GET,POST /v1/warehouses`, `GET,DELETE /v1/warehouses/{id}`, `POST /v1/warehouses/{id}/{activate,deactivate,namespaces}` | project-admin (`can_create_warehouse`) provisions a bucket per warehouse + binds namespaces to it; the project must already exist (404 naming `POST /v1/projects` — there is no bootstrap exception). `DELETE` gates on the **project's** `can_administer` (destroying storage is a tenant-level act), refuses **409 naming bound namespaces**, and takes three SEPARATE opt-ins that never share a default: `?cascade=true` drops those namespaces, `?purge_bucket=true` is the only thing that removes bytes (and refuses if a same-project sibling or platform reserve claims the bucket), `?force=true` overrides `protected` alone |
