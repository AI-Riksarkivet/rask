...
**Counted 2026-09-09, from the rows below rather than asserted: 235 tracked, 128 open, 107 struck.**
...
| Q17-23 | No authorization-DECISION event: the estate events the grant, never the check | med | **MEASURED 2026-09-09 against Lakekeeper's source** (owner asked whether our abstractions match theirs). `crates/lakekeeper/src/service/events/types/authorization.rs` records an `Authorization` on EVERY decision — `allowed: Option<bool>` (`Some(true)` allowed, `Some(false)` denied, `None` only when an upstream error prevented a decision), plus `for_principal`, `action`, `entity` and `determined_by: Vec<DeterminingFactor>` naming what decided it; a batch check populates one entry per inner check, and each entry is self-contained *"on purpose — log consumers should not have to zip parallel arrays or fall back to top-level fields"*. rask's `ControlAction` Literal carries 40 values including `grant_added`/`grant_revoked` — the STATE CHANGE — and none for a decision: **a denial here is a 403 and a log line, subscribable by nothing and retained by nothing.** That is the audit half of zero trust, and it is a different gap from F2-10, which names the CORRELATION problem and assumes the records exist. For allows the assumption holds (the control events are the record); for denials there is nothing to correlate. Closing shape: an `access_denied` control event carrying action, object type + id, principal and the deciding relation, on the broadcast the ring buffer already consumes |
