...
| Q3-33 | `SKG-09` | med | Every lakehouse control-plane record is an unvalidated `dict[str, Any]` — **RE-MEASURED AT HEAD 2026-09-09 (§ Q12-1's re-run): STILL OPEN, and the seam is one function.** `service_kit/lakehouse/records.py:150` — `read_json(...) -> tuple[dict[str, Any], str] | None` — is what every project, warehouse, binding, protection and trash record is read through, so the whole control plane inherits its shape from one signature. That is the row's real weight and also why it is CHEAP to close: a single seam typed generically, not a pattern scattered across services |
