...
**Counted 2026-09-10, from the rows below rather than asserted: 269 tracked, 84 open, 185 struck.**
...
| ~~Q4-8~~ | ~~`test_outbox_e2e::test_reconcile_sweep_drains_a_staged_outbox_event`~~ | expects a bare run id, the store holds `<id>@COMPLETE` | A key-format change the suite never saw  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** tests/e2e-py/test_outbox_e2e.py:75 now computes `key = f"{run_id}@{event['eventType']}"`, matching the producer: packages/service-kit/src/service_kit/lakehouse/outbox.py:36 `_object_key(run_id, event_json)` returns run id + event type (docstring at :39-48 records why — build_run_event deliberately excludes eventType from the run… |
