...
**Counted 2026-09-09, from the rows below rather than asserted: 251 tracked, 133 open, 118 struck.**
...
| Q17-39 | The estate's own verification gate runs serially on a 64-core machine | low | **MEASURED 2026-09-09, and it is the goal's VERIFICATION clause paying for itself.** `open_goal.md` records the full suite at ~16 minutes and rules it NOT-per-commit for that reason, which shapes how every change in this register is verified. The cost is not inherent: the host has **64 cores with a load average of 15.76** (~48 idle) and 383 GB of 503 GB free, and the two heaviest consumers are somebody else's — `k3s-server` at 616% and a `yolo` + four `pt_data_worker` PyTorch processes 23.5 hours old. The suite is slow because it is **SERIAL**: one pytest process, and a large share of `tests/unit`'s ~2,000 tests shell out to `helm template`, which is process-spawn and YAML-render bound. Sixty-four cores do not help a serial loop. `pytest-xdist` is NOT a dependency (`import xdist` → ModuleNotFoundError) and `addopts` is only `--import-mode=importlib`. **NOT A FREE WIN, which is why this is a row and not a commit**: the chart-render tests shell out and several suites touch shared state (the registry CAS e2e markers, the `lance.audit` logger, `configure_audit`'s process-global level), so `-n auto` needs the parallel-unsafe ones grouped with `--dist loadgroup` or marked serial. Worth doing — a 12-minute gate that could be two changes how often it is honoured, and the goal already records two full-suite runs killed by teardown, buying nothing |
