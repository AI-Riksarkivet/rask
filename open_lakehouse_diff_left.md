...
| Q3-35 | `CAT-CORE-08` | low | Service functions return `dict[str, Any]` that endpoints splat into models — **RE-MEASURED AT HEAD 2026-09-09 (§ Q12-1's re-run): STILL OPEN, 6 sites in 2 files.** `endpoints/maintenance.py` ×3 (`GcPreview(**result)`, `GcRunResult(**result)`, `CompactResult(**result)`) and `endpoints/warehouses.py` ×3 (`WarehouseResponse(**record)` among them). The cost is where it always was: a splat makes the SERVICE function's shape the contract while the MODEL claims to be, so a key the service stops returning is a runtime TypeError at the door rather than a type error at the seam |
