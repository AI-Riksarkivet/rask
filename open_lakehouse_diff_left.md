...
**Counted 2026-09-09, from the rows below rather than asserted: 247 tracked, 133 open, 114 struck.**
...
| Q17-35 | The maintenance work queue's retry and dead-letter story is promised in code and unconfigured in the chart | high | **FOUND 2026-09-09, and the estate's own gate cannot see it.** `chart/templates/dapr-resiliency.yaml` lists no `maintenance-work-pubsub`, `maintenance-index-pubsub` or `lineage-pubsub-maintenance` in `targets.components`. Per `.claude/skills/rask-dapr`'s own trap list, a missing row means **no sidecar retry AND no dead-letter**. On the executor deployment this register already proved in-cluster (the planner published 21 units, two workers consumed 7 and 8), a unit hitting a transient S3 or commit failure returns RETRY and is dead-lettered on the FIRST attempt onto a topic nothing reads — no metric, no alert. The component's own `maxDeliver: 3` / `backOff: 720s,720s` never engage, because the sidecar acks the original. § H4 records the lease and per-record `attempts` clauses for the TRASH record, not this. Fix both halves together: add the three components, and render `test_every_DURABLE_pubsub_component_has_a_sidecar_retry_target` with the maintenance topics so the gate that missed them stops missing them |
