...
| Q3-37 | `SKG-14` | med | The audited scope sits under a blanket 21-rule ruff exemption (5 lines still in `pyproject.toml`) — **RE-MEASURED AT HEAD 2026-09-09 (§ Q12-1's re-run): still 5 globs × 21 rules**, covering `services/{catalog,lineage,medallion,maintenance,viewer}`. **AND ITS OWN JUSTIFICATION HAD ROTTED:** the comment read *"NOT a permanent exemption — tracked in docs/OPEN-WORK.md so it is revisited, not forgotten"*, and **that file does not exist** — the sentence promising review pointed at nothing, which is exactly how a temporary waiver becomes permanent without anyone deciding it should. Repointed at this row, which does exist and is counted. **A WIDER MEASUREMENT came with it:** 15 `docs/*.md` paths named across the estate resolve to no file — though several (`docs/generic-tables.md`, `docs/lance-rest-service.md`) are LAKEKEEPER'S and GRAVITINO'S own docs cited in the build-vs-buy audit and are correct as written, so a gate on this shape must tell our docs from someone else's or it fires on accurate prose |
