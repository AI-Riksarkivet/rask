...
**Counted 2026-09-07, from the rows below rather than asserted: 209 tracked, 173 open, 36 struck.**
...
| Q17-7 | Kill the one shared service bearer (§F2-3) | high | **THE WIDEST HOLDER IS DONE 2026-09-07**, and the control is further along than the sweep says. `dapr_auth`'s warning — "any holder can pick the highest-privileged name" — is ALREADY FALSE for the five privileged cascade subjects: the door refuses a privileged name presented with the shared token. Measured live, what still shared it was `service-ingest`, `service-maintenance`, `notifications` and `service-web`. The frontend is now privileged and mounts its own credential off infra-credentials — SEVEN zone pods stop holding the estate's shared bearer, which the chart's own note calls the whole attack ("Reading one web pod's environment"). It needed no service change: the BFF already reads `LINEAGE_SERVICE_TOKEN` from env, so which secret fills it IS the client half. **What remains is `service-ingest`, `service-maintenance` and `notifications`**, and each needs its client half FIRST — all three have 0 `dedicated_token` references, and naming a subject privileged before it can present its own credential 401s it outright (measured 2026-08-26, when the server half alone stopped the whole cascade) |
| Q17-8 | Anonymous browser reads are laundered into a service identity (§F2-4) | high | CONFIRMED at the source and MEASURED, and the measurement raises it. `bff.ts:191-196` and `runs-feed.ts:224-231` agree: a signed-in user's bearer is used when there is one, and with NO session a READ falls back to `dapr-api-token` + `x-lance-service-identity: service-web`. What that subject actually holds, read off the live store 2026-09-07: **7 reader grants across 6 namespaces spanning 2 TENANTS, plus a `writer` grant on `table:bronze$events`**. So an anonymous browser reads across tenants as an allowlisted service, and what anonymous may see is not expressible or auditable as a grant — it is whatever `service-web` happens to hold. The BFF's `isRead` gate keeps the write grant out of reach through that path, but the grant is on the identity, not on the path. Used by the live run feeds in SIX zones (studio, compute, home, models, annotator, lakehouse) plus `ingest.remote.ts`. THE FIX IS AN IDENTITY SPLIT — anonymous gets its own subject so its reach is its own grant set — but WHAT that set should contain is a product decision the code does not answer: the docstring says the fallback exists so "a governed stack serves reads without a per-user login", and says nothing about which reads |
