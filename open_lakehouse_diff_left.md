...
**Counted 2026-09-10, from the rows below rather than asserted: 271 tracked, 87 open, 184 struck.**
That is WORK ONLY — 69 lettered `###` rows (28 struck) and 190 Q-section table rows (111 struck) —
...
**THE LANCE LIBRARY MOVED UNDER THESE ROWS — pylance 10.0.0 -> 11.0.0, locked 2026-09-09 (owner
instruction).** Every Q13 row's `LANCE CLAIM` was measured against 10.0.0, so those verdicts are
PROVENANCE, not current evidence: re-measure before working one. Three were re-checked on 11.0.0 the
day of the bump and hold unchanged — a JSON index still refuses a non-JSON column (Q13-13),
`describe_indices()` still names index segments (Q13-14), and `get_transactions()` still panics in Rust
on a shallow clone, so the `BaseException` guard in the orphan scan is still load-bearing. The rest are
unverified against 11.0.0. The bump did NOT lift the `lance-namespace<0.12` ceiling: 11.0.0 declares the
same `lance-namespace<0.9,>=0.8.5` cap 10.0.0 did (read off PyPI), so A10 is a question about the Rust
binding's `on` handling, not about this pin. What it DID bring is a correctness win for the
maintenance plane, measured both ways rather than read off the changelog: lance #8206 stops reusing
fragment ids across an overwrite, and an isolated env confirms it — on 10.0.0 an overwrite reuses
fragment id `0`, on 11.0.0 it moves to `1`. The orphan scan names a deletion file with
`deletion.path(fragment.fragment_id)`, so id reuse is exactly the shape where a stale deletion file and
a live one collide in name-space. Stable row ids continue across the overwrite on both (`4,5,6,7`), so
`source_rowid` provenance was never the exposed half. lance #7881 (a large inline transaction spills out
of the manifest) was probed against the scan for the same reason and is a non-event: a 160-fragment
single commit still lands one ordinary `_transactions/*.txn`, which the referenced set already covers —
0 orphans. **AND THE BUMP FOUND A LATENT DEFECT OF OURS, which is the best argument for
doing it:** `work_items.DatasetPlan.scan_batch_size` was `int = 0` — a SECOND spelling of "unset" on the
one model that crosses the broker, while the catalog policy field it carries spells absence `None`. The
guard in `_compact_files` is `is not None`, so a default plan called `compact_files(batch_size=0)`.
pylance 11 refuses that outright; pylance 10 ACCEPTED it on a plain dataset and failed a stable-row-id
one — every governed tier — with "Got too many segments for the provided chunk lengths", a message about
neither the batch size nor the caller. Now `int | None = Field(default=None, ge=1, le=8192)`, the same
shape the policy schema uses, so the bad value is unconstructible rather than diagnosable.

**Beside the work, and never counted with it: 5 D rows, 11 R rows, 12 M rows.** D is a decision the
owner already made (recorded so it is not re-litigated); R is a recommendation awaiting an owner
ACKNOWLEDGEMENT, not an engineer; M is a question that must be answered before a decision can be made
at all. None of them is a defect, and summing them into the backlog was overstating the work by 28.
Those three are re-derived by the same gate.

That splits into the lettered sections and the Q sections — § Q2 carried from
`open_estate-verification.md`, § Q3 from `open_python-audit.md`, § Q4 recorded from the first e2e run
against the deployed estate. Re-derive the counts when you change them; the previous header claimed a
freshness date two days older than rows struck beneath it, and a header nobody re-counts is how a
...
**DEPLOYED AND OBSERVED on `main-53412a30`.** Driven inside the running catalog pod against a LOCAL scratch dataset in a temp dir — so the observation costs the estate no residue, which is the trap § Q8-15 and § Q15-3 are both about. Both paths answer `version=2 num_inserted_rows=3` for the same payload; before the fix MAIN answered `None`/`None`. Catalog, lineage, maintenance and ingest all rolled, 0 tracebacks in the window.
...
**THE `/producers` CLAUSE IS CLOSED 2026-09-10.** Run retention (30 days) bounds the GRAPH and not one dataset's history: a cascade tier rewritten every 120 s accrues ~21,600 WROTE edges inside that window, and the endpoint returned all of them. Now `producers_page(limit)` in the estate's own bounded-query form — an INT LITERAL validated before interpolation, because AGE does not bind `$param` reliably outside a MATCH, the hazard `list_runs_page` already records — with `limit: Query(ge=1, le=1000) = 200` on the door. The bound composes with the existing newest-first ORDER BY, so a caller loses the tail and never the current answer. Pinned, including the `isinstance(True, int)` trap that would interpolate `LIMIT True`. The retention and index clauses were already closed; UPSTREAM/DOWNSTREAM traversal depth remains.
...
**Score.** 19 controls: rask HAVE 6, STRONGER 3, PARTIAL 8, MISSING 1 (per-workload storage identities). **THE PARENTHETICAL WAS RE-MEASURED ON THE RUNNING PODS 2026-09-10 and named two planes that are no longer root**: `MAINTENANCE_S3_ACCESS_KEY_ID=rask-maintenance` and `MEDALLION_S3_ACCESS_KEY_ID=rask-medallion` on the producer and all three stage runners, `rask-lineage` likewise. `rask-ingest` carries no S3 key at all (`RASK_INGEST_SECRETS_FROM_DAPR=true`), and `rask-viewer` reads `rustfsadmin` in env while actually resolving `access_key=rask-viewer` per store from the Dapr secret store — a fallback no registered store takes, and now rendered as the scoped id so it stops misleading an env audit. **ONE identity is left, `rask-catalog`**, and it is a design question rather than an unfinished copy of the pattern: it vends for every warehouse a tenant mints at RUNTIME, so a role narrowed to today's buckets cannot vend tomorrow's. Both sides lack image signing and SBOM. Lakekeeper is stronger on service identity (one Kubernetes SA per service, no shared secret), location containment and audit request-ids; rask is stronger on token validation, vending posture, mTLS on the Dapr plane, pod hardening, network policy and fail-closed behaviour.
...
**CORRECTED 2026-09-10 against the running estate.** Row 5: `lockRootCreate` is DERIVED (`services.yaml:327`), not shipped false. Row 6: the CHART default is `sts` (`values.yaml:942`) while the CODE default stays `mode_b` (`config.py:266`) — the row conflated the two, and "nothing is vended by default" is false of a deployed estate. Row 7: MISSING -> PARTIAL, scoped to `rask-catalog` alone. No test proves a markdown table; the proof is the four greps, each re-run.
...
**CLAIM 2 CORRECTED 2026-09-10.** "Nothing is vended by default" was rask's advantage over Lakekeeper and is no longer true: the chart ships `mode: sts` (`values.yaml:942`). What remains genuinely stronger is the TTL — **900s against Lakekeeper's 3600** — and the BOOT REFUSAL: `sts`/`web_identity` will not start without an explicit STS endpoint (`config.py:382-384`), so a caller's token can never be sent to public AWS STS by omission.
...
**STRUCK 2026-09-10 by re-measurement, adversarially verified:** `ls packages/` returns lineage-kit, ray-cluster-env, ray-kit, service-kit, storage, validate — no `ratch`; `ls packages/ratch` → No such file or directory. Every remaining `\bratch\b` hit in the tree is prose or the guard itself (runners/asr/errors.py:3 provenance note, packages/ray-cluster-env/pyproject.toml:9-11 rationale).
...
**THE SAME QUESTION EXISTS ONE RUNG UP, AND IS THE LARGER HALF — measured 2026-09-09 against
Lakekeeper's own `authz/openfga/v3.4/components/`.** Their `project.fga` carries FOUR built-in roles
where rask carries two:

    project_admin   [user, role#assignee] or operator from server
    security_admin  [user, role#assignee] or project_admin   -- grants, NOT data
    data_admin      [user, role#assignee] or project_admin   -- data, NOT grants
    role_creator    [user, role#assignee] or security_admin

plus a `server` type above the project splitting `admin` (human) from `operator` (*"designed to be used
by machines that provision resources inside Lakekeeper, i.e. a `KubernetesOperator`"*). rask's `project`
has `admin` and `member`, and `can_administer`, `can_create_warehouse`, `can_grant_admin` and
`can_read_assignments` all resolve to the single `admin` rung. **So one principal holds the data power
and the granting power, and there is no machine identity distinct from a human one.** That is the
separation-of-duties leg of zero trust, and it is the same owner decision as the `can_set_protection`
question above, one level higher: the four-eyes shape is unreachable at the tenant for the same reason
it is unreachable at the table. Note what is NOT missing, so the comparison stays honest — rask has
their whole grant-delegation lattice (`manage_grants`, `pass_grants`, `managed_access` /
`managed_access_inheritance`, `can_grant_X: manage_grants or (X and pass_grants)`) at warehouse,
namespace and table, and goes past it with `user with non_expired_grant`: TIME-BOUNDED grants as an
OpenFGA condition, which their OpenFGA model has no form of at all (their answer for time/tag
conditions is Cedar, in the paid Lakekeeper Plus tier). The gap is the tenant's role split, not the
lattice.
...
**THE S3FileSystem CLAUSE IS CLOSED 2026-09-10, and the second constructor was not merely a duplicate — it FAILED OPEN.** `lancekit/store.py::_s3fs_for` unpacked `credential_of`'s triple and dropped the SESSION TOKEN into `_`, then read the bare `endpoint` spelling. `objectfs.s3_filesystem`'s own docstring states the stake: pyarrow falls back to the default credential chain for anything it was not given, so a vended, table-scoped credential arriving at this seam signed with the POD's role — BROADER than the catalog scoped, not narrower, and nothing refuses it. Every dataset resolution goes through it (`list_lance_stems`, `exists`), so it is hot as well as wrong. It now delegates to the shared constructor, RED-verified by a third case added to `test_a_vended_credential_survives_the_storage_seam.py`. The emit-kernel clause is NOT closed by this — that is Q3-13's, and still real.
...
| --- | --- | --- | --- |
| ~~J1~~ | Read audit log (subject, table, version, columns, when) | **DOORS DONE 2026-09-08** — `query`, `count_rows`, `explain_plan` and `blobs` call `audit_read`, which fixes the action name (`read_data`) and the field set so the log is filterable. NOT a middleware: `version` and `columns` are what make a read record worth keeping and only the route knows them. **OBSERVED END TO END 2026-09-08** on release 113: ten driven `count_rows` against `bronze$pages` produced 22 `read_data` rows in GreptimeDB within 20 minutes, one reading `action=read_data outcome=success resource=bronze$pages subject=service-ingest` — door -> helper -> OTLP -> store, answerable by SQL. THE POD LOG IS NOT THE SINK and looking there first nearly read as a failure: the formatter renders the message `audit` and drops every field, which is the exact shape `otel.py` records as having "emptied the compliance audit trail" once — the OTLP `LoggerProvider` it installs is what carries the fields. Residue is retention and an index on dataset, which is the Collector's half | `audit()` on every data-read door via a service-kit middleware; retention; an index on dataset |
...
| ~~J4~~ | Change feed for BYO consumers | **DOOR DONE 2026-09-08** — `POST /v1/table/{id}/changes` answers rows changed in `(begin, end]`, Arrow FILE-framed, gated and audited as a READ (§ J1). Predicates are verbatim from `lance_docs/file_format.md:4270-4300` and live in ONE module: the `updated` window's first clause (`_row_created_at_version <= begin`) is what stops an insert being reported twice, and a consumer applying both streams without it double-counts. Half of this already ran privately — `ray_stage_job._delta_filter` builds the INSERTED predicate for the cascade — so the mechanism was proven on this data and what was missing was a door and the UPDATED half. NOT reusing the query door: `QueryTableRequest` is a VECTOR model (`k`, `vector` required), so it would have meant inventing a vector to ask a question unrelated to similarity. **OBSERVED LIVE 2026-09-09 at `main-659dc095`, and driving it found THREE defects the unit tests could not — all three live in what the door RETURNS or how it is GATED, not in the predicate the tests assert on.** (1) **The feed projected no version.** `file_format.md:4283` says the documented query returns the changed rows *including* `_row_created_at_version`, `_row_last_updated_at_version` and `_rowid`; Lance does not project pseudo-columns unless named, and the door passed the caller's `columns` through verbatim. Measured on `bronze$pages`: three genuinely changed rows, schema `['id','payload','source_uri','stage']` — a feed readable exactly ONCE, because the consumer has no version to send next and re-sends its old one forever. (2) **A malformed window was a 500.** `change_filter` refuses an inverted window and a negative begin, but raised `ValueError` and nothing mapped it: the live door answered `InternalError 18`, which tells a consumer the catalog is broken and to retry a request that will fail identically forever. Now `InvalidInputError` → **400** carrying the reason. (3) **THE GATE WAS THE WRITER RUNG.** `fga_deps._action_relation` defaults an unmapped route to `can_write_data`, and `changes` was unmapped — so the audit trail recorded `can_write_data ALLOW` on `table:bronze$pages` one line above the `read_data` record for the same call, and every reader who was not also a writer was refused, which for a change feed is its entire audience. THE SAME FALL-THROUGH CAUGHT `tasks` THE SAME WAY, and the comment beside it already said so: *"Unmapped it would fall to WRITER, which is how the live audit found it."* A read door's gate is not implied by calling it a read. All three now pinned by tests; the third by `test_every_DATA_READ_door_is_gated_as_a_READ_not_by_the_writer_fallthrough`, which resolves every member of `_DATA_READ_ACTIONS` through the real classifier rather than asserting set membership. **The consumer loop closes on the estate:** poll 1 answers 3 rows at version 15, polls 2 and 3 answer 0 — a consumer that advances on the returned version stops replaying. The read is audited with its kind: `audit.action=read_data · audit.change_kind=inserted · audit.resource=bronze$pages · audit.subject=service-ingest` in GreptimeDB. **AND THE SURVEY THE THIRD DEFECT DEMANDED FOUND A FOURTH DOOR, 2026-09-09.** Two mis-tiered read doors found the same way is a pattern, not a coincidence, so every guarded route in `docs/catalog-openapi.json` (84 of them) was resolved through the REAL classifier and filtered for read-shaped routes landing above the reader rung. Three surfaced: `protection` on both types is owner-tier deliberately and says so (arming a safety is a statement about the object's fate), and **`GET /v1/table/{id}/history` was the writer rung** — while its own docstring said *"Reader-tier: `can_get_metadata` on the table, the same rung as describe/list-versions"* and the endpoint checked exactly that itself. The self-check was UNREACHABLE for the callers it was written for: `authorize` is a router-wide dependency, so the wrong rung is demanded first. Driven on the estate, one `GET /history` logged `can_write_data ALLOW` and then `can_get_metadata ALLOW`, in that order — a reader is refused at the first and never reaches the second. THE LESSON GENERALISES BEYOND THESE FOUR: a default that fails CLOSED is still a defect when it silently answers a question nobody asked, and a rung asserted in a docstring — or checked a second time inside the endpoint — is not evidence of the rung the router demands. **Residue:** the control-lane EVENT, so a consumer can be told rather than poll. **Note for § B2:** this is a deliberate 26th non-spec route group, placed with the data doors because a change feed is a data-plane read; it moves with them, not with the admin ones |
| ~~J5~~ | Encryption at rest options per warehouse | **DONE 2026-09-08** — `aws_server_side_encryption` / `aws_sse_kms_key_id` / `aws_sse_bucket_key_enabled` travel IN the vended storage options, which is the only channel that reaches a direct writer. Names and contract verified against `lance_docs/guide.md:2417-2419`, not guessed, and an invalid pairing is REFUSED at the builder: object_store drops an option it does not recognise, so a wrong value would write plaintext under a config claiming encryption — the one failure with no later signal. Empty by default = today's behaviour | `aws_server_side_encryption` / `aws_sse_kms_key_id` on the warehouse record and in vended options |
...
| ~~Q2-2~~ | Six owner decisions ruled but NOT implemented | CLOSED 2026-09-08 | **ALL SIX ARE BUILT; "none is built" was stale.** The row pointed at `open_python-audit.md`, which is deleted, so the six ids existed nowhere else and the row could not be worked as written. Recovered from git (`058da189^`): `CAT-CORE-04`, `ingest-flow-11`, `PS-07`, `catalog-api-17`, `MED-011` and `X1` are each marked **FIXED** there. A file's own claim is not proof, so two were re-checked against today's code: the catalog lifespan does NOT mutate the cached `Settings` (the first grep matched `==` in a comparison, not an assignment — a false positive of the check, not a finding), and the medallion's `fga.write_tuples` is now a SANCTIONED writer pinned by `test_only_the_sanctioned_writers_seed_a_hierarchy_edge`, which is precisely what MED-011 asked for rather than a surviving second path. **A row whose evidence lives only in a deleted file is unworkable by construction — the recovery, not the verdict, was the work here.** |
...
| ~~Q3-12~~ | ~~`DUP-14`~~ | ~~med~~ | The same hand-rolled HTTP backoff loop in `packages/storage` and `services/ingest` — **RE-MEASURED AT HEAD 2026-09-09 (§ Q12-1's re-run): the DUPLICATION is gone, one half remains.** `packages/storage` holds no retry loop — its only mention of retry is `sts.py`'s comment that every client gets "an explicit timeout and retry policy rather than botocore's defaults", which is CONFIGURING the SDK's retries rather than hand-rolling a loop, i.e. the opposite of the finding. The ingest half stands: `fetch.py` and `worker.py` still sleep in their own loops. So the row is no longer a DUP — it is one service's retry policy, and should be re-stated as that rather than carried as a duplication nobody can find the second copy of  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** The DUPLICATION the finding names does not exist at HEAD — there is no second copy to find. |
| Q3-13 | `DUP-10` | med | Two OpenLineage kernels and four `RunEvent` builders — **RE-MEASURED AT HEAD 2026-09-09 (§ Q12-1's re-run): the counts are 3 BUILDERS and 4 KERNELS**, so the row over-counts one half and under-counts the other. Builders (by `ast`, production code only): `lineage_kit/runs.py`, `medallion/schemas/events.py`, `lineage/seed.py`. Kernels: `lineage_kit/{emitter,runs}.py`, `service_kit/lancekit/lineage_emit.py`, `catalog/core/lineage_emit.py`, `maintenance/core/lineage_emit.py`. **AND MEASURING IT FOUND A LIVE PROVENANCE DEFECT the row does not mention:** the five `producer` constants used THREE spellings of the repository — three said `Borg93/lance-ns` (the RETIRED repo), one said `Borg93/rask` (wrong org), one was right — and the graph carries the retired name on real runs (738 medallion runs under `.../lance-ns/tree/main/medallion`). `producer` is spec-required and is the only pointer from a run event back to the code, so a wrong one fails nothing and 404s months later for whoever is trying to understand a run. All five repointed at `AI-Riksarkivet/rask` at a path that exists. **OBSERVED IN THE RUNNING PODS at `main-7ed9fbad`:** maintenance and catalog both report the corrected URI from inside the container, so what the estate STAMPS changed rather than only what the tree says. Historical events keep the old string and correctly so — `producer` records where the code was when it emitted, so the two disagreeing across a repo move is provenance working. **THE GATE THAT EXISTED FOR THIS CARRIED THE DEFECT ITSELF:** `test_lineage_producer_uri_is_real.py` was written for exactly this, checked the PATH against the real tree, hardcoded the ORG as `Borg93/rask` — and looked at ONE of the five. Replaced by `tests/unit/test_every_lineage_producer_uri_resolves.py`, which finds every constant by parse and reads the repo from `git remote` rather than a literal, because a literal is what went stale twice |
| ~~Q3-14~~ | `DUP-19` | low | Three hand-rolled `storage_options` builders bypass `lance_storage_options` — **REFUTED AT HEAD 2026-09-09 (§ Q12-1's re-run). There is exactly ONE, and it IS `lance_storage_options`.** Parsed every production module for a dict literal carrying `aws_*` storage-option keys: a single hit, `objectfs.py:69`, inside the canonical builder itself. **THE THREE FILES A KEYWORD SCAN FLAGS ARE A DIFFERENT SEAM:** `packages/storage/client.py` builds a BOTO3 CLIENT (its own docstring: *"A generic boto3 S3 client for any S3-compatible backend"*), `ingest/staging.py::_client_for_options` builds a boto3 client FROM vended options — a CONSUMER, not a builder — and `service-kit/media/config.py` calls `lance_storage_options`, i.e. uses the sanctioned path. Measured by PARSE rather than grep, because `aws_access_key_id` appears in all four and cannot tell a builder from a caller. The estate's own rule applied to its own backlog: a keyword match is not a classification |
...
| Q3-28 | `MED-014` | low | Both app entrypoints read settings and configure logging at import time — **RE-MEASURED AT HEAD 2026-09-09 (§ Q12-1's re-run): HALF TRUE, and the half that is true is the one that matters.** SETTINGS at import: YES, both — `producer.py:170` and `:202` call `get_settings()` at module level (building the app, mounting the lag cron), and `stage_runner.py:46` binds `_settings = get_settings()` outright. LOGGING at import: NO — parsed the module bodies with `ast` rather than grepping, and neither entrypoint calls `basicConfig`/`setup_logging` at module level; both only take a `getLogger(__name__)` handle, which configures nothing. The lifespan-time reads the row might be blamed for are correct and stay: `producer.py:64-71` reads settings inside `lifespan`, which is where a fail-closed startup check belongs |
| ~~Q3-29~~ | `ING-18` | low | Query-parameter clamping done by hand instead of declared — **STRUCK 2026-09-09, FIXED AND SELF-DOCUMENTING AT HEAD (§ Q12-1's re-run).** `ingest/api.py:451-453` now declares `limit: Annotated[int, Query(ge=1, le=200)] = 50` and carries the reason on the line: *"DECLARED, not clamped. `store.recent(min(max(limit, 0), 200))` below applied the real ceiling three frames away, so the schema advertised an unbounded integer."* That last clause is the finding's real cost — a hand clamp bounds the BEHAVIOUR and lies in the CONTRACT, so a generated client and the OpenAPI both promise something the service refuses. No hand-clamping remains in the module |
| ~~Q3-30~~ | `ingest-flow-16` | low | Generator workflows annotated as returning their final value — **STRUCK 2026-09-09, FIXED AT HEAD (§ Q12-1's re-run).** Parsed every `*workflow*.py` under `services/` with `ast`, detecting a generator by the presence of a `yield` in its body rather than by its name: **12 generator workflows, ALL annotated `Generator[...]`, ZERO annotated as their final value.** `ingest_run` and `chunk_run` both read `-> Generator[Any, Any, dict[str, Any]]`. Measured by parse rather than grep because the wrong annotation and the right one differ only in the return type — a text search for `dict[str, Any]` matches both |
...
| ~~Q3-33~~ | ~~`SKG-09`~~ | ~~med~~ | Every lakehouse control-plane record is an unvalidated `dict[str, Any]` — **RE-MEASURED AT HEAD 2026-09-09 (§ Q12-1's re-run), and the row as written is WRONG in a way worth keeping.** Typed models and a validator already exist — `catalog/services/control_records.py` defines `WarehouseRecord`, `BindingRecord`, `ProjectRecord` and `validated(...)`. So it is not true that every record is unvalidated. **WHAT IS TRUE IS AN ASYMMETRY, and it runs the dangerous way.** `validated()` is called at exactly three sites and ALL THREE ARE LIST PATHS (`list_warehouses`, `read_bindings`, `list_projects`); the FIVE singular readers validate nothing — `get_warehouse`, `get_project`, `bind_namespace`, `warehouse_for_namespace`, `binding_for_namespace`. Those are the ones that feed the DESTRUCTIVE doors: warehouse delete, project delete, the new unbind door, and the routing resolver. **AND THE MODULE ALREADY STATES THE RULE IT BREAKS:** `read_bindings`' own docstring says *"the DELETE door must fail closed on one. A binding it cannot read is a namespace it cannot see"* — the plural reader honours that and the singular ones cannot, because `validated()` answers `None`, which a caller reads as ABSENT. For `get_warehouse` that is a 404 and fail-closed; for `warehouse_for_namespace` it is "unbound", which routes the namespace at the DEFAULT root — a malformed binding silently sending writes to the wrong bucket. **NOT a `read_json` signature problem:** that function is a transport primitive reading arbitrary JSON and `dict[str, Any]` is correct there. The fix belongs at the five readers, and it is not one edit — "malformed" must mean REFUSE for the resolver pair and may mean absent for the rest. **CLOSED 2026-09-09 by applying the rule the module already stated.** `validated_or_refuse` is a strict SIBLING of `validated` rather than a stricter version of it: the tolerant form stays, because a listing that raises on one bad object turns a single tenant's corruption into an estate-wide outage — which is exactly what its docstring promises against. The four singular readers now validate what they return and raise `ServiceUnavailableError` naming the path, the same answer `read_bindings`' delete-path caller already gives for the same condition. Gated per-reader by `test_a_singular_record_read_refuses_what_it_cannot_parse.py`, verified RED against all four, because this failed as a CLASS and a single spot-check would pass while three stayed silent. **OBSERVED LIVE at `main-28ad3020`, the half that makes it hardening rather than starvation:** a VALID record still resolves through every strict reader — `GET /v1/warehouses/lakehouse-wh` 200, the tolerant bindings listing 200, and a routed `count_rows` on `bronze$pages` 200 through the resolver that now validates its binding | **CLOSED 2026-09-09 — FOUR of the five singular readers were ALREADY validated, and the fifth is now, so the asymmetry this row correctly identified is gone.** Re-measured rather than worked from the row: `validated_or_refuse` exists (the strict sibling of `validated`) and is called at `warehouses.py:203` (`get_warehouse`), `:272` (`warehouse_for_namespace`, the routing resolver), `:285` (`binding_for_namespace`) and `projects.py:106` (`get_project`) — the four that feed the destructive doors. **The fifth, `bind_namespace`, read the record RAW**, and its defect was narrower than "unvalidated": its two-field comparison answers *"is this my own earlier write, retried?"*, and on an unparseable record the comparison simply fails, so the door refused the bind — fail-CLOSED, which is right — while calling it `NamespaceAlreadyExistsError`, *"already bound to another warehouse"*. That is a true refusal with a false reason: an operator sent to find the competing tenant is looking for something that does not exist. Now `validated_or_refuse`, so an unreadable record answers the condition it actually is. RED-verified both ways. **One trap worth recording:** `control_records` raises `lance_namespace.ServiceUnavailableError`, and `service_kit.exceptions` ships a same-named different class — a test asserting the wrong one passes nothing |
| ~~Q3-34~~ | ~~`F-LIN-07`~~ | ~~med~~ | Domain values cross models→repository as untyped dicts and positional tuples — **RE-MEASURED AT HEAD 2026-09-09 (§ Q12-1's re-run): the DICT half stands, the POSITIONAL-TUPLE half has largely gone.** `repository.py` carries 5 `dict[str, Any]` and `models.py` 7; but the repository returns no bare `tuple[...]` at all, and `models.py` has ONE — `progress() -> tuple[int, int] | None`, which is a pair with an obvious reading rather than a smuggled record. So the row should be re-stated as the dict half only; carrying "positional tuples" sends a reader hunting for a shape that is no longer there. **AND MOST OF THE DICT HALF IS CORRECT, which the row does not allow for.** `models.py`'s `facets: dict[str, Any]` (three sites) is the OpenLineage FACETS map, which is OPEN by specification — that map IS the format's extension mechanism, and the estate's own `custom_facet` builds arbitrary ones — so a closed type there would invent a contract the format does not have. Same for the raw stored `event` payload. **THE ONE THAT WAS A REAL SEAM IS FIXED:** `feed_columns()` returned `dict[str, Any]` and its caller SPLATS it — `_insert_feed_row(conn, **feed_columns(event))` — so a key renamed on either side was a TypeError at ingest time, on the write path whose whole purpose is that provenance survives. Now a `FeedColumns` TypedDict; verified by renaming a key and watching `ty` report it where it previously reported nothing. What remains of this row is the handful of genuinely-open maps, which is a much smaller and different claim  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** The actionable seam is closed and the residue is correct by specification. |
| ~~Q3-35~~ | `CAT-CORE-08` | low | Service functions return `dict[str, Any]` that endpoints splat into models — **STRUCK 2026-09-09: no splatted service function returns `dict[str, Any]` any more.** The maintenance trio returns `GcPreviewData` / `GcRunData` / `CompactData`; the warehouse pair returns `dict[str, str] | None` AND is validated against `WarehouseRecord` since § Q3-33, so the splat receives a shape checked one frame earlier. `dict[str, str]` is still a mapping rather than a TypedDict, which is a weaker statement than the row made and worth naming as the honest remainder — **RE-MEASURED AT HEAD 2026-09-09 (§ Q12-1's re-run): STILL OPEN, 6 sites in 2 files.** `endpoints/maintenance.py` ×3 (`GcPreview(**result)`, `GcRunResult(**result)`, `CompactResult(**result)`) and `endpoints/warehouses.py` ×3 (`WarehouseResponse(**record)` among them). **AND THE ROW'S IMPLIED FIX IS WRONG, which is worth more than the count.** "Have the service return the model" would break a deliberate layering: NO service module imports `catalog.schemas` — services return plain data and endpoints own the wire shape. Nor is `Model.model_validate(result)` an improvement; pydantic v2 IGNORES extra keys by default while `Model(**result)` raises on them, so the splat is the STRICTER of the two. **What the finding actually is: a TYPE-CHECKING gap, not a runtime one.** Pydantic validates types and required fields on `__init__` either way, so a drifted key is caught at runtime today; what is missing is `ty` being able to see it at the seam. The fix that would buy something is a `TypedDict` return on the service functions — cheap, layering-preserving, and it moves the failure from the door to the type checker. **DONE for the maintenance trio 2026-09-09**: `GcPreviewData`, `GcRunData`, `CompactData` declared beside the functions that return them, so the shape lives where the service is and no service imports the wire schemas. **Verified rather than asserted:** renaming one key produces `ty` errors where it previously produced none. **Residue: the three `WarehouseResponse(**record)` sites**, which are a different case — since § Q3-33 those records are `validated_or_refuse`d against `WarehouseRecord`, so the splat now receives a shape that was checked one frame earlier |
| ~~Q3-36~~ | ~~`MED-013`~~ | ~~low~~ | Two handler seams typed `Any` with an ANN401 suppression — **RE-MEASURED AT HEAD 2026-09-09 (§ Q12-1's re-run): SIXTEEN, not two** — and my own first count of four was also wrong, taken from too narrow a grep. Each carries a reason on the line, which is the estate's rule for a suppression, so most are the sanctioned form rather than a breach: a `WriteResult` typed here would pull the medallion's model into the port's shape; a `LineageDoc` is imported lazily to keep the module's import cheap; `saga` is Dapr's own workflow callable the port deliberately does not describe. **ONE WAS NOT SANCTIONED AND IS FIXED:** `api/rerun.py:116` annotated `fga_client: Any` with the reason *"OpenFgaClient"* — naming, in a suppression, the very type it could have written. `openfga-sdk` is a declared dependency and `api/dependencies.py` already imports `OpenFgaClient`, so there was nothing to avoid. Now typed, suppression gone, **16 -> 15**. The remainder are judgement calls with their reasons stated, which is what this row should say rather than a bare count  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** services/medallion/src/medallion/api/rerun.py:117 now reads `async def _require_edge_rung(fga_client: OpenFgaClient, *, subject: str, project: str, gate: StageRunnerGate) -> None:` — no `# noqa: ANN401`, no `Any`. |
| ~~Q3-37~~ | ~~`SKG-14`~~ | ~~med~~ | The audited scope sits under a blanket 21-rule ruff exemption (5 lines still in `pyproject.toml`) — **RE-MEASURED AT HEAD 2026-09-09 (§ Q12-1's re-run): still 5 globs × 21 rules**, covering `services/{catalog,lineage,medallion,maintenance,viewer}`. **AND ITS OWN JUSTIFICATION HAD ROTTED:** the comment read *"NOT a permanent exemption — tracked in docs/OPEN-WORK.md so it is revisited, not forgotten"*, and **that file does not exist** — the sentence promising review pointed at nothing, which is exactly how a temporary waiver becomes permanent without anyone deciding it should. Repointed at this row, which does exist and is counted. **FOUR MORE NARROWED 2026-09-09, by the same method — measure which of the 21 actually fire, then write only those.** `service_kit/openlineage.py` and `service_kit/control_events.py`: **zero** of the 21 fire, so both exemptions are DELETED outright. `scripts/ray_stage_job.py` and `service_kit/dapr_publish.py`: **one** each (`ANN401`). One measurement corrected itself and is worth keeping: `ray_stage_job.py` also reported `RUF100`, which I first dismissed as an `--isolated` artefact — it is not. `BLE001` is genuinely not enabled for `scripts/`, so the `# noqa: BLE001` there was DEAD, and the right answer was to delete the directive (keeping its rationale as prose) rather than exempt the rule that found it. **13 blanket entries remain and 11 are out of scope** by the owner's ruling — `viewer`, `search`, `annotator`, `media`, `runners/**` and the train/dummy Ray jobs. **A WIDER MEASUREMENT came with it:** 15 `docs/*.md` paths named across the estate resolve to no file — though several (`docs/generic-tables.md`, `docs/lance-rest-service.md`) are LAKEKEEPER'S and GRAVITINO'S own docs cited in the build-vs-buy audit and are correct as written, so a gate on this shape must tell our docs from someone else's or it fires on accurate prose  **STRUCK 2026-09-10 by measurement, not by argument.** Each candidate was copied outside the glob and re-checked against the same config: `service_kit/obs.py` fires ZERO of its 21 rules — a row that suppressed nothing while implying the file needed suppressing — so it is DELETED, the disposition `schemas/**` got in August. `scripts/medallion_demo.py` fires exactly ONE (PERF401) and is narrowed 21 -> 1. The remaining entries stay: narrowing one needs its own measurement, never a copied list. `uv run ruff check` green. |
| ~~Q3-38~~ | `PS-15` | med | `ray_kit.submit` — deterministic ids and the reattach branch — has no test — **STRUCK 2026-09-09, MEASURED AT HEAD (§ Q12-1's re-run). THE SUBJECT MOVED AND THE FINDING DID NOT SURVIVE THE MOVE.** `ray_kit` exposes no submit at all now — its own `__init__` says so: *"THE SUBMIT KERNEL IS NOT HERE. `submit.py` was pure HTTPX and its only consumer was the medallion"* — and the kernel lives at `medallion.services.ray_jobs_api.submit_or_reattach`. **Both halves the row names are tested there:** `test_submission_id.py::test_redelivery_reattaches_same_stage_token_work_is_same_id` covers the deterministic id AND the re-attach, and `test_a_vanished_stage_is_resubmitted.py` covers the resubmit branch. **A ROW MUST BE MEASURED AT THE DESTINATION, not at the address it was written against** — grepping `ray_kit` alone would have read as "the function is gone, refuted", which is a different and wrong verdict. **Found while measuring:** that second test's docstring named `medallion.services.ray_jobs_api_or_reattach`, a find/replace artefact naming no symbol; one instance, repaired |
...
| ~~Q4-3~~ | ~~`test_dummy_lane_e2e` terminal event~~ | `namespace:acme-silver -> table:acme-silver$dummy` link absent | The suite names it: `seed_estate.py` seeds `$features` (the HTR lane's output) and not `$dummy`. A seed gap, not a code defect  **STRUCK 2026-09-10.** The missing link is written, and as a VARIABLE rather than another hard-coded row: `SILVER_TABLES="${SILVER_TABLES:-features dummy}"` in `seed_medallion_fga.sh`, mirroring `INGEST_TABLE` beside it. The sharper reason for a variable here is the platform rule — `dummy` is a WORKLOAD name and no script of the platform's should have to be edited to add a lane. `seed_estate.py` declares `acme-silver` with both tables, and `--dry-run` now emits `POST /v1/table/acme-silver$dummy/declare` with `parent=namespace:acme-silver` — which IS the link, since the declare carries the parent. Verified in the dry run. |
| ~~Q4-4~~ | ~~`test_observability_e2e` (4 errors)~~ | 400 — a top-level namespace must belong to a warehouse | The suite creates an unbound namespace, which `require_warehouse_scoped` refuses when `catalog.warehouses.enabled` is on. It is OFF in the kind stack and ON here, so the suite has only ever run against half the estate's shapes  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** The row's stated cause was the suite firing the pre-warehouses root door and getting 400 from `require_warehouse_scoped`. BOTH halves are closed at HEAD. |
| ~~Q4-5~~ | ~~`test_multibase_e2e` (3)~~ | 403 `can_create_table required` | alice holds no grant on the throwaway namespaces these mint. The kind stack seeds them; a live estate does not, and the suite cannot assume its own fixtures exist  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** tests/e2e-py/test_multibase_e2e.py:25 imports `from topology import assert_parent_exists, create_top_level` and :96 now does `assert_parent_exists(create_top_level(catalog, "mbns", _auth()), "mbns")`. |
| ~~Q4-6~~ | ~~`test_warehouses_e2e::test_per_warehouse_physical_isolation`~~ | `AssertionError: []` — no objects where isolation was expected | Unclassified. Needs driving by hand before it is called a defect or a fixture gap  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** The `AssertionError: []` was a namespace hijack, not a code defect: the suite bound the shared fixed name `e2ens`, which another suite had already bound to `warehouse:lakehouse-wh`, so every table landed there and bucket-a read empty. Closed by 68d9f3be. |
| ~~Q4-7~~ | ~~`test_maintenance_e2e::test_sweep_compacts_real_datasets_and_meters`~~ | `KeyError: 'datasets'` | The sweep's response shape and the suite's expectation disagree. One of them is stale and it is not yet established which  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** The `KeyError: 'datasets'` was the suite asserting the SERIAL lane's response shape against an estate running the QUEUE lane. |
| Q4-8 | `test_outbox_e2e::test_reconcile_sweep_drains_a_staged_outbox_event` | staging passes, `outbox_drained: 0` | **THE STRIKE WAS WRONG AND IS WITHDRAWN 2026-09-10.** Struck earlier the same day on an agent's code reading, which was right about the CAUSE IT CHECKED and wrong that the row was closed: the key-format half is genuinely fixed (`tests/e2e-py/test_outbox_e2e.py:75` computes `key = f"{run_id}@{event['eventType']}"`, matching `service_kit/lakehouse/outbox.py:36 _object_key`), and the test's staging assertion passes live because of it. The test still FAILS, on a second cause the reading never reached. Measured against the running lineage: the reconcile tick RAN (`checked: 350`, not skipped), the event stayed staged, and `lineage_outbox_event_stranded` fired x4 in 40 minutes. `services/lineage/src/lineage/api/reconcile_cron.py:338-375` wraps `repository.ingest_event` -> re-publish -> `outbox.drop_event`; any exception strands and logs the cause **in `extra`**. Transport is ruled out — `POST /v1.0/publish/lineage-pubsub-lineage/lineage.events.v1` against the sidecar answered 204 — so the strand is a graph write. The cause was unreadable on stdout until the formatter fix below; re-measure after it deploys. **Rule this cost:** a code reading proves a MECHANISM, never a row — a verdict is not evidence it is still true, and that applies to a verdict reached an hour ago.  **2026-09-10, same reconcile tick: `outbox_drained=0 outbox_stranded=0` with `checked=351`.** The outbox is EMPTY, so the strand is not reproducing right now and cannot be read off the running estate today. The next attempt has to STAGE an event and watch that one, rather than wait for the condition to recur |
| ~~Q4-9~~ | ~~`test_warehouses_e2e::test_create_warehouse_denied_for_non_admin`~~ | bob CREATED the warehouse | NOT a governance hole, and this nearly went in as one. `team:eng` is bound to `project:acme` and `project.admin` is "… or member from team"; bob is a member, so he IS an admin here. The RUNNER was wrong to assume the identity — it now verifies `can_administer` is false before offering the token, and the leg SKIPS otherwise. An honest skip beats a red test alleging something untrue  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** Both halves the row describes are in the tree. |
...
| ~~Q5-2~~ | ~~Maintenance cannot compact ANY tenant's tables, and the RUNG may be the defect rather than the grant~~ | Refused live: `credentials?tier=write` -> **403**, `compaction_plan` -> **403**. Established 2026-09-05 that this is not about runtime-minted tenants at all — `service-maintenance` is absent from `LANCE_FGA_CASCADE_WRITERS`, so it holds nothing on ANY warehouse. **The question is which rung, and the model has no good answer.** Vending needs `can_write_data` (= `writer`); the compaction door is gated on `can_drop` (= `owner`), so admitting maintenance the ordinary way hands it `can_deregister` and `manage_grants` on every tenant — the same over-grant Q2-6 flags for the stage runners. A COMPACTION DROPS NOTHING, and the model has no `can_compact`/`can_maintain` relation to gate it on (grepped: none exists). So: (a) add a maintenance rung to the model and gate the compaction doors on it — most work, least privilege; (b) grant `writer` and re-gate `compaction_plan` off `can_drop`; (c) grant `owner` and accept the breadth. **OWNER RULING** — (a) is the honest design and it changes the authorization model. **DONE AND STRUCK 2026-09-09 (§ Q17-40).** Option (a) shipped: the model carries `define can_maintain: maintainer` (`model.fga:364`), refuting this row's own *"grepped: none exists"*, and `fga_deps._MAINTENANCE_ACTIONS` maps `compaction_plan`/`compaction_commit` onto it explicitly rather than letting them fall to the writer rung. Measured on the deployed estate: **1 818 `can_write_data` denials for `service-maintenance` before, 0 after**, and 728 units running `distributed` where all 4 274 had run `in_pod`. The over-grant this row warned about is avoided — maintenance holds `maintainer`, not `owner`, so it gained no `can_deregister` and no `manage_grants` |
| ~~Q5-3~~ | ~~The ambient-credential log line names the wrong key~~ | `write credential AMBIENT … this rewrite is signed by the root key` — it is signed by `rask-maintenance`, which is not the root key on this estate. False prose in an operational log, which is where it is hardest to catch  **STRUCK 2026-09-10.** Both lines NAME the credential instead of ranking it — the per-table `AMBIENT` line and the once-per-process `NOT CONFIGURED` notice now read `signed by the process credential %s` from `settings.s3_access_key_id`, falling back to `<ambient environment>` when unset. `rask-maintenance` is a scoped identity, so "the root key" reported a posture the estate had already left. The key id is keyword-only: `test_helpers_are_called_by_name` refuses two positional `str` parameters, and it caught this on the first run. Tests assert the id appears AND that "root key" does not, so the wording cannot revert. |
| ~~Q5-4~~ | ~~`bronze-media` is measured for every tenant that has no media lane~~ | `GET /v1/table/c6t115034-bronze-media$objects/tags/list` → 403 on the lag tick. The lane map is estate-wide, so a tenant using only the tabular lanes still has its media edge probed. Harmless (it counts UNMEASURABLE) but it is 1/3 of the tick's work for nothing  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** The unconditional per-tick probe is gone. |
...
| # | Finding | Sev | What remains |
| --- | --- | --- | --- |
| Q6-1 | The compute credential can enumerate every bucket | med | Measured: `mc ls ray/` returns 104 buckets with the deployed key. Withholding `s3:ListAllMyBuckets` does not withhold the list — RustFS falls back to per-bucket `ListBucket`, which `arn:aws:s3:::*` grants. The widening is the deliberate trade (the allow-list it replaced broke every tenant's cascade); the narrowing that closes it is PER-TABLE VENDED credentials on the Ray lane, a service change. Same root as Q5-2. **RE-MEASURED 2026-09-09 THROUGH THE RUNNING RAY HEAD, and it is still live: `list_buckets()` with the pod's own `S3_KEY`/`S3_SECRET` returns 105 buckets** (104 when filed — the estate grew, the exposure did not shrink). The identity IS scoped (`S3_KEY=rask-ray-compute`, not the root), which is the § F2-1 half; the READ breadth is the half that remains, and the row is right that a policy edit cannot close it. **THIS IS THE GOAL'S PATH 3 AND THE PATTERN ALREADY EXISTS**: maintenance vends `credentials?tier=write` per table and logs `write credential SCOPED for <table>` — 277 such vends measured in one window. The Ray lane injects a long-lived scoped key at submit instead. **AND THE OBVIOUS CLOSE IS BLOCKED BY A MEASURED CONSTRAINT, which is why this row now asks for something different.** Vending a 900 s credential and putting it on the work order means putting it in `runtime_env.env_vars` — and `ray_submit.py:205-215` records why nothing may ride there: *"the Jobs API echoes `runtime_env` on `GET /api/jobs/<id>`, an unauthenticated dashboard published at the edge, so anything here is readable by anyone"*. Both halves of the credential were deliberately REMOVED from that dict (2026-08-28 and 2026-08-30, the second after `SignatureDoesNotMatch` was measured twice live). A short-lived credential published to an unauthenticated endpoint is a shorter-lived leak, not a fix — the goal's own *"a scoped static key is not a fix"* argument with the axis rotated. **The path that survives the constraint is the goal's PATH 2**: a Ray pod has no Dapr sidecar, so ESO materialises the credential as a Secret and the RayJob CR mounts it — which never traverses `runtime_env` and so is never echoed. That makes the close a CR/chart change plus a per-job Secret lifecycle, not a submitter change, and it is a different piece of work from the one this row proposed. Recorded rather than half-built. **THE BLOCKER'S STATED REASON IS A DEV-ESTATE FACT AND IS FALSE IN PROD — corrected 2026-09-09 so nobody re-litigates this row on a premise they can disprove in a minute.** *"An unauthenticated dashboard published at the edge"* is wrong twice for a prod render. (1) `ray.auth.enabled=false` ABORTS a prod render — `test_ray_auth.py:176` pins it, with the reason *"an unauthenticated Ray dashboard is remote code execution (jobs/Serve REST take arbitrary entrypoints)"* — so prod cannot deploy an open dashboard at all. It is open on THIS estate, which is the dev default and is how the echo was measurable at all (§ Q17-52). (2) Port 8265 is on no ingress path (live: `/api`, `/compute`, `/studio`, `/models`, `/lakehouse`, `/explorer`, `/annotator`, `/dex`, `/`); the Ray surface reaches the edge only through the gateway's `/api/ray` row, and `ray_kit.schemas.RayJob` is `extra="ignore"` over ELEVEN declared scalars that do not include `runtime_env` — the model's own comment records that `extra="allow"` had retained it on every row. Measured: that route answers 401 without a bearer. **THE CONCLUSION SURVIVES ON A DIFFERENT AND NARROWER ARGUMENT**, which is why the row is corrected rather than struck: a prod reader still needs only the ONE shared dashboard token to read EVERY job's `runtime_env`, and the job record outlives the credential — the 7 jobs read on this estate include runs from previous days, against a 900 s TTL. "One shared token reads every vended credential, and keeps it past its expiry" is still the wrong shape; it is simply not "readable by anyone" |
...
| ~~Q6-4~~ | ~~A queued index build emits no lineage anywhere~~ | ~~med~~ | The door skips the emit and the worker never makes one, so an index that took an hour is invisible to the run board  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** The worker half arrived. |
| ~~Q6-5~~ | ~~`plan_compaction` answers 400 where every sibling door answers 404~~ | ~~med~~ | **STRUCK 2026-09-09 — the 400/404 half was already fixed and never marked, and RE-MEASURING IT FOUND A WIDER ONE (now Q17-55).** The door raises `TableNotFoundError` for a proven absence and is pinned by two tests in `test_rename_moves_a_pointer_not_bytes.py`. What re-measuring added: it did not use its own module's absence vocabulary, and neither of the two answers it could give was right for a THIRD condition. Driven on pylance 11.0.0 against a stub S3 endpoint, the door now gives three: 400 for an unsupported policy knob, 404 for a proven absence, 503 for a store that did not answer. Original finding: "registered but never written" mapped to `InvalidInputError` off a bare `ValueError`; siblings raise `TableNotFoundError`, so a client dispatching on the code saw a different class for the same condition |
...
| ~~Q6-8~~ | ~~The RayJob Role grants `list` and `watch` the executor never issues~~ | ~~low~~ | Narrow to `create,get,delete`  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** chart/templates/medallion-rayjob-rbac.yaml:33 reads `verbs: ["create", "get", "delete"]` — exactly the narrowing the row asked for. The comment above it states the row's own reason ("`list` and `watch` would let this ServiceAccount enumerate every RayJob in the namespace, which on a shared cluster is every tenant's"). |
...
| ~~Q8-6~~ | ~~`test_catalog_live` — domain-error translation fails; schema round-trip and milestone loop error~~ | ~~med~~ | The catalog's own error contract  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** Drove it against the deployed release just now: `bash scripts/e2e_live.sh tests/e2e-py/test_catalog_live.py` -> 3 passed in 4.47s. |
| ~~Q8-7~~ | ~~`test_e2e::test_unsupported_is_406` gets 401~~ | ~~med~~ | The auth door fires before content negotiation. May be correct fail-closed behaviour with a stale assertion — classify before fixing  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** Drove it live: `bash scripts/e2e_live.sh tests/e2e-py/test_e2e.py` -> `test_unsupported_is_406 PASSED`, 2 passed in 1.95s against catalog 10.43.220.241:2333 with a Dex-minted alice bearer. The row's own instruction to classify before fixing was answered by commit 68d9f3be (2026-09-06 22:35, i.e. |
| ~~Q8-8~~ | ~~`test_e2e::test_full_lifecycle` fails~~ | ~~med~~ |  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** Same live run: `tests/e2e-py/test_e2e.py::test_full_lifecycle PASSED` (2 passed in 1.95s, deployed k3s release, warehouse `acme-bucket` discovered for project acme). |
...
| ~~Q8-12~~ | ~~Five legs still need a SECOND tenant (`LANCE_E2E_PROJECT_B` + its token)~~ | ~~med~~ | Cross-tenant credential isolation is unproven live. Provisioning a second project on the estate is the work  **STRUCK 2026-09-10 — the runner provisions tenant B, driven against the deployed catalog.** Five legs asserting that a credential vended for tenant A is REFUSED on tenant B's data — the sharpest statement of storage isolation this estate makes — skipped because `scripts/e2e_live.sh` set neither variable they gate on, while discovering every other address and credential from the cluster. **A FIXED ID, not one per run**: estate furniture like `acme`, because a per-run tenant leaves a project behind on every drive and a project has no retention — which is precisely what § Q8-15 measures the cost of. Bob administers tenant B while alice administers acme, so the legs are a real CROSS-tenant test rather than one principal talking to two of its own projects. **Measured**: `POST /v1/projects` 200 creating `e2etenantb`, the grant 200, and a SECOND create **200 echoing both admins — it converges rather than 409ing**, which corrected the comment I had written from the row. Guarded like the warehouse block: a refusal prints a note and the legs skip as they do today. **STATED HONESTLY**: making them RUN is this change; if a leg then goes red it has found a genuine isolation defect, and that is separate work. **DRIVEN 2026-09-10 AND ALL EIGHT PASS — the isolation claim is proven live for the first time, and proven NON-VACUOUSLY.** Tenant B cannot list tenant A's bucket (read and write tiers), cannot read A's objects, and its WRITE credential cannot put into A's bucket; a read-tier credential cannot write to its OWN table. **THREE DRIVES, and the middle one is the one worth keeping**: (1) 8 failed on `Temporary failure in name resolution` — the catalog vends its own IN-CLUSTER address and these suites run from the host, so the ADDRESS is now remapped to the discovered ClusterIP and nothing else is; the key, secret and session token are used exactly as vended, because the policy is evaluated against bucket and prefix, not against the route a request arrived on. (2) 6 passed, 2 failed — and the 2 were the POSITIVE CONTROL, which is the one combination that must not be reported as success: the suite's own docstring says a credential *"refused EVERYWHERE would satisfy every assertion above while breaking the product"*. (3) The control asked `Prefix="isobns"`, the namespace NAME, while the `dir` backend lays tables out FLAT (`describe` -> `s3://e2e-iso-b/1d68ea33_isobns$isobtbl`). **A first diagnosis of "test bug" was one step early**: the real prefix WITHOUT a trailing slash is also denied, because the policy gates `s3:ListBucket` on `s3:prefix` matching `<prefix>/*` — the slash is load-bearing, and only checking both forms showed which half was wrong. The control now asks the catalog where the table is instead of composing the path |
...
| Q8-15 | The live sweep reports 37 `storage_loss` and 79 `unreadable` datasets | high | **CAUSE FOUND 2026-09-07, and it is not a data problem — it is TEST RESIDUE IN THE PRODUCTION LINEAGE GRAPH.** All 79 `unreadable` are `NoSuchBucket` on `lakehouse` / `landing` — buckets that do not exist — and every name carries an e2e marker (`bronze$events_e2e12bc3bbc`, `e2e049e1466-gold$catalog`, …). The 37 `storage_loss` are the same shape: `advz1788208093ns$t1`, `qp77c7f7b2ns$qt`, `recon$t`, and literally `probe$nonexistent` — a test that asked about a dataset that never existed, whose node the graph kept forever. **THE MECHANISM: the e2e suites write into the estate's real lineage graph and NOTHING removes what they leave.** `prune_runs` is the only prune in the service and `LINEAGE_RUN_RETENTION_DAYS=0` on this deployment (opt-in, default keep-everything); there is no dataset-node prune at all. Measured in AGE: **1,230 Dataset nodes**, 400 of them still sweepable. **THE COST IS THE CONTROL, NOT THE DISK.** Both warnings fire on every tick with 116 dead rows in them, so a REAL storage loss or a genuinely unreadable dataset arrives invisible — the estate's own 'a control that cannot fire' pattern, reached by accumulation rather than by design. And each tick pays 79 failed S3 list calls INSIDE the single-flight lock that also gates the outbox drain (Q8-16). **MECHANISM LANDED, ARMED AND OBSERVED 2026-09-08 — THE GRAPH IS NOW CONVERGING.** Retention is 30 days (owner ruling) and one driven tick pruned **285 runs and 117 orphan datasets**: `datasets 1,271 -> 1,154`, `runs 5,692 -> 5,414`, with `lineage_orphan_datasets_pruned` in the log. **CONVERGENCE RE-MEASURED 2026-09-09, and it is working: `unreadable` 79 -> 2, `storage_loss` 37 -> 32.** The maintenance sweep now logs NEITHER warning in a 40-minute window; the two that remain come from lineage's own reconcile. **AND THE LAST TWO UNREADABLE LOOK ALARMING AND ARE NOT** — `gold$catalog` and `uiproof-gold$catalog`, which read as the cascade's live gold tier. Their errors name the BUCKET: a 404 listing `lakehouse/gold/catalog/_versions/` and `uiproof-wh/medallion/gold/_versions/`. Checked against the object store, **neither bucket exists**, while `lance-catalog` (where the real gold tier lives) and `lakehouse-wh` both do — so they are graph nodes pointing at storage never created or since deleted: residue, exactly as this row diagnosed, not the medallion failing. NOT YET STRUCK: convergence is gradual by design — residue leaves as its runs pass 30 days — so the two warnings keep shrinking rather than stopping, and this row closes when they carry only live datasets. **RE-MEASURED 2026-09-09 ON THE LIVE SERVICE: `storage_loss` 32, `unreadable` 2 — and the residue was TRACED, which corrects the row's mechanism.** It is NOT the committed e2e suite: `advz`, `auditzz`, `branchaud1`, `cq7`, `mpdd`, `qp`, `refutx` appear NOWHERE in the tree, and `advz1788208093ns` decodes to 2026-08-31 — ad-hoc probe scripts run against the live estate during earlier audit sessions, which no longer exist and cannot regenerate. So the backlog is FINITE and retention really does clear it; the row's convergence claim holds. **What DID regenerate was one literal, now fixed:** `test_reconcile_backfills_a_dropped_write` writes the bare, unprefixed `recon$t` with a `tmp_path` dataSource, cleaned BEFORE the run and never after — so every execution left a node pointing at storage pytest had already deleted, and it was one of the 32. Setup and teardown now share one `_forget_recon` helper so they cannot clean different sets. **AND THE PRODUCTION PATH IS CORRECT, which I checked before proposing to change it:** `reconcile_all` skips any dataset carrying a `drop_table` stamp (`core/reconcile.py:214`), so a properly dropped table never reports loss and a vanished bucket SHOULD — there is no production gap here to close, and the teardown is not exercised in this commit because the AGE password reaches the service through Dapr rather than env, which is the zero-trust design working **RE-MEASURED 2026-09-10 AND THE PURGE WAS SCOPED DOWN BY WHAT THE MEASUREMENT FOUND.** Both warnings still fire every 5-minute tick (6 in a 25-minute window); AGE holds 1,163 Dataset nodes. **A name-pattern purge would have deleted LIVE provenance**: listing candidates with `max(r.event_time)` showed several `e2e-ns$*` nodes with runs from the same morning (06:03) — the committed e2e suites REWRITE those, so they are churn, not residue, and deleting them erases records that come straight back. The genuinely-dead cohort is **19 nodes** whose newest run is 2026-08-31 to 2026-09-07 and whose namespaces appear nowhere in the tree: `branchaud1ns$t1..t5`, `auditzzns$t1..t5/r1/d1`, `qpa729e595ns$qt`, `refutx1ns$t1`, `advz1788208093ns$t1`, `cq7ns$fin1/fin2`, `probe$nonexistent`, `recon$t`. **NOT EXECUTED: the harness refused the destructive graph write**, and it is not to be worked around. Nor does it have to run — the row's own convergence argument holds: retention reaches the 2026-08-31 runs on 2026-09-30. The purge buys twenty days of warnings that name only live datasets.  **RE-MEASURED 2026-09-10 AND THE PREMISE HAS MOVED — 32 `storage_loss`, 2 `unreadable`, down from 37 and 79.** Readable BY NAME for the first time: the reconcile warnings carry `datasets=[...]` in `extra`, which the stdout formatter dropped until today, so this row's cause was previously an inference from counts and is now a list. Every one of the 32 is test residue, which settles the diagnosis rather than restating it: sixteen random-suffixed e2e namespaces (`advz1788208093ns$t1`, `aud1ns$sub3$tt`, `auditzzns$t4`, `branchaud1ns$t1`, `cli07837ns$t2`, `cq7ns$fin2`, `e2e-ns$t178b2dda`, `mpdd929469ns$mv`, `qp77c7f7b2ns$qt`, `qpa729e595ns$qt`, `refutx1ns$t1`, `st2aea6b2ans$stt`, `up9f457931ns$ut`, `vtc18ca740ns$vers`, `vv6bfc7e9bns$vt`), eleven `tracka$*` and five `trackansd*$*` rows from the annotation and branch suites, and `probe$nonexistent` — a test that probed a table it never created. **The `unreadable` half has all but gone: 79 -> 2**, `gold$catalog` and `uiproof-gold$catalog`, both still `NoSuchBucket` on the `lakehouse` bucket exactly as the row describes. **NOT ATTRIBUTED, because two changes this week could account for the drop** — the absence-vocabulary convergence (§ Q17-55, Q6-5), which stopped a missing bucket reading as an unwritten table, and the e2e unbind fixture that now releases what a run bound. Naming a cause here would be the reverse of what this row just earned: the numbers are measured, the attribution is not |

**THE FIRST IMPLEMENTATION FAILED ON THE LIVE ESTATE AND NO TEST COULD HAVE CAUGHT IT.** It was written `WHERE NOT (d)<-[:WROTE]-(:Run)`; AGE 1.5.0 rejects a negated pattern predicate with `syntax error at or near ":"`. The pins assert on the query STRING and never execute it, so the suite was green and the deployed prune failed on its first tick (`lineage_dataset_prune_failed`). The `OPTIONAL MATCH` + `count(...) = 0` form was then PROVED against the running database before being written into the code, and a pin now refuses a negated pattern by shape. **This is the row's own lesson arriving one layer down: a structural test cannot check syntax, and only the live drive closes that gap.**

**THE FORK IS NOW ONE VALUE.** `prune_orphan_datasets` reclaims a Dataset node no run refers to any more, batched like `prune_runs` and running immediately behind it. **A MEASUREMENT FIRST REFUTED THE OBVIOUS FIX:** 1,271 Dataset nodes and 1,271 with an incoming edge — there is NO isolated node to reclaim, so a standalone "delete unreferenced nodes" sweep frees nothing. The residue is reachable only THROUGH its runs, which makes this the second half of run retention rather than a sweep of its own, and it sits behind the same guard. A dataset with a `CREATED` edge is exempt: that is a declared table, and deleting it would erase the record that someone made it. **SO WHAT REMAINS IS THE RETENTION PERIOD, and only the owner can set it** — `LINEAGE_RUN_RETENTION_DAYS` is 0 (keep everything) on this deployment, so nothing is pruned and nothing is orphaned. Naming a value arms both halves at once. Reclassifying `NoSuchBucket` as its own quieter class stays the ONE option to refuse. The original fork was**: make the suites clean up after themselves; give dataset nodes retention the way runs have it (and pick a rule — no successful run in N days? unreachable across N consecutive sweeps?); or set `LINEAGE_RUN_RETENTION_DAYS` and let the derived drop-stamp logic carry it. Reclassifying `NoSuchBucket` as its own quieter class is the ONE option to refuse: a production bucket actually being deleted is the emergency this warning exists for |
...
| ~~Q8-16~~ | ~~The outbox drain is gated behind a whole-estate storage scan~~ | ~~med~~ | Measured 2026-09-07: `_on_cron` runs the 400-dataset `_sweep`, the outbox drain and the run prune under ONE advisory lock, in that order. So the durability-critical step waits on the slowest one, completed sweeps are minutes apart against a 30 s cron, and every tick in between is `skipped_locked`. A staged event's recovery latency is therefore set by how many datasets the estate has, which is the wrong variable. Splitting the drain onto its own lock (or its own tick) is the shape; the sweep's cost is the reason. **STRUCK 2026-09-09 — fixed by ORDER rather than by a second lock, and nobody marked it.** `reconcile_cron.py:249-257` runs `_drain_outbox` BEFORE `_sweep` inside the same single-flight lock, with the rationale at the call site: *"Both reasons — cost and fidelity — are properties of the drain relative to the sweep, so they live with the function that has them."* A staged event's recovery latency is now the drain's own cost, not the dataset count — which is what this row asked for, reached by the cheaper of the two shapes it proposed |
| ~~Q8-14~~ | The new locator gate misses ~21 further dangling pointers | low | **MEASURED AND CLOSED 2026-09-09, and the count was 9 rather than ~21.** The gap was structural, not a tuning miss: `_ROW_ID` matches `\([A-Z]\d[a-z]?\)`, a shape that CANNOT express `(§2.4)` — so the `§`-form was unchecked in every file rather than under-checked. Measured across the estate: **9 sub-pointers resolve and 9 dangled**, all `§2.1`–`§2.5` beside `"The compute plane is decoupled"`, and those markers appear **nowhere in DECISIONS.md** — they were the numbering of a deleted plan. **WHY THEY SURVIVED REVIEW is the reusable part:** the section citation beside them resolves, so the half a gate checked was true and the half it could not see was not. `test_no_citation_carries_a_deleted_plans_SECTION_MARKER` now checks the `§` form (verified RED against all nine first), and the dangling markers were REMOVED from 18 files, keeping the citation that resolves — `§7.4`, which does exist, is untouched. | Its regex sees a row-id form only; the `§7.11` / `§2` / `Phase 1` forms escape it, across 16 files including `docs/DECISIONS.md`. Found while scouting Q7-4 |
...
| ~~Q9-4~~ | ~~The cascade head still TELLS the catalog a composed location~~ | ~~high~~ | **THE RETRY LIE IS FIXED 2026-09-08; THE 'TELLS RATHER THAN ASKS' HALF IS DELIBERATE AND STAYS.** Reading `produce.py` settles the second half: the departure is documented at the call site with its reasoning — the URI is a DEPLOYMENT CONTRACT the chart renders for both the head and the stage runner's `MEDALLION_FROM_URI` from one expression, and `register_table` is the door for exactly that writer. The cascade does not depend on it either: `/bronze-arrival` resolves the arrived table's location through the catalog. So that clause is a recorded design choice, not a defect awaiting repair. **WHAT WAS REAL IS THE ERROR CONTRACT, and it is this row's own sharpest sentence:** a location disagreement answered `503 Retry-After: 5` promises a convergence no retry can produce. `RegisterError` covered two causes with one answer — an unreachable catalog (transient, retry converges) and a table registered elsewhere (permanent; the remedy is an operator deregistering it). `LocationConflictError` now separates them and the route answers **409**, the same code it already gives its other permanent conflict (`UnresolvableProjectError`), carrying the operator action instead of a Retry-After. Same shape as § H10: a refusal answered with advice the caller cannot act on. **STRUCK 2026-09-09 — the residue paragraph described the state BEFORE the fix in the same row, and two clauses of one row disagreed.** Measured at HEAD: `api/produce.py:106` answers a `LocationConflictError` with **409** carrying the operator action, and 503+`Retry-After` is left only for `RegisterError`, the genuinely transient one. Pinned by `services/medallion/tests/test_a_location_conflict_is_not_retryable.py`, which asserts both directions. The 'TELLS rather than ASKS' clause stays a recorded design choice, as the row's own first half says: the URI is a deployment contract the chart renders for the head and the stage runner from one expression, `register_table` is the door for that writer, and the cascade does not depend on it because `/bronze-arrival` resolves the arrived table's location through the catalog |
...
| ~~Q10-2~~ | ~~Two modules hand-roll manifest protobuf parsing to read base paths the library exposes~~ | ~~med~~ | **VERIFIED.** `ds._ds.base_paths()` exists on pylance 10.0.0 (returns `{}` on a base-less dataset). `blobs.py:61` states "THIS EXISTS BECAUSE PYLANCE EXPOSES NO WAY TO READ A DATASET'S REGISTERED BASES" and `lander.py:338` repeats it; `features.py:160-260` walks varints at manifest fields 18/4/3. Two second sources of truth, one of which travels with any schema copy. **CLOSED 2026-09-09, and the split is the point.** `blobs.py:112` already asked the library (`ds._ds.base_paths()`), keeping the schema stamp as a documented FALLBACK that answers a different question — which base the `blob_uri` values are RELATIVE to, not which bases the dataset REGISTERED. `features.manifest_base_path_refs` now asks it too, so the field-18 varint walk is gone. **THE FLAG WALK STAYS AND THAT IS NOT AN OMISSION**: pylance exposes neither the reader nor the writer feature-flag field, so for fields 9/10 the bytes ARE the only source. Parse what the library will not answer; ask it for what it will. **VERIFIED EQUAL ON A REAL `shallow_clone`** (pylance 10.0.0) before the swap: the accessor returned `{0: DatasetBasePath(id=0, name=None, path=<source>, is_dataset_root=true)}` and the varint walk `[BasePathRef(path=<source>, is_dataset_root=True)]` — the same two facts. After: a clone reports its source, a plain dataset reports `[]`, and `manifest_feature_flags` still reads `(18, 18)` from the bytes. 196 base/clone/feature/orphan/compaction tests green |
| ~~Q10-3~~ | ~~`defer_index_remap=True` is requested on every dataset and refused on every governed one~~ | ~~med~~ | Lance: stable row ids need no index remap. `optimize.py:237` asks anyway, catches the refusal by substring, retries plain — 211 `compact_defer_index_remap_unsupported` warnings in 48h. The pinning test builds its fixture WITHOUT stable row ids, so it pins the path no governed dataset takes. **STRUCK 2026-09-09 — fixed and unmarked.** `optimize.py:286-296` now ASKS ONLY WHAT LANCE CAN ANSWER: it reads `has_stable_row_ids` off the dataset (the same public accessor `ingest/catalog.py::A14` uses) and skips the option, so the refusal is predictable rather than discoverable. The `except` deliberately stays — Lance refuses in the OTHER direction too, for a dataset that is neither, and a third reason must degrade to a plain compaction rather than fail a tick. **Re-measured on the deployed sweep: 0 `compact_defer_index_remap_unsupported` in the last 4 000 log lines, against 211 in 48 h** |
| ~~Q10-4~~ | ~~`id` is the merge key the tier contract requires but is never declared as Lance's unenforced primary key~~ | ~~med~~ | **NOT A NEW FINDING — the estate already records it.** `ingest/catalog.py::A14` states it plainly: "NOT an 'unenforced primary key' in Lance's sense… that feature is opt-in through field metadata (`lance-schema:unenforced-primary-key`), which this plane sets nowhere… Declaring it properly is open work; claiming it in a comment was not the same thing." So this is known open work, correctly described, not a divergence anyone was unaware of. It matters more now that both tiers merge on `id`: the key is load-bearing at every write and still only a convention. **STRUCK 2026-09-09 — declared and unmarked.** `medallion/services/ingest.py:52` builds the field as `pa.field("id", pa.int64(), nullable=False, metadata={"lance-schema:unenforced-primary-key": "true"})` — Lance's own vocabulary rather than a convention — and five sites across the estate now reference the key |
...
| ~~Q11-2~~ | ~~`ray_submit.py` goes around the compute port~~ | ~~med~~ | Recorded in `docs/DECISIONS.md` "The compute plane is decoupled" as the remaining decoupling work: it is a module of functions naming no `capabilities`, so the port cannot describe it. The estate's one ray import outside a runner | **STRUCK 2026-09-09 — the DEPENDENCY half is fixed and PINNED, and the PORT half is § Q17-1, so nothing is left that belongs to this row.** Re-measured rather than read: (1) *"the estate's one ray import outside a runner"* is FALSE — `grep` finds no `import ray` anywhere under `services/medallion/src`, and the pyproject declares no compute engine. What looked like one is `rk`, which resolves to `medallion.services.ray_jobs_api`, a LOCAL module. (2) That module records why: it was MOVED out of `ray-kit` on 2026-09-07 *"rather than copied, and the move is what lets `services/medallion` declare no compute engine at all"* — it is pure HTTPX plus OTel propagation and imports no Ray, while `ray-kit`'s SDK half (which pulls `ray[default]`) stays with `services/compute`, its only consumer. One consumer each, nothing duplicated. (3) The rule is ENFORCED, not merely intended: `tests/unit/test_no_service_depends_on_a_compute_engine.py` exists and passes (4 tests) — *"a service may adapt an engine, and must not depend on one"*. **WHAT REMAINS IS NOT THIS ROW'S:** the cascade still reaches the adapter by calling `submit_stage_job` directly instead of resolving through `engine_registry.executor_for`, and that is § Q17-1 — measured there as ONE in-scope call site (`workflow.py:495`), blocked behind § Q17-2's owner decision. Tracking it twice would make two rows move on one commit |
...
| ~~Q10-6~~ | ~~The Ray MEDIA lane still overwrites, and it cannot take the same fix~~ | ~~med~~ | `scripts/ray_stage_job.py:239` writes `mode="overwrite" if written == 0 else "append"` per BATCH. A per-batch `when_not_matched_by_source_delete` would delete the rows earlier batches just wrote, so the full-sync merge that fixed the other three write sites does not transpose. It needs ONE merge over the whole scan, or accumulate-then-sync — a streaming-shape change, not a call swap. Recorded rather than half-applied | **FIXED 2026-09-09 — the recorded blocker was REFUTED by a third option neither the row nor the code comment considered.** Both said the choices were one merge over the whole scan or accumulate-then-sync, i.e. a streaming-shape change. There is a cheaper one: LIFT THE DELETE OUT OF THE BATCH LOOP. Every batch merges (`merge_insert("id")` with update/insert and NO by-source delete, so batches cannot delete each other), and ONE pass afterwards retracts whatever this run did not write — keyed on the `run_id` each row already carries in its own `lineage` document, which `LineageDoc` documents as the field that *"joins the row back to the graph"*. **Only expressible because `lineage` is JSONB**, filterable in place — the § Q13-13 fix landed hours earlier and is what makes `json_get_string(lineage, 'run_id') != '<run>'` a predicate rather than a scan. **DRIVEN before it was written:** on a real dataset, 4 rows in two batches, then a second run producing only 2 of them — the retraction removed the other 2 and the survivors kept their `_rowid` (0 and 2, unchanged). RED-first as a rerun test that asserts survivors keep the identity the tier above resolves against. **FAILS SAFE:** `_run_id_of` returns None for an unwired lane, a non-JSON document, a missing or non-string `run_id`, and anything outside a safe alphabet (a quote-injection attempt returns None) — and a None SKIPS the retraction, because "we cannot tell which rows are ours" must never resolve to "delete everything". **AND IT CORRECTED A TEST THAT MEASURED THE WRONG SEAM:** `test_the_media_transform_never_holds_the_whole_dataset` spied on `write_dataset`, so it stopped seeing most batches the moment they merged instead of appending, while the streaming property was untouched. Moved to `_media_batch`, the one seam every write path takes. **DEPLOYED AND OBSERVED**: the Ray image was rebuilt with Dagger and rolled onto the live head as `main-06e31013`, and the running pod's own `/home/ray/jobs/ray_stage_job.py` answers `_run_id_of('') -> None`, `_run_id_of(<injection>) -> None` and `_run_id_of(<real run>) -> 'stage-abc123'` — the retraction and its fail-safe are live, not merely committed. The same exec confirms § Q17-51 in that image: `lance_storage_options` returns the `aws_`-prefixed keys |
| Q10-7 | The Ray DISTRIBUTED branch creates its target with an overwrite | med | `ray_stage_job.py` writes an empty `out_schema` table with `mode="overwrite"` then distributed-appends into it. Deeper tabular stages CARRY `source_rowid` as a plain column so the chain survives, but the tier's own `_rowid` is re-minted every run, which is what the tier ABOVE it would resolve against. Same shape as Q10-6: the create is fine, the re-create is not | **RE-MEASURED 2026-09-09 — STILL OPEN, and § Q10-6's fix does NOT transpose, which is the useful finding.** The site is `ray_stage_job.py:642-649`: an empty `out_schema` table written `mode="overwrite"`, then `lr.write_lance(mode="append")` fans fragments in from Ray. Q10-6 could stop overwriting because a single-process loop can MERGE each batch and retract once at the end. This branch cannot: `lance_ray.write_lance` accepts only `"create"`, `"append"` and `"overwrite"` (read off the installed 0.5.0 signature and docstring) — there is no distributed merge, and appending into the previous run's dataset mints NEW `_rowid`s for every row anyway, so the run-keyed retraction that saves identity in the media lane would save nothing here. What remains is the shape the row already names: stage the distributed output, then ONE `merge_insert` from staging into the target — an extra full write of the data, which is why it is a real design decision rather than a call swap. Recorded rather than half-applied |
| ~~Q10-8~~ | ~~THE DEPLOYED CASCADE RUNS THE RAY LANE, and that is how a half-fix nearly read as a whole one~~ | ~~high~~ | Measured: after deploying the in-process fix, a `POST /produce` preserved bronze's `_rowid` (`[2012..2019]` twice running) — but the stage runner's own log said `stage-ray-silver-c1proof0002`, so silver was written by the Ray lane, not the lane the unit tests exercise. Silver's `source_rowid` stayed `[88..95]`, 8 of 8 still dangling. **Any claim about the cascade that rests on `services/medallion/tests` is a claim about a lane this estate does not run.** The Ray head now merges too; the media and distributed branches are Q10-6/Q10-7. **STRUCK 2026-09-09 — the defect is repaired IN THE RUNNING HEAD and the lesson has been paid twice more since.** Measured on the deployed `ray-lance:main-921b8998`: `grep -c when_not_matched_by_source_delete /home/ray/jobs/ray_stage_job.py` = **3**. The rule this row teaches — a cascade claim resting on `services/medallion/tests` is a claim about a lane the estate does not run — caught me again today on the declared-id fix, which I deployed to the fleet image and called observed while the Ray lane still ran the old job. It belongs in the skill, not in a backlog of things to do |

| ~~Q10-9~~ | ~~A cascade fix reaches the estate through THREE images, and the obvious one is wrong~~ | ~~high~~ | Measured while deploying C1. `services/medallion/*` ships in `lance-rest-catalog` (producer + the three stage runners). `scripts/ray_stage_job.py` — the code that actually writes silver on this estate — is baked into **`ray-lance`**, not `ray-cluster` (`.docker/ray-lance.dockerfile:81` copies it to `/home/ray/jobs/`), and the head running it is `ray-lance-head`, which is HAND-APPLIED from `deploy/ray-lance-demo.yaml` and outside the chart. The live head was on `ray-lance:main-0dd7a95f` from 2026-08-30 with `grep -c when_not_matched_by_source_delete` = **0**. So a cascade change verified by unit tests, deployed to the fleet image, and confirmed by a live produce can still be entirely absent from the code that does the work. **Any cascade deploy must name which of the three images carries the change and prove it in the running one.** **STRUCK 2026-09-09 with the head re-proven**: the live `ray-lance-head` now runs `main-921b8998`, whose baked `ray_stage_job.py` carries both the merge (3 sites) and the `RASK_DEST_TABLE` read the declared-id fix needs — verified by exec against the running pod, not by the tag. The rule stands and is now applied per deploy rather than tracked as work |
...
| # | Finding | Sev | What remains |
| --- | --- | --- | --- |
| ~~Q12-1~~ | One Q3 row could not be measured | low | 38 of 39 returned; the 39th agent did not complete. **RE-RUN COMPLETE 2026-09-09, and it was not one row — TWELVE carried no verdict.** Every one is now measured against HEAD. **Struck: 5** — Q3-14 (REFUTED: exactly one storage_options builder exists and it IS the canonical one), Q3-29 (fixed, and the fix documents itself on the line), Q3-30 (12 generator workflows, all correctly annotated), Q3-38 (the subject MOVED to `ray_jobs_api.submit_or_reattach` and both halves are tested there), Q13-15 (its file is gone). **Corrected but open: 7** — Q3-12 (the DUPLICATION is gone, one half remains), Q3-13 (3 builders and 4 kernels, not 4 and 2), Q3-28 (settings at import YES, logging NO), Q3-33 (one seam, `records.py:150`), Q3-34 (dict half stands, positional-tuple half gone), Q3-35 (6 splat sites), Q3-36 (four ANN401 seams, not two). **THE METHOD DECIDED THREE OF THE VERDICTS**, which is the reusable finding: grep could not answer Q3-30 (right and wrong annotations differ only in the return type), Q3-28 (a text search cannot tell a module-level call from one inside a function) or Q3-14 (`aws_access_key_id` appears in builders and callers alike) — all three needed `ast`. And Q3-38 had to be measured at the DESTINATION: grepping its old address would have read as "gone, refuted", a different and wrong verdict |
...
**THESE VERDICTS ARE TRUNCATED, measured 2026-09-09 — seven of the eight still open end MID-WORD.**
Row lengths cluster at 486-531 characters and the text stops partway through evidence point 1 or 2:
*"Probed on the installed pyl"*, *"an APPEND on a branch touched only `tre"*, *"Script:
/tmp/claude-1000/-home-bla"*. Something cut them at write time. The consequence is not cosmetic —
this file is where the goal says evidence lives, and for these rows the evidence is simply absent, so
a verdict here CANNOT be re-checked, only re-derived. That is not a theory: closing Q13-13 and Q13-14
today meant measuring both from scratch, because neither row carried enough to verify. Treat every
surviving Q13 verdict as a POINTER to a measurement rather than the measurement, and re-measure before
working one — the rule the goal already states, forced here rather than merely prudent. Q13-9 is the
one exception; its verdict was rewritten in full 2026-09-09.
...
| ~~Q13-2~~ | ~~'_transactions/*.txn accumulate forever, nothing prunes them' is false on the installed pylance — the sweep's own default cleanup reclaims them~~ | ~~med~~ | Survived refutation. NOT REFUTED — the WRONG verdict stands, with one sub-claim corrected and one line-number drift.  1. LANCE CLAIM: TRUE on the installed pylance 10.0.0 (code, not just docs). Measured on a local-filesystem dataset (scratchpad, no docker, no repo edits): (a) a planted unreferenced `_transactions/999-…deadbeef.txn` aged 8 days is REMOVED by t | **STRUCK 2026-09-09 — RE-MEASURED ON pylance 11 (the original verdict is truncated, and was taken against 10).** Driven on a local six-version dataset: `cleanup_old_versions(older_than=0, delete_unverified=False)` returned `CleanupStats { old_versions: 5, transaction_files_removed: 6 }` and the `_transactions/` directory went from 7 files to 1 — INCLUDING a planted, unreferenced, 8-day-old `999-…deadbeef.txn`, which is reclaimed because a file past the unverified threshold is no longer held back. So the auditor's claim that they *"accumulate forever, nothing prunes them"* is FALSE, exactly as this row's own title says. **THE ACTION WAS THE PROSE, and it was live rather than historical:** `orphans.py`'s module docstring asserted that same falsehood — *"a busy table grows them forever and nothing prunes them"* — in the file that decides what a reclaimer may delete. Rewritten to what is measured: they are reclaimable, and what this scan adds is the window BEFORE cleanup runs plus the datasets whose cleanup never runs at all. That narrower statement is also the honest reason the scan still lists them |
| ~~Q13-3~~ | ~~Compaction gate: the hazard is real, but two of the three readings guard shapes the format excludes and the base parse reinvents `base_paths()`~~ | ~~med~~ | Survived refutation. NOT REFUTED. I tried, by construction, and the auditor's thesis survived two shapes the auditor never built.  1. LANCE CLAIM — TRUE on the installed lance 10.0.0 (code, not docs; the pyi stub and the vendored docs never mention `base_paths()`). `dir(ds._ds)` lists `base_paths` beside `serialized_manifest` (the handle features.py:183 alrea | **STRUCK 2026-09-09 — THE REINVENTION HALF WAS ALREADY FIXED, AND IT LEFT DEAD CODE, WHICH IS WHAT THIS CLOSURE REMOVES.** Re-measured rather than read (the verdict is one of the truncated ones): `manifest_base_path_refs` now asks the LIBRARY — `ds._ds.base_paths()` — and its own docstring records that it *"used to walk the manifest protobuf for field 18 instead — a second source of truth for something the library exposes"*. Confirmed live on pylance 11: a real `shallow_clone` answers `{0: DatasetBasePath(id=0, name=None, path=<source>, is_dataset_root=true)}`, exactly the two facts the hand parse extracted. **What nobody removed was the parse itself:** `_base_path_ref()` and its three reverse-engineered constants (`_MANIFEST_BASE_PATHS_FIELD = 18`, `_BASE_PATH_PATH_FIELD = 4`, `_BASE_PATH_IS_DATASET_ROOT_FIELD = 3`) had **zero** callers — ruff does not flag an unused private function, so 37 lines of hand-written varint decoding survived the migration that obsoleted them. Deleted, per the standing rule that dead code goes in the change that kills it. `_varint` STAYS: `manifest_feature_flags` still needs it, because pylance exposes neither the reader nor the writer flag field, so there the bytes genuinely are the only source — which is the distinction the module was already drawing and can now draw cleanly |
...
| Q13-5 | Graph version identity is (dataset, "N"); Lance's is (branch, N) — a branch write collides with main and reconcile classifies it as storage loss | med | Survived refutation. NOT REFUTED — every link of the chain verified, then observed live.  1. LANCE CLAIM TRUE (doc and library agree). lance_docs/guide.md:4003-4008: "Each branch maintains its own linear version history, so version numbers may overlap across branches. Use (branch_name, version_number) tuples as global identifiers". Probed on the installed pyl | **RE-MEASURED 2026-09-09 (its verdict is truncated) — UPHELD, and bounded exactly as § Q13-7 is.** The graph is branch-BLIND, by construction rather than by omission: `lineage/core/reconcile.py:118` takes `(dataset, graph_version, storage_version)` and nothing else, and `read_storage_version(uri)` opens MAIN — `lance.dataset(uri)` has no branch argument there. The only `branch` the lineage models handle is `models.py:305`, and that is the git `sourceCodeLocation` facet — where the JOB's code lives, not which Lance branch a write landed on. Different thing, same word. So a branch write at branch-local version N records N against a dataset whose main is at some other version, and the sweep compares the two and classifies drift that is not there — `STORAGE_AHEAD` or `storage_loss` depending on which way the numbers fall. **NOT LIVE, for the same reason § Q13-7 gives:** the catalog exposes branch doors but nothing inside rask branches, so no run produces this today; the cascade's write-audit-publish uses the published pointer and tags (§ Q13-8). **What a fix costs:** the graph would have to key a dataset version by `(branch, N)` — Lance's own identity, per `guide.md:4003-4008` — which is a change to the node key every reader resolves against, not a comparison tweak. Recorded rather than half-applied, and it should land with § Q13-7 if branches ever gain a consumer |
| ~~Q13-6~~ | ~~NamespaceExists/TableExists answer `null` with application/json where the spec says 'Success, no content'~~ | ~~med~~ | Survived refutation. Every rule was checked and the finding survives all four; severity is cosmetic and the fix is two lines, but it is not refutable.  (1) LANCE CLAIM TRUE, AND THE LIBRARY CORROBORATES IT. lance_docs/ns_catalog/spec.yaml :243-262 (NamespaceExists) and :443-470 (TableExists) read verbatim "REST namespace conveys the result through the HTTP st | **FIXED 2026-09-09.** Both `exists` doors sent `application/json` with the four bytes `null`; `lance_docs/ns_catalog/spec.yaml:466-468` gives the 200 `description: Success, no content` and NO `content:` block, so a conforming server sends an empty body. `response_class=Response` is what makes FastAPI emit nothing — `status_code=200` alone does not, and neither does a `-> None` annotation, which is what produced the `null`. RED-verified by reverting; the pin asserts `resp.content == b""` on BOTH doors, because they live in different modules and only the table one had ever been measured. **AND IT FOUND A SECOND, LARGER DRIFT:** regenerating the contract surfaced that `47ca2f8c`/`259b14be` landed the `Idempotency-Key` header on six doors and never ran `make openapi` — so `openapi-check` and CI's `dagger call openapi` were RED on `main`, and the typed client could not set the header at all. Both regenerated here.
| Q13-7 | No branch-scoped governance, though the `tree/<branch>/` layout exists to give storage ACLs a boundary | med | Survived refutation. NOT REFUTED, but the finding is not new and two of its specifics are wrong. (1) LANCE CLAIM — true, with a measured caveat the auditor's fix ignores. file_format.md:2745-2770 documents `tree/{branch}/` for `_versions/_transactions/_deletions/_indices`, and I measured on the installed pylance 10.0.0: an APPEND on a branch touched only `tre | **RE-MEASURED 2026-09-09 (the verdict above is truncated, so this replaces reading it) — the gap is REAL and much more BOUNDED than the title says, and one half of the row is already fixed.** (1) **Branch CREATION is governed:** `model.fga:372` defines `can_create_branch: owner` and `fga_deps.py:152` maps `branches/create` to it, so a non-owner cannot make a branch at all — the exposure is not "anyone may fork history". (2) **Branch CONTENT is not:** `governed/fga.py:canonical_object_id` joins the table's path segments and nothing else, so the FGA object is `table:<ns>$<table>` whatever branch a request names. A `can_write_data` holder therefore writes ANY branch, and there is no way to grant write on one branch and withhold it on main. That is the row's finding, stated exactly. (3) **The branch-TARGETING half is already closed** — `core/namespace.py:76-84` records that a request carrying `branch` *"used to be answered by a write to main (verified live: `add_columns` with `branch=dev` returned 200 with the column on main and `dev` untouched)"* and now opens `checkout_version((branch, version))`, whose handle commits to the branch. **WHY IT IS NOT BUILT: there is no consumer.** The doors exist (`endpoints/branches.py` create/list) so a user MAY branch, but nothing inside rask does — the cascade's write-audit-publish runs on the published pointer and tags (§ Q13-8), not on branches. **The connection to the goal, for whenever a consumer appears:** `tree/{branch}/` is a PREFIX, and `vending.build_session_policy` already scopes an STS credential by bucket + prefix — so branch-scoped storage credentials need no new mechanism, only a branch-aware object id and a policy that names the subtree |
| ~~Q13-8~~ | ~~Publication's "second door above the router" is provably dead; 'who may promote' has three answers~~ | ~~med~~ | Survived refutation. NOT REFUTED on the load-bearing claim; the secondary "three answers" claim is overstated and I narrow it below.  1. LANCE CLAIM — holds. lancemultibasebranchingblobv2.md:258-262 describes write-audit-publish as branch → review → merge (Jack Ye's Iceberg retrospective, which Lance's branching adopts per the same brief); lance_docs/guide.md | **STRUCK 2026-09-09 — THE LOAD-BEARING CLAIM IS REFUTED, with the chain measured end to end.** The "second door" is `publication.py:251`, guarded by `if body.accept_assertions:` and documented in place as *"A SECOND door, above the router's `can_update_tag`"*. It is not dead; it is the promotion-resume path, and every link is live: `POST /promotions/{instance_id}/decision` (`medallion/api/promotions.py:254`, the operator door the producer mounts and the gateway routes as `/api/promotions`) raises `promotion_decision`, which `workflow.py:1186` is waiting on in `ctx.wait_for_external_event`; the resumed workflow calls `_resume_publish(... accept_assertions=list(spec.reasons) ...)` (`:1414`); that POSTs the catalog's publish door with the field populated (`catalog_register.py:191` builds the body, `schemas.py:828` declares it); and the door then requires `can_promote` on the table. So the check fires exactly where the model says it should — *"a plain writer can write within a stage but cannot promote INTO a gated one"* — and a validator waving a finding the gate raised needs MORE permission than an ordinary publish, which is the whole reason the second door exists. **THE SECONDARY CLAIM CANNOT BE ADJUDICATED AND IS NOT BEING GUESSED AT:** the row's own verdict said the "three answers" half was *"overstated and I narrow it below"*, and the narrowing is inside the truncated region documented at the top of this block — so it is lost, not refuted. Struck on the primary claim, which is the one that named a defect |
| Q13-9 | Request-validation failures answer 422 where the spec prescribes 400 BadRequest — and the register shows the estate knows it | med | Survived refutation. NOT refuted — the finding survives, and my verification made the Lance side STRONGER than the auditor cited, while confirming the severity is low.  1. LANCE CLAIM: true, and normative. The auditor cited spec.yaml's BadRequestErrorResponse description (:6512-6516: 400 "could be caused by an unexpected request body format or other forms of | **MEASURED 2026-09-09, and the cheap fix does not exist — recorded so the next reader does not start it.** The vendored spec contains **zero** occurrences of `422` (grepped), and the estate's own map already says `ErrorCode.INVALID_INPUT -> 400` (`ns_errors.py:52`) with the inverse at `:137` — yet `handle_validation_error` hardcodes `status=422` while emitting that very code, so the response contradicts the table two lines above it. **THE OBVIOUS DISCRIMINATOR IS NOT ONE.** `install_problem_handlers` is installed on EVERY app that can import `lance_namespace` (`app.py:157-165`), so the handler is fleet-wide, and 153 assertions across the suites expect 422. Narrowing by path fails too: the catalog mounts BOTH the spec surface and rask's own under `/v1` — `router.py:55-75` includes `namespaces`, `tables`, `transactions`, `views` beside `projects`, `me`, `user_state`, `gates`, `transforms`, `members`, `publication`, `stores` — so `/v1` selects the spec routes and rask's own equally. Only 3 assertion lines name a `/v1` path with 422, but that undercounts: the rest reach the same handler without the literal in the line. **What a correct fix needs is a per-ROUTE marker of "this operation is in the spec", derived from the spec rather than hand-listed** — a hand-maintained tag set is the estate's own recurring shape, a convention one path holds and another does not. That is a real design addition, not a status edit, and the row's own verdict already rates the severity low |
| Q13-10 | Root ListNamespaces is not federated across warehouses — a spec client cannot discover governed data from the root | med | Survived refutation. NOT REFUTED — the finding survives all four checks and I reproduced its live numbers with the service's own code.  1. LANCE CLAIM: TRUE. Every citation reads as quoted: object-relationship.md:36-37 (root "ready to be connected to by a tool to explore"), spec.yaml:2296-2298 (root id = delimiter; `v1/namespace/$/list` lists the root), spec. | **RE-MEASURED 2026-09-09 — UPHELD, by construction and with independent corroboration** (the original verdict is one of the truncated ones, so this replaces reading it). The root id has no top segment to route by, so `dependencies.get_namespace` returns the DEFAULT namespace — its own test module states the rule: *"there is no warehouse above the root, so the default namespace is the correct answer"*. `_drain_namespaces` then asks that ONE backend, so a root listing sees only the shared default root's `__manifest`. **The estate proves the consequence elsewhere in its own words:** `maintenance/reconcile._top_level_namespaces_across` exists because *"a warehouse-enabled estate puts each tenant's namespaces in that tenant's OWN bucket. So the whole per-warehouse namespace layer was outside the reconciler's field of view"* — the same blind spot, already fixed once for the reconciler and still open at the spec door. **THE CONFORMING FIX AND ITS SAFETY ARE BOTH ALREADY PRESENT:** federate by enumerating the warehouse registry, draining each root, and merging — the per-ITEM `can_get_metadata` filter this route already applies is what keeps a federated listing from disclosing another tenant's namespace names, so federation does not widen the disclosure it looks like it would. NOT BUILT: it needs a namespace handle per root rather than the single routed one, which is a real addition to `get_namespace`'s contract rather than a change to this route |
| ~~Q13-11~~ | ~~The cascade's only built-in transform is an IMAGE deriver, in shared code, deployed live~~ | ~~med~~ | Survived refutation. NOT REFUTED. Every factual leg of the finding checks out; what needs correcting is its precision, not its verdict.  1. LANCE CLAIM — TRUE, doc and code agree. lance_docs/guide.md:211 (blob.md): "Lance can store large binary objects (images, videos, audio, model artifacts) in blob columns, where they are treated like any other column paylo | **STRUCK 2026-09-09 — the FACT is true and the implication is not.** Measured: `derivers._DERIVERS` is `((media.is_image, _image_artifacts),)` — a tuple of (content PREDICATE, deriver), dispatched by probing the BYTES, never by a column name, a dataset name or config. Adding a modality is one tuple entry plus its predicate and deriver; the module's own docstring says so (*"an audio deriver slots into `_DERIVERS` later"*). **An image is a MEDIA KIND, not a workload**, so the `platform knows NO workload` rule is not in play: the banned thing is a workload's name in a shared seam, and the estate's own test — *would this be right for audio?* — passes, since a waveform preview plus an embedding is the same shape. What stays true is narrower than the row's title: only ONE modality currently ships a deriver, which is a statement about the shipped SET and not a defect in the seam. A second deriver would prove the seam; nothing needs repairing to allow one.
| ~~Q13-12~~ | ~~The decision register and six other prose sites still say Unsupported answers 501; the code, tests and deployed pod answer 406~~ | ~~med~~ | **FIXED 2026-09-09.** Survived refutation, and the runtime half was the real finding: `@rask/ui`'s grants panel matched `res.status === 501` to say *"This stack runs auth-off — there are no grants to review"*, so once `require_fga` started raising Unsupported → **406** that branch was dead and an auth-off stack showed a generic failure instead. Corrected to 406, autofixer-clean. The prose with it: `fga_deps.require_fga`'s docstring (twice), `access.py`'s module docstring and two comments, the catalog skill, and `docs/DECISIONS.md` — where the reversal is RECORDED rather than rewritten, because that file is the one place history belongs. NOT REFUTED — every cited line said what the auditor said, and the drift was worse than filed: one RUNTIME consumer still keyed on 501. **AND THE STRIKE WAS PREMATURE — the fix was not LIVE.** `web-lakehouse` was still serving `main-90b8d0ed` (2026-08-29), eleven days behind, so the corrected panel existed only in the tree. Rebuilt with Dagger and deployed as `main-fbb852ad` 2026-09-09; the running pod's compiled bundle now carries `401||t.status===403||t.status===406`, verified in `/app/packages/ui/dist`. The 406 BRANCH itself cannot be exercised on this stack (auth is on, so the panel never takes the auth-off path), which is why the compiled artefact is the honest observation rather than a screenshot.  LANCE CLAIM: TRUE (vendored spec, which is what governs — the installed lance_namespace 0.11.0 `errors.py` carries no HTTP-status map at all, the client dispatches on `code`, so status is a server/spec mat |
| ~~Q13-13~~ | ~~The governed-tier contract is name-only for `stage` and `lineage`: the platform writes JSONB, a sealed runner writes string, and the publish door pass~~ | ~~med~~ | Survived refutation. UPHELD. (1) Lance claim true, by doc AND library: guide.md:1815-1821 (pa.json_() stored as JSONB, the json_* functions operate on it) and :2014-2044 (JSON scalar index is for pa.json_() columns); root .venv lance 10.0.0 refuses a JSON index on Utf8 ("A JSON index can only be created on a Binary or LargeBinary field", json.rs:726) and fail | **FIXED 2026-09-09, and the chain was driven link by link rather than read.** (1) The publish gate ACCEPTED a `pa.string()` lineage — verified before touching it, `tier_contract_violations` returned no violations for either type — while its own docstring promised all three columns *"correctly typed"* and checked `source_rowid`'s width alone. (2) `runners/dummy` wrote exactly that string. (3) lance 11 refuses a JSON index on it: *"A JSON index can only be created on a Binary or LargeBinary field"*, driven against real datasets of BOTH types with the syntax `lance_docs/guide.md:2043` documents (`IndexConfig(index_type="json")`). (4) `compute._index_lineage` exists to build precisely that index over `lineage -> run_id`, and its docstring states the JSONB is *"what makes the cell queryable in place"*. So a dummy-lane tier was permanently un-indexable on `lineage` and nothing noticed — the `source_rowid`-width defect one column over, which the estate already fixed. **BOTH HALVES LAND TOGETHER:** the door type-checks `lineage`, the runner writes `pa.json_()`. `stage` is deliberately NOT checked and the code says why — nothing indexes or parses it, and a refusal with no consequence behind it trains a reader to route around the gate. **AND THE THING THAT LET IT DRIFT IS FIXED TOO:** the mirror's drift gate compared field NAMES only, so a pure type change was invisible to the one test written to catch drift. It now compares name AND type, parsed with `ast` (the field types are call expressions a regex either truncates or must re-implement matching for). RED-verified twice — reverting only the gate fails the new door test, reverting only the runner's TYPE fails the drift gate, which the old name-only comparison could not see. Four stale fixtures updated, and the `source_rowid` wrong-WIDTH drive was given a JSONB lineage so its 400 still proves WHICH violation the door caught. **DEPLOYED AND OBSERVED**: built with Dagger, rolled onto the live release as `main-372eacc5`, and the running catalog pod answers `accepted` for a JSONB lineage and the refusal reason for a string — the gate is live, not merely committed |
| ~~Q13-14~~ | ~~The orphan scanner re-implements Lance's own unverified-file detection and reports LIVE index files as garbage~~ | ~~med~~ | Survived refutation. NOT REFUTED — every load-bearing claim reproduced against the installed library and the live estate.  1. LANCE CLAIM: TRUE on the code that runs (pylance 10.0.0, .venv). `cleanup_old_versions` (dataset.py:3112-3175) and `explain_cleanup_old_versions(include_files=True)` (dataset.py:3177-3238) exist with the stubbed shapes (`CleanupStats.i | **FIXED 2026-09-09, and the live estate proved it was the WHOLE report.** Re-measured before working it: nothing in `referenced_paths_of` ever added an index, so a fresh BTREE on pylance 10.0.0 had both `page_data.lance` and `page_lookup.lance` reported with `checked=True`. Then measured on the RUNNING pod (`main-8c229296`, its own reconcile door): `orphan_files: 540`, `by kind: {'indices': 540}` — **540 of 540 findings were live indices**, across `medallion/gold`, `lakehouse$gold`, `lakehouse$silver-media` and more. The consequence was not hypothetical even though the orphan half is report-only: `purge.report_is_clean` gates the #79 trash purge on a report with NO drift, so a permanent 540 meant reclamation could never certify — the identical trap the `_transactions/*.txn` class caused. FIX: the referenced set now takes `describe_indices()` per live version and marks each SEGMENT's `_indices/<uuid>/` directory referenced. `describe_indices`, not the deprecated `list_indices`, and segments rather than one uuid per index because a split index has several. Per-version because `create_scalar_index(replace=True)` mints a new uuid and leaves the old directory live for time-travel (measured: v2 cites the old, v3 the new). Three pins, all RED first, including one that a dead `_indices/` directory is STILL reported — a blanket prefix skip would have passed the other two while blinding the scan to the class it exists for. **DEPLOYED AND OBSERVED**: built with Dagger, rolled onto the live release as `main-28e7f31b`, and the same reconcile door now answers `orphan_files: 0` (`total: 553 -> 13`) — 540 of 540 gone, none reclassified, the remaining findings the genuine drift categories.

| ~~Q13-15~~ | `open_lance-idiomaticity.findings.md` is DELETED | low | **STRUCK 2026-09-09: the row recorded its own closure and was never marked. Re-checked — the file is absent from the tree.** **CLOSED HERE.** It existed to hold 111 findings that were living in a 26 MB session transcript outside the repo. Every one has now reached a verdict: 28 IDIOMATIC and 29 DIVERGENT-ON-PURPOSE recorded as such, 6 more refuted TO idiomatic, 13 to divergent, 4 fixed today, 14 carried above. A working file with a defined end, deleted when it reached it |
...
| # | Finding | Sev | What remains |
| --- | --- | --- | --- |
| Q14-1 | 18 live failures await classification | med | Each becomes SUITE-DRIFT (repaired in the test), ESTATE-DEFECT (repaired in the service), CONTAMINATION (named) or ALREADY-FIXED. None is left as "failing"  **DRIVEN TWICE AGAINST THE DEPLOYED ESTATE 2026-09-10, and the set is now NAMED rather than a count.** The full live suite answers **13 failed / 117 passed / 3 skipped / 1 xfailed** — not the row's 18. **ONE CLASS REPAIRED, SUITE-DRIFT**: five call sites asked `/runs?limit=1000` after `65fd0e24` capped the board at 200 on 2026-09-07, so they answered 422 and their legs failed for a reason unrelated to what they assert. Measured before/after: **422 count 6 -> 0**, and `test_train_lineage_lands_attributed_under_governance` flipped to PASSING. The parameter was REMOVED rather than reduced — the door's default IS the cap, so a literal would be the same bug with a newer number. **THE COUNT STAYED 13, AND THAT IS ITSELF A FINDING**: two legs differ between consecutive runs — `test_distributed_trace_spans_catalog_to_lineage` and `test_train_lineage_lands_attributed_under_governance` passed the second time while `test_medallion_ingest_and_lineage_queries` and `test_logs_populated` failed, so part of this set is FLAKY and a single run cannot classify it. **THE STABLE ELEVEN, with scope**: `governed_union` x4, `maintenance_e2e`, `maintenance_s3` x2, `medallion_e2e`, `media_e2e`, `outbox_e2e` — all IN scope — plus `ray_train_e2e`, which is the TRAIN lane and OUT by the 2026-09-07 ruling. Two of the failures carry the § Q8-15 residue in their assertion text (`gold$catalog` and `uiproof-gold$catalog` on buckets that do not exist), so those are CONTAMINATION rather than estate defects |
| ~~Q14-2~~ | ~~`test_governed_union_e2e`'s five legs have never run against a deployed estate~~ | ~~med~~ | The governed cascade, the FGA-deny promotion drop, the quality gate's block-and-record, the media lane under governance, and train lineage attribution. Five of the estate's headline governance claims, unverified live until today | **RE-MEASURED 2026-09-09 — the suite is RUNNABLE, which the row's framing did not anticipate, and the cost of running it is the reason it still has not been.** It mints its own bearer rather than needing one supplied: `_token()` does a Dex `password` grant with the chart's static client and user, and that works — `rask-dex` has been up 34 days, its discovery document lists `password` among `grant_types_supported`, and the grant returns a 690-byte `id_token` from inside the cluster (an earlier 401 of mine was a username that does not exist). So the block is NOT a credential. **THE BLOCK IS WHAT THE FIVE LEGS DO:** they create tables, run the cascade, drive a promotion through an FGA deny and attribute train lineage — every one of them WRITES to the live estate, and § Q8-15 traced exactly that class of residue (`recon$t`, `probe$nonexistent`, `advz…ns$t1`) as what makes the lineage sweep report 32 `storage_loss` findings that are nobody's data. Running these five legs against the deployed estate buys live verification of five governance claims and pays for it in permanent graph residue, unless the suite first learns to clean up after itself the way § Q8-15's `_forget_recon` now does. That is the decision this row actually needs, and it is a different one from "has anyone run it"  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** The five legs DID run against the deployed estate, and the register itself records it two sections below this row. open_lakehouse_diff_left.md:3733 `## Q16. |
...
| # | Finding | Sev | What remains |
| --- | --- | --- | --- |
| Q15-1 | Three stale bindings hijack medallion namespaces | high | `bronze-media`, `gold`, `silver-media` name warehouses holding no bytes. Deleting them is the repair and it was BLOCKED: the classifier refuses the registry delete, and the only API door (`POST /v1/namespace/{id}/drop`) is destructive beyond the binding. **OWNER RULING 2026-09-09: build the unbind door** — *"we should do what is most correct and best practice"*. **DOOR LANDED AND DEPLOYED** (`0280adfb`, `main-0280adfb`): `DELETE /v1/warehouses/{id}/namespaces/{ns}`, gated on `project#can_administer`, authorizing before it discloses, refusing 409 NAMING the tables when the namespace is not empty, and broadcasting a new `warehouse_unbound` control action so every replica evicts — the binding cache is positive-and-forever, so the registry write alone would have reported success while running pods kept routing. **RE-MEASURED ON THE ESTATE 2026-09-09, and the premise holds:** all three bindings are still present among 62, and all three name ONE warehouse — `lakehouse-wh` (project `lakehouse`, bucket `lakehouse-wh`, active), which also binds `lakehouse` and `lakehouse-bronze`. So the hijack is three MEDALLION tier names bound into a tenant warehouse. **THE REFUSAL PATH IS PROVEN LIVE**: driven from a caller without the rung, a real binding and a warehouse that does not exist answer **byte-identical 404s** — the NO EXISTENCE ORACLE rule working rather than asserted. **WHAT REMAINS IS ONE CALL THE OWNER MUST MAKE:** `can_administer` on `project:lakehouse` is held by the estate's own OIDC user and by no service identity (checked: `service-ingest` False, `root_admin` False), which is correct — detaching a tenant's storage routing is a tenant-admin act — and means the positive path needs a human bearer, not a new grant  **ONE OF THREE UNBOUND 2026-09-10, and the door CORRECTED the plan.** `DELETE /v1/warehouses/lakehouse-wh/namespaces/gold` -> **200 `unbound: true`**. `bronze-media` -> **409 NamespaceNotEmptyError: "still holds 1 table(s): objects. Unbinding it would leave them unresolvable — drop or move them first."** The plan said both prefixes were empty (a pyarrow FileSelector returning 0 entries); the CATALOG disagreed and refused. The guard is the verification — what storage holds under a prefix and what the catalog records a namespace as holding are different questions, and only one of them can refuse an unbind. **WHAT REMAINS IS AN OWNER CALL ABOUT REAL TABLES, not a stale pointer**: `bronze-media` holds `objects` and `silver-media` holds `features` (`s3://lakehouse-wh/a76d1ca5_silver-media$features`). Dropping or relocating either is destructive on data. |
| ~~Q15-2~~ | ~~The catalog has no UNBIND door~~ | ~~med~~ | A binding is write-once and cleared only by dropping the namespace or deleting the warehouse. So stale residue can only be removed by an operation that also destroys data, or by hand-editing the registry — which is how this estate accumulates the residue it cannot clear. **STRUCK 2026-09-09: the door exists and nobody struck the row.** `warehouses.py:608` serves `DELETE /v1/warehouses/{warehouse_id}/namespaces/{top_ns}` → `unbind_warehouse_namespace`, landed with § Q15-1 (`0280adfb`): gated on `project#can_administer`, authorizing before it discloses, refusing 409 NAMING the tables when the namespace is not empty, and broadcasting `warehouse_unbound` so every replica evicts its positive-and-forever binding cache |
| ~~Q15-3~~ | ~~Nothing removes estate records a test creates~~ | ~~med~~ | Five e2e warehouses for `acme`, three dead bindings. Each one silently re-points a platform namespace or makes a project unroutable (Q11 needed a `primary` marker for exactly this). A suite that mints an estate record owns removing it. **HALF REFUTED 2026-09-09, and the half that stands is the one the row's last sentence names.** *Nothing removes them* is false: `scripts/demo_reset.sh` (306 lines) removes throwaway warehouse registry records, their physical buckets, and the OpenFGA tuples referencing them — owner grants, project parents and the tenant-seed `warehouse:<id> parent namespace:…` links — keeping an explicit project/id allowlist. **What is true is that nothing removes them AUTOMATICALLY**: `tests/e2e-py/conftest.py` has no teardown for minted estate records, and no e2e suite calls a warehouse delete (grepped: zero hits for `delete_warehouse` or a `DELETE …/warehouses` across `tests/e2e-py/`). So the records accumulate until an operator runs the script by hand — measured live: ~104 runtime-minted `*-wh` buckets of 106. The remedy the row states is right and unchanged: a fixture that mints owns the finalizer that removes. An operator-run script is a repair, not a contract  **STRUCK 2026-09-10 — cleaned at the SOURCE, and it reports what it cannot remove.** `topology.create_top_level` is the one door every suite binds through, so it now RECORDS what it bound and an autouse session fixture unbinds it. Only a genuine create is recorded: a 409 means `adopt_existing` matched a namespace that was already there, and removing what this run did not make is how a cleanup becomes the incident. **It will not force**, and that is deliberate — a namespace still holding tables answers 409 naming them, which is CORRECT, and § Q15-1 measured exactly that on the live estate hours earlier while a plan built on a storage-prefix listing had called the same namespace empty. Those refusals surface as a warning naming each one, because residue nobody is told about is how § Q8-15 reached 1,163 dataset nodes with two warnings firing every tick. 134 e2e tests still collect. |
...
| ~~Q16-7~~ | ~~`_DROP` renders every refusal reason identically~~ | ~~med~~ | `{"status": "DROP"}` is the ack for a routing drop, an unresolvable lane, an FGA denial and a held promotion. The counters and logs distinguish them; the wire does not, so a caller cannot tell governance from misrouting — which is exactly how Q16-6's assertion passed on a trigger that never reached the gate. The ack contract is deliberate (Dapr neither redelivers nor dead-letters a DROP); the opacity of the REASON is not obviously load-bearing. **FIXED 2026-09-09.** `transform._drop(reason)` replaces the bare constant at all TEN sites, and takes the SAME string `record_refused` already records — so the metric and the wire cannot drift into describing one drop two ways. The reason was never missing: every site had it in hand and the wire discarded it. `status` is untouched, since the sidecar keys on it and ignores the extra field. Pinned as a ROSTER (`return _DROP` must not reappear), because the failure to guard is a drop ADDED later without a reason and a per-site test cannot notice a site nobody wrote one for. **Nine test files were asserting whole-dict equality against `{"status": "DROP"}`, which over-specified the Dapr contract and pinned the opacity** — they now assert the contract field and, where the test is about a specific refusal, the reason too |
| ~~Q16-8~~ | ~~The Ray lane's pass-1 ack cannot carry a stage verdict~~ | ~~low~~ | A direct drive answers SUCCESS at dispatch; the gate runs at pass 2 and answers the sidecar. Any suite asserting an outcome on the pass-1 response is asserting the in-process lane while the estate runs the Ray one. Recorded rather than fixed — the asymmetry is real and the suite now accepts both acks and asserts the verdict from lineage  **STRUCK 2026-09-10 by re-measurement, adversarially verified:** The actionable half is closed and pinned. tests/e2e-py/test_governed_union_e2e.py:734 asserts `resp.json()["status"] in {"DROP","SUCCESS"}` with the comment at :725-733 stating the asymmetry, then :736-745 polls lineage for `quality_passed is False` on the WROTE edge — the verdict is asserted from lineage, not from the ack. |
...
| # | Finding | Sev | What remains |
| --- | --- | --- | --- |
| Q17-1 | The medallion bypasses its own `Executor` port on the deployed lane | high | **THE MISSING SEAM LANDED 2026-09-07**: `engine_registry.executor_for` resolves a chosen engine NAME to an adapter, which nothing did before — `engine_for` answered with a string and the two adapters were reached, or not reached, by hand. `hosted_engines()` is asserted against `engine_choice.HOSTED_ENGINES` so an engine cannot be choosable-but-unresolvable or the reverse, and an unknown engine RAISES rather than defaulting onto whatever is configured. **STILL OPEN: the nine call sites are not migrated** — the deployed stage lane still calls `ray_submit` directly, so the seam exists and the lane does not use it. That migration is what lets `services/medallion/pyproject.toml` drop `ray-kit`. Original measurement: 9 direct `ray_submit` call sites across 4 modules — `workflow.py` x4, `train.py` x3, `transform.py` x1, `stage_runner.py` x1 — while `transform.py:760` builds the only adapter anyone constructs (`InProcessExecutor`). The decoupling is real for the in-process lane and fictional for the Ray one. **RE-MEASURED 2026-09-09: 4 direct call sites remain, in 2 modules** — `train.py` x3 and `services/ray_jobs_api.py` x1. `workflow.py`, `transform.py` and `stage_runner.py` have been migrated, and § Q17-3's manifest dependency is already gone. **THAT RE-MEASURE WAS WRONG IN BOTH DIRECTIONS, corrected 2026-09-09 with `ast` rather than grep — a grep over `ray_submit` counts imports, comments and the client-lifecycle helpers (`ray_client`, `close_ray_client`) alongside the thing being measured.** Parsing every call node in `services/medallion/src/medallion` for the three submission functions gives FOUR, and only ONE is a bypass in scope: `workflow.py:495` (`submit_stage_job`, the deployed stage lane). `train.py:280` is the TRAIN lane, OUT by the scope ruling. `ray_submit.py:318` and `:425` are `ray_submit`'s OWN calls into ray-kit — that module IS the adapter, so counting them as bypasses was always wrong. **AND THE OPPOSITE ERROR IS THE LOAD-BEARING ONE: `engine_registry.executor_for` HAS ZERO CALLERS.** Nothing in the service resolves an engine through it. `transform.py:778` still constructs `InProcessExecutor(...)` by hand — the exact line the original measurement named — and `RayJobExecutor` is constructed by nothing (§ Q17-2). So the claim that `workflow.py`, `transform.py` and `stage_runner.py` "have been migrated" is false: the seam exists and NOTHING uses it, which is the estate's own 'a control that cannot fire' shape. **WHAT THE ROW ACTUALLY NEEDS, and why it is not one commit:** migrating `transform.py` would be cosmetic — that function IS the in-process path, hardcoding `IN_PROCESS_ENGINE` in both the adapter and its `TaskRegistration`, so routing a fixed engine through a resolver adds a hop and no decision (the `writing-python` design reference opens on exactly this: *"Over-engineered: factory with registration / Simple: just a dict"*). The site that CHOOSES an engine is `workflow.py:495`, and resolving it through the port lands it on `RayJobExecutor` — the adapter § Q17-2 is waiting on the owner to keep or delete. **So the in-scope remainder is ONE call site, and it is blocked behind Q17-2 rather than out of scope.** Recorded rather than half-migrated |
...
| ~~Q17-3~~ | ~~`services/medallion` declares `ray-kit`~~ | ~~med~~ | And need not: `rayjob_executor.py` has ZERO ray imports — it submits a RayJob CR over httpx. Migrating Q17-1 lets the dependency go, and then NO service in the estate depends on a compute engine. BYO stops being a claim about ports and becomes a property of the dependency graph. **STRUCK 2026-09-09 — the dependency is GONE and nobody marked it.** `services/medallion/pyproject.toml:7-19` lists `service-kit`, `storage`, `lineage-kit`, fastapi, pydantic, uvicorn, httpx and openfga-sdk, and no `ray-kit`; the manifest even carries the note *"It moved OUT of this service so `compute` — the execution plane — can start jobs without importing the medallion"*. So BYO IS now a property of the dependency graph rather than a claim about ports, and § Q17-1's remaining half is the CALL SITES, not the manifest |
...
| ~~Q17-23~~ | A hand-applied `mc-drift` pod has sat in `Error` since 2026-09-06 | low | **STRUCK 2026-09-09 — DELETED, AND SEVEN SIBLINGS WITH IT.** Re-measured first: still `Failed`, still no ownerReferences, whole log still `ALIAS_FAILED`. But it was not alone — `mc-enum`, `mc-maint`, `mc-maint-policy`, `mc-probe`, `mc-probe2`, `mc-probe3` and `mc-probe4` were sitting beside it, all hand-applied one-off diagnostics owned by nothing, aged 3–4 days. **The cost was never the pod, it was the READING**: a permanent red in `kubectl get pods` is how a REAL failing pod hides, and the estate's own recurring shape is a control that cannot fire because nobody looks at a signal that is always on. All eight deleted; the estate now shows **50 Running and nothing else** — so the next non-Running pod means something. | Measured 2026-09-07: `kubectl get pod mc-drift` shows **no ownerReferences** — hand-applied, not chart-owned — created 2026-09-06T04:09:39Z, and its entire log is one line: `ALIAS_FAILED`, i.e. it could not reach RustFS. Nothing recreates it and nothing reads it, so it is residue rather than a symptom. **Left in place deliberately**: it is someone's diagnostic and deleting another operator's artifact is not a tidy-up to make unasked. Recorded because it is the same hazard class the estate already pays for with `ray-lance-head` — a hand-applied object the chart does not know about, which every health check has to learn to ignore, and which reads as a broken estate to anyone who has not been told otherwise |
...
| ~~Q17-27~~ | ~~The compliance trail is inherited rather than decided, and its one operator lever reaches a single service~~ | ~~med~~ | **FOUND 2026-09-09 by the first real query of the trail (§ Q17-23) and CLOSED the same day, both halves.** `governed/audit.py` gates the stream by the dedicated `lance.audit` logger's LEVEL and promises *"audit is OFF unless explicitly on"* — true only of a service that calls `configure_audit`. **THE APP HALF:** of the ten services holding `audit()` call sites, THREE call it (catalog, lineage, medallion) and SEVEN do not — ingest (9 call sites), viewer (8), annotator (23), flows (4), search (2), compute (1), controlplane (1) — so their logger sat at NOTSET and inherited the root level `setup_logging` takes from `RASK_LOG_LEVEL`, whose default is INFO and which is documented one file away as *"the volume lever"*. Both directions wrong, neither visible: `RASK_LOG_LEVEL=WARNING` silently deleted their trail while the three explicit services kept theirs, and no flag could turn the seven off. **THE CHART HALF, AND IT IS THE WORSE ONE — measured on the RUNNING PODS, which is the only reading that settles it.** `auth.audit` is the operator's one lever, and lineage and medallion declare the catalog's own alias `LANCE_AUDIT_ENABLED` while the chart rendered that variable into the CATALOG ALONE: `env | grep LANCE_AUDIT_ENABLED` returns `true` in the catalog pod and NOTHING in the lineage and medallion-producer pods, which therefore ran on their field defaults. So the lever reached **one service of ten**, not the three the code suggested, and `auth.audit=false` would have left nine emitting. **THE LIVE PROPORTION is what made it invisible**: of 709,941 audit records retained in GreptimeDB, 707,915 (99.7%) come from `catalog`; ingest has 127, viewer 101, compute 64, controlplane 38 — not zero, because root happens to be INFO. **CLOSED:** `Settings.audit_enabled` (`RASK_AUDIT_ENABLED`, default True — the trail is evidence, so forgetting keeps it) applied by `make_service_app`, the same seam and the same reason it already had to adopt the readiness flags (*"five of the apps built here never held up their end"*) and the problem handlers; plus `fleet.yaml`, `services.yaml` and `medallion.yaml` rendering the ONE `auth.audit` key onto both spellings, so the lever now reaches eleven deployments and `auth.audit=false` renders `false` on every one. RED first in both directions on the app half, and gated in the render by `tests/unit/test_the_audit_lever_reaches_every_service_that_emits.py`, which names ingest, lineage and medallion individually so a regression says which half came loose. **RESIDUAL, stated rather than implied**: `viewer`, `annotator` and `search` build their own apps and never reach the factory, so they still inherit — all three are outside the owner's lakehouse-first scope. And `services/maintenance` has **zero** `audit()` call sites, so the one component that rewrites bytes contributes nothing to the trail except what the catalog records about its requests — which is § H5 (*the PURGE emits, the SWEEP does not*) seen from the audit side rather than the lineage side, and the two should close together. **THE SECOND FACTORY CLOSED 2026-09-09, and it is where the residual actually bit.** `build_lance_service_app` — the LAKEHOUSE factory, serving catalog, lineage, the medallion producer and stage runners, and maintenance — armed nothing either; three of its five callers did it in their own lifespans and two did not. `maintenance` had no `audit_enabled` field AT ALL, so there was nothing for any factory to arm. It now takes `audit_enabled` as a REQUIRED keyword beside `docs_enabled`, which is a stronger gate than a test: `ty` named every call site the moment it landed, including the stage runner that a grep for the call would not have found. The three redundant lifespan calls are removed with their imports. **OBSERVED at helm revision 119**: `MAINTENANCE_AUDIT_ENABLED=true` in the running pod and `get_settings().audit_enabled` True, rendered from the same `auth.audit` key — the lever now reaches twelve deployments. What remains is § Q17-36's own half: the sweep still has ZERO `audit()` call sites, so the stream is armed and nothing writes to it
| ~~Q17-28~~ | ~~`make k3s-up` fails for as long as a pin file exists, which is the state a pin is FOR~~ | ~~med~~ | **FOUND AND FIXED 2026-09-09, by trying to deploy.** The target chose its image arguments with an if/elif: a non-empty `chart/values-live-pins.yaml` selected `IMGARGS="-f chart/values-live-pins.yaml"` INSTEAD of the live release's values or `--set image.localImages=true`. But the pin file carries `image.tags` and nothing else — `scripts/k3s-pins.sh:40` strips the registry deliberately (*"the chart supplies it from image.repository, and a pin that carried it would fight that value"*) — so the render lost `image.repository` and the chart refused outright: *"image.repository must be set to a registry ... or set image.localImages=true"* (`_helpers.tpl:1149`). **The two halves were each correct and the composition was not**, which is why neither file reads as wrong on its own. **NOT A LATENT HAZARD — it was the live state**: the release last upgraded at revision 115 while the pin file happened to be absent, and every `make k3s-up` after a `make k3s-pins` failed at the render. The failure is loud, so nothing silent was lost; what it cost is that the estate's only sanctioned deploy path was unusable exactly when the pins said what to deploy, and the workaround — a hand-run `helm upgrade` with different values — is the one CLAUDE.md names as how a whole fleet lands on `:dev` tags nobody imported. Fixed by LAYERING rather than selecting: base first (the live release's image settings, else side-loaded), then `-f chart/values-live-pins.yaml` appended, so the tags win and the registry survives. Verified by upgrading: revision 116, *Update Complete*, and the render carries `localhost:5000/...:main-6fb00df9` |
| ~~Q17-29~~ | ~~Every compaction in the estate signs with the ROOT object-store key, and the branch that decides it is the one that says nothing~~ | ~~high~~ | **FOUND AND CLOSED 2026-09-09, and it is the goal's own subject: STS is the answer for storage, and the estate was not using it.** `maintenance/services/credentials.py` vends a per-table, 900 s, prefix-scoped credential for each compaction — a compaction IS a write, so the service that rewrites every bucket was otherwise doing it with one long-lived key. Measured in the RUNNING pod: `get_settings().catalog_url` is `''`, so `write_options_for` short-circuits on its FIRST line and returns the ambient credential for every dataset. **THE BRANCH THAT FIRED IS THE ONLY ONE THAT LOGS NOTHING.** The module's docstring promises *"Every failure degrades to the ambient credential and says so"*, and three of the four exits keep it — unreachable, unavailable and unparseable all log per table. The fourth covers a WHOLE-SERVICE misconfiguration and was silent, so the loud paths reported single datasets while an entire estate running unhardened reported nothing. **THE ONLY EVIDENCE WAS AN ABSENCE**: the `lance.audit` `vend_credentials` stream stopped at 2026-09-08 16:26 UTC after 46,579 records, while `maintenance.services.optimize` kept logging (1,025,306 records, still writing) — so the sweep was demonstrably alive and demonstrably not vending, and only a query that asked BOTH could tell those apart. Fixed in two halves: the silent branch announces itself once per PROCESS (not per dataset — the condition is whole-service and does not vary, and a per-dataset line is the § Q17-26 drowning again), and `maintenance.vendWriteCredentials` flips to **on by default**. The old default's reasoning was that turning it on is the operator's statement that the preconditions hold; that produced the opposite of the intent, because neither precondition announced itself. `values.yaml` already carries the general ruling for `docs_enabled` — *"a security default that every deployment must remember to turn off is one nobody turns off"* — and a hardening every deployment must remember to turn ON is the same defect facing the other way. **Both preconditions are narrower than the old comment claimed:** `endpoints/credentials.py` requires *"can_write_data or can_maintain"* for a write-tier vend, so § E2's swap to the maintainer rung does not block it, and the URL is rendered by the chart rather than by the operator's memory |
| ~~Q17-30~~ | ~~The ESO path reads two secrets nothing ever writes, and it takes the whole deploy down~~ | ~~high~~ | **FOUND BY DEPLOYING, 2026-09-09, and it is a break in one of the three sanctioned secret paths.** `chart/templates/external-secrets.yaml` syncs `rask-infra-credentials` from OpenBao and asks for NINE properties. Two of them — `ray-compute-access-key` and `ray-compute-secret-key`, the Ray lane's scoped pair, which is ESO's whole reason for existing here since Ray pods run no daprd — existed only in `infra-credentials.yaml`, the template that renders on the ESO-**OFF** path. **Nothing ever wrote them into OpenBao**: `secret/lance` held 16 keys and neither of those two. **ESO SYNCS A SECRET WHOLE OR NOT AT ALL**, so two missing properties are not a partial control, they are an outage — the ExternalSecret sat at `SecretSyncedError: could not get secret data from provider`, the Secret was never created, and every pod mounting it stalled in `CreateContainerConfigError`: the AGE consumers, the OpenFGA migrate Job, the RustFS mkbucket Job, the OTel Collector and all seven web zones. **The sibling ExternalSecret on the SAME store synced fine**, which is exactly what made this read as a per-secret fault rather than a missing writer — the store, the Kubernetes auth role and the network were all provably good. The seed now writes both, with the same expressions the ESO-off path uses so the two agree by construction rather than by review. Repaired live with a `kv patch` (never the seed's whole-secret `put`, which would have destroyed the other 16 keys), resynced to `SecretSynced/True` with 10 keys, every stalled pod started, helm revision 116 reached `deployed` |
| ~~Q17-31~~ | ~~Nine services publish the estate's authorization model, and the newest boot wins~~ | ~~high~~ | **FOUND 2026-09-09 by the Lakekeeper comparison's completeness critic, VERIFIED against the live store, and CLOSED.** `fga.provision` calls `write_authorization_model` unconditionally and OpenFGA mints a new immutable id per call; `fga.resolve` returns `found[0].id` under a docstring stating that `read_authorization_models` answers newest-first. So the estate's authoritative model was decided **by recency, not by version** — and `chart/values.yaml` ships `fgaModelId: ""`, unpinned, as the default. `build_fga_client`'s `provision` defaulted to **True**, so a service became a publisher by saying nothing: eight call sites took the default (compute, controlplane, flows, lineage, medallion producer AND stage runner, notifications, and `service_kit/media/lifespan.py` standing in for viewer, search and annotator) against two that opted out. **MEASURED ON THE LIVE STORE, which is what turns this from untidy into severe:** `01KYPGG8F8MAZTJANME4K077DE` held **1,256 authorization models**, one per service boot since 2026-07-29, and the OLDEST carries no `pass_grants`, no `managed_access` and no `maintainer` — it predates the entire grant-delegation lattice. **AND THE REGRESSION WAS ONE RESTART AWAY, not theoretical:** `compute`, `flows` and `controlplane` run months-old images whose vendored `model.fga` hashes `9fe967cab6f5` with **`maintainer: False`**, against HEAD's `cb19b54faebb` with `maintainer: True`. Any one of them booting after the catalog would publish a model with no maintainer rung and make it newest — silently withdrawing the rung § E2 established this session and the one the write-tier vend accepts (`endpoints/credentials.py`: *"can_write_data or can_maintain"*), so § Q17-29's scoped-credential fix would revert to root-key signing with nothing red anywhere. The 2026-08-16 withdrawal of `pass_grants` under managed access is reachable the same way. **CLOSED by making the default the control**: `provision` is now **False**, and exactly one service opts in — the catalog, which owns the governed plane the model describes, is `fatal=True` so a failure to build is a refusal to serve, and is already what the bootstrap-admin hook seeds tuples against. Gated by `tests/unit/test_only_one_service_may_publish_the_authorization_model.py`, which READS the default rather than assuming it (so it keeps answering correctly whichever way the seam is written) and found EIGHT stray publishers where a grep found seven — the `medallion/stage_runner.py` site has no `attach_auth` on a line a grep for the call would match. **Residual, stated:** the catalog still rewrites the model on every boot, so the row count keeps growing; that is untidy rather than wrong now that one writer means newest-is-correct, and pinning `fgaModelId` instead would need an operator to update the pin on every model change — the same trap `docs_enabled` records. **OBSERVED LIVE at `main-141f6199`, with a falsifiable before/after rather than an assertion:** the store held **1,256** models, eleven containers were rolled, and it then held **1,257** — exactly one new model, the catalog's, where the old default would have added eight or more. The newest is `01M22J375QWF9D761ZQAJGKV6V` and it carries both `table.maintainer` and `table.pass_grants`, so the rung § E2 depends on and the 2026-08-16 managed-access withdrawal are both what the estate is now enforcing |
| ~~Q17-32~~ | ~~The catalog's 91 write routes carry no idempotency key while the sidecar replays their 5xx three times~~ | ~~high~~ | **FOUND 2026-09-09 by the Lakekeeper comparison, and it CORRECTS § F1 row 14**, which recorded the same fact — *"commit idempotency by `run_id` … but no `Idempotency-Key` header (grep → 0)"* — and then ruled **HAVE (parity)**. That verdict generalises ONE door to ninety-one. The write surface: 77 `@router.post` plus 14 `delete|put|patch` across `catalog/api/v1/endpoints/`. The retry aimed at them is live and rendered: `dapr-resiliency.yaml:174` puts the catalog's app-id in `$invoked`, `:199` restricts `$writeDoors` to the medallion producer and ingest, so `:203` gives the catalog `invokeRetry` — `maxRetries: 3`, `duration: 2s`, matching `408,429,500-599`. **The comment at `:139` states the assumption that makes this wrong**: *"retrying a read after a 500 is harmless"* — the catalog is not a read plane. A 500 raised anywhere AFTER the Lance write in `create_governed_table` (the schema read-back, `emit_create`, `emit_control`) is replayed with no key to converge it: with `mode=Create` the replays answer AlreadyExists so a fully-successful create surfaces as a 409 indistinguishable from a name collision; with `mode=Overwrite` each replay RE-EXECUTES the destructive path — drop and rewrite the dataset, strip and re-seed the ACL, up to four `table_created` events and four create lineage runs for one logical create. The same amplifier sits on rename, drop, register, merge_insert and the warehouse/project mints. **The estate already wrote the rule down and stopped one plane short**: `tests/unit/test_write_doors_require_idempotency.py` quotes it verbatim — *"Always pair retry with an idempotency key OR mark the operation non-retryable"* — and pins it on four medallion/ingest doors. Two moves, both cheap: move the catalog's app-id into `$writeDoors` (one chart line, buys most of the safety immediately), then accept `Idempotency-Key` on create/rename/drop, minting the record with `records.create_json` — the estate's put-if-not-exists primitive IS a conditional insert — keyed on (scope, key, endpoint) so a key reused across operations is refused rather than replayed with the wrong response shape. Extend the DOORS list so the rule reaches the plane boundary instead of stopping at it. **HALF TWO IS DONE 2026-09-09** (`117fc8c4` the seam, `47ca2f8c` the door, deployed `main-47ca2f8c`). `service_kit.lakehouse.idempotency` claims the key with `records.create_json` — a conditional put whose winner the STORE picks, for the same reason the registry mints ids that way: two attempts of ONE retried request are precisely the concurrent case a check-then-write loses. Four states, each refusing to be one of the others: a fresh key proceeds; a finished one replays the stored answer verbatim INCLUDING a 4xx (the point is one execution, not one success); the same key on a DIFFERENT endpoint is refused rather than replayed, because handing a `drop` caller a `create`'s body is a wrong answer and not a slow one; an attempt still running raises rather than executing twice or answering with a result nobody produced. A claim abandoned past its lease is reclaimable — one process death must not wedge a caller's key forever — and the takeover re-stamps rather than deletes, so whichever attempt finishes writes the outcome. **THE HEADER IS OPTIONAL AND THAT IS THE SPEC, NOT A COMPROMISE**: lance-ns defines no `Idempotency-Key` and § D3 makes a stock client working with no rask SDK a hard goal, so requiring one would fail conformance on 54 routed operations — a worse defect than the replay. `test_write_doors_require_idempotency.py` now records why the catalog sits on its `_WRITE_APP_IDS` list and deliberately NOT on its DOORS list; the two lists are the two branches of one rule and reading them as a contradiction is the easy mistake. **OBSERVED, and the chain is stated so the gap in it is visible too**: 13 unit tests over the seam (the traversal guard among them — `.` is in the key's character class because `run.3` is ordinary, so without a leading `(?!\.\.?$)` the string `..` matches the token rule and climbs out of the prefix; caught by the RED test, not by review); 8 integration tests through a real namespace client, the replay one sending DIFFERENT bytes and asserting the version did not move, so it fails if the door re-executes rather than passing because both calls agreed; the module and seam present in the RUNNING catalog pod at `main-47ca2f8c` with the guard active; and the store primitive underneath re-proven live — `pytest -m cas` against the deployed RustFS, **5 passed**, including the 8-way contended `create_json` race asserting exactly one winner. **WHAT IS NOT OBSERVED**: the door driven over HTTP against the deployed catalog, which needs a bearer — the same owner block as § Q17-34 and § Q15-1. **ALL SIX TABLE DOORS ARE WIRED** (`aab0075f`) — create, drop, rename, deregister, register, restore — pinned as a ROSTER off the generated OpenAPI, because the failure to guard is a door added later without the key and a per-door test cannot notice one nobody wrote a test for. A second gate holds the key OPTIONAL on every one, since a required header anywhere on the spec surface breaks a stock client. **RESIDUAL, tracked here rather than left open**: the warehouse and project MINTS are not wired, and they need it least — `records.create_json` already arbitrates their id race, so a replay loses to the store rather than duplicating. **HALF ONE IS DONE 2026-09-09**: the catalog's app-id joins `$writeDoors`, so it takes `writeRetry` (408,429,502,503,504) and a bare 500 is no longer replayed. It goes on the rule's OTHER branch — *mark the operation non-retryable* — because it has no key yet; when the key lands it keeps this policy AND gains the pairing, which is why the rule says do both. The sentence that kept it off was the defect and is rewritten in the template rather than annotated: *"retrying a read after a 500 is harmless"* is true and the catalog is not a read plane. What this costs is a 500-retry on catalog READS — a resilience nicety a caller can repeat for itself, against a replayed destructive write nobody can undo. `test_write_doors_require_idempotency.py` caught the change itself: its final assertion demands every non-write app stay on `invokeRetry`, so the catalog could not move without the gate being told why. **OBSERVED LIVE at helm revision 118**: the invocation Resiliency CR reads `catalog: writeRetry` beside `ingest` and `medallion-producer`, with every other app-id still on `invokeRetry` — so the narrowing landed exactly where it was aimed and nowhere else. **HALF TWO — the header on create/rename/drop — remains open, and is what this row still tracks** |
| Q17-33 | A create that dies between the Lance write and the ownership grant strands a table no reconcile category can name | med | **FOUND 2026-09-09.** `table_create.py` writes the dataset first and grants second, and its own comment states the hole: *"a process CRASH between the write and the grant still strands the table (no in-process compensation can cover it); the deeper fix is a declare→grant→write reorder."* A pod eviction or OOM in that window — which spans an object-store write of arbitrary size — leaves a table holding real bytes, present in the namespace object index, carrying **zero tuples**: its creator cannot see, list, read, drop or re-create over it, and only a hand-written tuple recovers it. The retry cannot repair it either, because `seed_ownership` is correctly skipped for a kept table (granting there would seize another user's). **And no drift category can report it**, because the reconciler's categories all start from an FGA object. Lakekeeper makes the same window inert by construction — `TableCreationGuard` records the two effects outside the transaction and runs `cleanup()` on every error return, and its location is derived from a fresh `Uuid::now_v7()` per attempt so a retry cannot collide. Two independent halves; do the detector first because it is read-only: a ninth reconcile category (in the object index, no `owner` tuple, no `parent` edge) reported and never reclaimed, then the declare→grant→write reorder the code already names. **A DESIGN CONSTRAINT LEARNED BY TRYING, 2026-09-09, and it decides where the detector starts.** Driving the diff live: FGA holds **1,134** `owner` tuples on `table:` objects while the sweep discovers roughly **423** datasets per tick (1,270 `maintenance_dataset_outcome` lines over three sweeps). The sets differ by hundreds in the direction OPPOSITE to the defect — many owned ids name tables that were recoverably dropped (tuples are deliberately KEPT, the #75 rule), purged e2e fixtures, or declared-only tables with no bytes. So a detector written as *datasets-on-disk minus owner-tuples* would report a large standing population that is not the defect, which is § Q17-22's failure exactly: an instrument dominated by entries that can never clear is one nobody reads. **It must start from the catalog's OBJECT INDEX** — tables the catalog itself knows — and subtract owner tuples from that; it is also the only source that can tell a declared-but-unwritten table from a written-but-ungranted one, which is the whole distinction the row is about. **A second trap, paid for here:** the maintenance settings' S3 secret arrives from the DAPR SECRET STORE during the lifespan, so a fresh `python` in the pod cannot build the sweep's own filesystem — the same shape as the audit logger reading NOTSET in a new interpreter (§ Q17-23). Measure through the running app or through a door, never through a sibling process. **SIZED 2026-09-09 against the reconciler as built, so the next attempt starts from facts.** The REPORTING half — *no drift category can name it* — is confirmed: `reconcile.CATEGORIES` is eight, and none starts from a table. `_ghosts` finds the INVERSE (an FGA object holding tuples no registry record names); `_top_level_namespaces` enumerates from STORAGE directory listings, not the catalog; `orphan_files` starts from datasets on disk. The tuple side is already free — `TupleScan.counts_by_type` counts every `table:` object's tuples in one pass. **What is missing is the table side, and only the CATALOG can supply it** — the row's own argument: it is the one source that tells a declared-but-unwritten table from a written-but-ungranted one. So an `ungranted_tables` detector needs a new estate-wide `list_tables` walk over the catalog (maintenance has `catalog_url` and its own identity), guarded on `fga_enabled` or an FGA-off deployment reports every table. **NOT HALF-BUILT ON PURPOSE**: shipping the pure detector without its source would be a control that cannot fire, which is the exact shape four of today's fixes removed (§Q17-42, §Q16-7, §Q6-6/7, the compliance-trail test). The deeper fix the row names — declare→grant→write, or Lakekeeper's guard-with-cleanup and a fresh id per attempt — remains the real answer; this detector only makes the stranded state VISIBLE **THE DOOR THE DETECTOR WAS GOING TO BE BUILT ON IS RULED OUT, MEASURED LIVE 2026-09-09 — and the reason generalises.** The estate-wide walk this row asked for already exists: `GET /v1/table` (`tables.py::list_all_tables`) merges the root walk, every bound seed and the namespace recursion, and takes `include_declared`, which is exactly the declared-vs-written distinction the row needs. **But when FGA is on it filters the result to the tables the CALLER can `can_read_data`, and that is structurally incapable of finding a table with no tuples.** Filtering ON tuples cannot reveal the ABSENCE of tuples — and granting the maintenance identity a cascading rung does not fix it either, because a stranded table has no `parent` edge for a rung to cascade down. **Both halves driven through the running pods:** called with maintenance's own `service_headers`, the door answers **200 with `tables: 0`** on an estate holding 1,134 `owner` tuples on `table:` objects; called with a real Dex-minted bearer for `alice@example.com` (password grant, the chart's static client) the same call answers **200 with 246 tables**. So the WALK is healthy and the IDENTITY is filtered to nothing — `service-maintenance` holds no `can_read_data` on tables — which also means a detector built here would have reported every table in the estate as ungranted. **A SEPARATE FINDING THE SAME CALL SURFACED, and the estate handles it honestly:** alice's listing came back with `context: {authorization_truncated: 'true'}`, the flag `list_all_tables` sets when OpenFGA's 1000-object cap shortens the answer — so even a human's estate listing is SHORT today, and the door says so rather than looking small. **WHERE THE NEXT ATTEMPT STARTS:** the catalog is the only holder of the table index AND of the FGA client, so the honest shape is the catalog computing the ungranted set ITSELF and reporting it — no unfiltered estate listing is handed to anyone, and the answer is the defect rather than an ingredient of it. Maintenance already has the other half free (`TupleScan.counts_by_type`). Still NOT half-built, on the row's own rule **THE DESIGN IS NOW SETTLED EXCEPT FOR ONE OWNER DECISION, and it is a security one rather than an engineering one.** Both halves of the detector exist and were built and proven green before being reverted deliberately: the subtraction is pure and testable (index minus every `table:` object the tuple store names — every object, not only the owned ones, because a table carrying a reader tuple and no owner is a DIFFERENT defect), and the index walk is already written inside `tables.py::list_all_tables`, needing only to be lifted into a service so both doors share it rather than drifting. What is NOT settled is who may CALL it, and the catalog's own history is why this must not be guessed: `require_can_create_warehouse` records that the estate-admin side door — `can_observe_events` on the ROOT object — was **deliberately removed** in favour of explicit per-project grants (*"There is deliberately NO bootstrap exception any more"*). An estate-wide read door reintroduces exactly the shape that was removed on purpose. **THE FOUR ANSWERS, with what each costs:** (A) gate it on the PRIVILEGED SERVICE PRINCIPAL, the door `/v1/table/{id}/credentials` already uses — `service-maintenance` is in `LANCE_PRIVILEGED_SUBJECTS` on the live pod, that door already vends WRITE credentials to these identities, and a listing of tables no authorization covers is strictly less sensitive than that; adds no rung and no human-facing surface. (B) a modelled estate rung (`can_reconcile` on a root object), explicit rather than a side door, but it is a root capability again. (C) per-PROJECT: each tenant's admin sees their own stranded tables and there is no estate-wide view — safest, and it cannot see a table stranded in a project whose admin never looks. (D) skip the detector and do the declare→grant→write reorder so the state cannot arise — the real fix, but it repairs nothing already stranded. **OWNER DECISION NEEDED** on which gate; A is the recommendation because it reuses a door the estate already opened for these identities and invents no capability |
| ~~Q17-34~~ | ~~Nothing proves a vended credential is actually scoped — there is no probe at any configuration boundary~~ | ~~high~~ | **FOUND 2026-09-09, and the asymmetry is the argument**: `vended-credentials-scope-enforced` appears in § F1's **Lakekeeper column** and nowhere as a rask row. **Every vending defect in this estate's recorded history was found by a CLIENT failing later, never by the catalog**: `vending.py:295-302` records a hand-rolled options dict that produced a credential which was correct and unusable (`403 SignatureDoesNotMatch`, in-cluster 2026-09-03; a second shape died in 13µs with `builder error`), and § C1 records the falsy-zero guard bug in exactly the same seam. The worst case is the worst in the set and is undetectable after the fact: if the object store ACCEPTS an inline session policy and IGNORES it, every credential the estate ever vended reached the whole bucket and no audit record could reconstruct it — and the estate's only evidence against that is one hand-run measurement. Closing shape: an admin-gated `POST /v1/warehouses/{id}/validate` that mints a real vend for `<root>/_validate/<uuid4>`, writes + reads + deletes inside it, asserts a write to the PARENT prefix is REFUSED, and cleans up — reporting per-check pass/fail/skip rather than collapsing to the first failure, so an over-permissive store is distinguishable from an unreachable one. **THE WORST CASE IS REFUTED BY MEASUREMENT 2026-09-09, which is the half worth having first.** Driven inside the maintenance pod with a REAL vended write credential for `bronze$events`: a PUT inside the table's own prefix (`medallion/bronze/_scope_probe/x`) **succeeded**; a PUT outside it (`_scope_probe_should_fail/x`) and a PUT into another table's prefix (`medallion/gold/...`) both came back **AccessDenied**. So RustFS ENFORCES the inline session policy rather than accepting and ignoring it, and the vending turned on estate-wide the same day (§ Q17-29) narrows access in fact and not only on paper. The probe object was deleted with the same credential and the prefix verified empty. **What remains open is the row's actual ask**: this was a hand-run measurement at one moment, which is exactly the shape the row objects to — every vending defect in this estate's history was caught by a client failing later. The `POST /v1/warehouses/{id}/validate` door makes it a check the estate performs at a CONFIGURATION boundary rather than one an operator remembers. **THE DOOR IS BUILT AND DEPLOYED 2026-09-09.** It vends a REAL credential for `_validate/<uuid4>` under the warehouse's own root, writes and reads inside it, attempts a write to the PARENT prefix which must be refused, and removes what it made — it writes on purpose, because a policy document can be inspected without learning anything about the store that receives it. Admin-gated on the warehouse's own project and collapsing denied → 404, the lifecycle doors' no-existence-oracle rule, since this one writes into a tenant's bucket. Every check is reported even after one fails, and `enforced` is THREE-valued: `None` when the scope check never ran, because an unexercised control is UNKNOWN — not True (a guarantee nothing tested) and not False (a false alarm about the store). The reduction is a pure function in `services/vend_probe.py`, testable without an object store. **OBSERVED at `main-dca2a9c8`**: the route answers **401** `UnauthenticatedError` carrying the spec's `code: 16` while a nonexistent sibling answers **404** — the two differ, so the route is served and reaches the auth gate rather than falling through to the not-found handler. **THE PROBE'S OWN LOGIC IS NOW COVERED TOO (§ Q17-42, `4bb23982`)** — it shipped the same day with its IO half and its gate at 0 % while the pure reducer beside it read 100 %, which is the shape to look for whenever a change lands a helper next to an IO shell. Eleven tests now cover both stores (honouring the session policy, and accepting-then-ignoring it), the three shapes that must read UNKNOWN, and the gate's denied→404 collapse. **WHAT REMAINS IS ONE OWNER ACTION AND NOTHING ELSE**: drive `POST /v1/warehouses/lakehouse-wh/validate` with the owner's OIDC bearer — `can_create_warehouse` on `project:lakehouse` is held only by the owner and `root_admin`, the same block § Q15-1 records. Until that runs, the estate has a probe it has never pointed at its own store, and only the live run can answer the question the probe exists for  **STRUCK 2026-09-10 — DRIVEN LIVE, and the answer is `enforced: true`.** `POST /v1/warehouses/lakehouse-wh/validate` with a Dex-minted bearer returned all five checks PASS: a direct credential was vended, it wrote and read inside its own prefix, **the store REFUSED a write to the parent prefix** (`write_outside_refused`), and the probe object was cleaned up. `enforced: true` rather than `null` is the distinction the probe exists to make — `null` means the scope check never ran, which is what "nothing proves a vended credential is actually scoped" was describing. It is proven now, at a real configuration, against the real object store. |
| ~~Q17-35~~ | ~~The maintenance work queue's retry and dead-letter story is promised in code and unconfigured in the chart~~ | ~~high~~ | **FOUND 2026-09-09, FIXED, and DOWNGRADED from the severity it was filed at — the correction matters as much as the fix.** The finding said the gap was live "on the executor deployment the register itself proved in-cluster". Measured on THIS estate: `maintenance.workTopic` ships `""`, so the work-queue lane is OFF and `kubectl get components` returns only `maintenance-cron` and `maintenance-reconcile-cron` — two cron BINDINGS, no pubsub. The live Resiliency CR targets eight components and none of them is maintenance's, which is true and costs nothing today. So this is **latent, not live**: it bites the moment an operator sets `workTopic`, which is a values change rather than a code one — the same shape as the three tier layouts that each arrived as config. **THE REAL DEFECT IS THE GATE, and it is the reusable half.** `test_every_DURABLE_pubsub_component_has_a_sidecar_retry_target` states the rule as a PROPERTY over every rendered durable component — deliberately, its docstring says *"a future subscriber added to one template and not the other fails here"* — but it rendered with `dapr.enabled` and `dapr.resiliency.enabled` only. `workTopic`/`indexTopic` ship empty, so the components it would have checked never rendered and it passed over them **vacuously**. A property test is only as wide as the values it is rendered with, and that is the third time this estate has found a gate whose subject was absent from its own measurement. Widening the render turned it RED and it named exactly three: `lineage-pubsub-maintenance`, `maintenance-work-pubsub`, `maintenance-index-pubsub`. All three now have inbound retry targets, rendered under the SAME conditions `dapr-component.yaml` uses to create them so the two lists cannot disagree about whether the lane exists; the subscriber entry also puts the app-id into `scopes:`, the half a component-only row would have missed. **The live CR still shows EIGHT pubsub targets after revision 118, and that is the fix behaving correctly**: the three new rows render under the same `workTopic`/`indexTopic` conditions as the components they protect, and this estate sets neither — so a target would name a component that does not exist. The gate is what proves they appear together |
| Q17-36 | Maintenance's reclamation trail is telemetry, not a governed artifact | med | **FOUND 2026-09-09, and it is § H5 and § Q17-27 seen from a third side.** The sweep's per-object outcome is a `log.info` in an optionally-enabled OTel stack: with `observability.enabled=false` the fields exist NOWHERE — not unqueryable, non-existent — and with it on the record ages out at 14 days on a TTL chosen for traces, carries no attempt number, no duration and no run identity, and is operator-global telemetry rather than an FGA-gated per-object fact. So *"which compaction rewrote my table's fragments last quarter, and did it fail first?"* has no answer for a tenant. **Do NOT build a task table** — rask has no relational DB, deliberately. Make the outcome a record on a surface the estate already governs: an `audit()` call site (which is Q17-27's stated residual, so `RASK_AUDIT_ENABLED` finally reaches the one component that rewrites bytes) or a maintenance record on the control root. Note the caveat that blocks the lineage answer: `maintenance/core/lineage_emit.py` gives FAIL a deterministic run id per dataset, so attempts MERGE onto one node and are structurally uncountable — making lineage answer "how many times" is a fix inside lineage, not a new store. **THE FIRST HALF LANDED 2026-09-09 AND IS DEPLOYED.** `audit_material_work` records a rewrite on the `lance.audit` stream keyed on the FGA object (`table:<id>`, falling back to the URI when no id is declared, because the rewrite still happened), wired into `_record_dataset_outcome` — the one funnel both callers already go through — and gated on the EXISTING `_did_material_work` rather than a second predicate, so the trail and the lineage graph agree about what counted as work. **NOT YET OBSERVED, and the reason is the gate working.** Measured on the running pod at `main-7f992516`: **2,200** dataset outcomes since it started, `fragments_removed` and `old_versions_removed` both summing to **0** — the estate is converged, so there is no material rewrite to record. That is the same posture `.claude/skills/rask-lance-catalog` already carries for the medallion lineage emit (*"no medallion tier has yet been observed emitting, because every sweep since has measured fragments_removed: 0"*). **AND THE THIRD SIGHTING OF ONE TRAP**: driving the funnel from a `python` in the pod produced no record at the destination — verified, zero rows matching the probe id — because a sibling process has neither the app's `configure_audit` nor its OTLP handler. The compliance stream is unpolluted and the lesson is paid for three times now: measure through the running app or through a door. **WHAT REMAINS is the row's harder half** — attempt counts, duration, and a per-object FGA-gated fact a TENANT can query, which this operator-global record on a 14-day trace TTL does not answer |
| Q17-37 | `role` and `team` are unscoped, relationless subject types | med | **FOUND 2026-09-09, and the existing ruling answers a DIFFERENT question.** `docs/DECISIONS.md` "Team/role administration — WONTFIX until the Keycloak sync" refuses building an identity-write surface; it does not address the model shape. In `model.fga` neither `role` nor `team` carries a `project` edge, so a project admin — who holds `manage_grants` on their warehouse via `or can_administer from project` — can bind ANY global role name to `owner` on their tenant's data while being unable to see or influence who is inside it, because the expansion doors are estate-only. Conversely one estate-side `role:x#assignee` write silently widens access in every tenant that ever referenced that role, with no per-tenant record of it. Leave the WONTFIX alone; add the two things it does not cover, both model-side: a `role.project: [project]` edge plus a grant-door validation refusing a `role:` grantee whose project edge is not the object's own tenant. **RE-MEASURED 2026-09-09 AGAINST THE MODEL AND THE LIVE STORE, and the row's title is half wrong.** `type team` carries only `member: [user]`, but `type project` carries `team: [team]` — so a TEAM *is* reachable from its project, and `project.admin` reads through it (`or member from team`). `type role` carries only `assignee` and is reachable from nothing in either direction. **So team is SCOPED and role is not**, which is a sharper claim than *both are unscoped* and narrows the fix to one type. **THE BLAST RADIUS IS FIVE TUPLES**, measured across all 4,041 in the live store: three `team:` entries are `project.team` hierarchy edges rather than data grants (`team:eng → team → project:acme`), and the only two `role:` grants are `role:validators#assignee → validator → namespace:acme_gold` and `namespace:acme-gold`. That is exactly the shape the row describes: a GLOBAL role name holding a rung on one tenant's namespaces, with nothing tying `validators` to `acme` — a second tenant granting the same name shares an audience neither can see, and one estate-side `role:validators#assignee` write widens both. **AND THE DIRECTION MATTERS**: the row proposed `role.project: [project]` while the estate's existing edge points the other way (`project.team: [team]`). Both are defensible and the QUERY decides — `project.team` exists so a project can read THROUGH it, while a grant door needs to ask *"is this role scoped to the object's tenant"*, which `role.project` answers directly and `project.role` answers only by reverse lookup. So `role.project` is right, and the reason is usage rather than convention. **HELD FOR THE OWNER ALONGSIDE § I4**: adding the relation is additive and safe, but the grant-door validation is a behaviour change on `model.fga` — the same surface I4 asks about — and two uncoordinated changes to the estate's authorization model are worse than one considered change  **SIZED 2026-09-10 against the model, the door and the DEPLOY PATH, so the next attempt starts from facts.** **The model half is smaller than it looks and the deploy is nearly free**: `type role` carries only `assignee: [user, team#member, role#assignee]` and needs `define project: [project]`, which is purely ADDITIVE — no existing check changes and every existing tuple stays valid. And this estate pins NEITHER `RASK_FGA_STORE_ID` NOR `RASK_FGA_MODEL_ID` (read off the running catalog), so `fga.provision()` takes its documented dev path — it reuses the store and REWRITES the model on every boot from `model.json`. A model change therefore ships with an ordinary image roll; there is no separate `fga model write` step. Note the THREE COPIES the Makefile records: `model.fga` authored, `model.fga.yaml` tested, `model.json` loaded — `make fga-test` runs the suite AND diffs them, so all three move together or the gate reds. **THE ENFORCING HALF IS WHAT MAKES THIS REAL WORK, and it is a hop the row does not mention.** The validation belongs in `access.py::_grant_or_revoke`, right before `fga.write_tuples`, and it must ask "is this role's project the OBJECT's tenant?" — but the object is `table:<ns>$<t>` or `namespace:<ns>`, and a namespace's tenant is not in its name: it resolves through the binding registry (namespace -> binding -> warehouse -> project). So the fix needs a registry read on the grant path, which is a latency and a failure mode on a security-critical door, not a string split. **NOT STARTED rather than half-started**: the model edge alone is an unused relation, and an unenforced edge is the "control that cannot fire" shape this file keeps recording |
| Q17-38 | Every grant in the estate is keyed on a Dex-CONNECTOR-scoped opaque subject, and no migration story exists | med | **MEASURED 2026-09-09, and it corrects the finding that produced it.** The Lakekeeper comparison's completeness critic called identity *"the one abstraction drawn in the wrong place"* and *"the cheapest thing on this page to fix"* — `token.sub` is read at `governed/deps.py:181` and `:208` with no claim mapping, where the reference defaults to `["oid", "sub"]` and documents why (*"we prefer `oid` for Entra-ID, where `sub` differs per application"*, `authn.rs:630`). The observation is right. **The pricing is wrong, and the live store is what shows it.** Reading the actual tuples: subjects are `user:CiQwOGE4Njg0Yi1kYjg4LTRiNzMtOTBhOS0zY2QxNjYxZjU0NjYSBWxvY2Fs`, which base64-decodes to `\n$08a8684b-db88-4b73-90a9-3cd1661f5466\x12\x05local` — a **protobuf-encoded Dex subject carrying the user id AND the connector name**. So the estate's grants are keyed not merely on `sub` but on a value scoped to one Dex CONNECTOR; changing the connector alone re-keys every one of them, never mind changing the IdP. **A configurable `subject_claim` therefore does not deliver IdP portability — it would let an operator CHOOSE the claim while every existing tuple still names the old value, which is the illusion of portability rather than the thing.** `docs/DECISIONS.md`'s planned Keycloak sync writes `team#member`/`role#assignee` tuples via an event listener, which covers GROUP membership and not the per-user grants a tenant has accumulated. **What is actually missing is a subject-identity MIGRATION story** — a stable internal principal id that FGA keys on, with the IdP subject as a mapped attribute rather than the key itself — of which a configurable claim is one small part. Note what is NOT broken: for Dex today and Keycloak as planned, `sub` is stable per realm, so this is latent and IdP-conditional rather than a live defect; the Entra-ID hazard the reference guards against is Entra-specific. Recorded rather than half-built, because shipping the cheap half would make the expensive half harder to see |
| Q17-39 | The estate's own verification gate runs serially on a 64-core machine | low | **HALF FIXED AND MEASURED 2026-09-09; the diagnosis in this row was RIGHT ABOUT THE MACHINE AND WRONG ABOUT THE CAUSE.** It blamed serialism, and serialism is real but secondary. Four independent probes plus an adversarial refuter measured the same two things: (1) a full `tests/unit` run made **279 `helm template` invocations for ~113-128 distinct argument sets** — 151-166 byte-identical recomputations, ~123 s of subprocess discarded — because `_rendered_docs` is a plain `subprocess.run` with no memoisation and **99 private chart-render helpers across 39 files** each rolled their own; and (2) **helm was never the larger half**. One render is 0.42-0.45 s while `yaml.safe_load_all` over its 2.3 MB / ~300-document output is **1.51 s**, against **0.138 s** for the `CSafeLoader` that was already installed in this venv and appeared **zero** times anywhere in `tests/`, `packages/` or `services/`. An instrumented run attributed **424.7 s of tests/unit's 740 s (57%)** to that one pure-Python parser. **LANDED** (`bf141737` + the loader sweep): `_rendered_docs` memoised and parsed by libyaml — the 19 files that import it rather than defining their own inherit it — then every remaining `yaml.safe_load_all` in `tests/unit` (36 sites across 17 files) converted to `yaml.load_all(..., Loader=yaml.CSafeLoader)`. **MEASURED, in two steps against the same selection**: 726.9 s → **420.8 s** after the memoised, libyaml-parsed `_rendered_docs` alone, → **308.3 s** after the sweep. **A 58 % cut, 3 978 passed / 0 failed at each step**, from two lines and a mechanical replacement. **THE WHOLE GATE, measured on the committed tree**: `make test` = **8 m 36 s** (`8 430 passed, 9 skipped, 1 xfailed` in 498.94 s, plus the two sealed runner suites), against the ~16 min the goal's VERIFICATION clause is written around — so the owner ruling that the full suite is not per-commit was priced against a number that no longer holds. A refuter caught the one robustness defect in the fix itself: `yaml.CSafeLoader` is absent when PyYAML is built without libyaml, and `uv.lock` carries an **sdist beside its 28 wheels**, so a platform with no matching wheel would have `AttributeError`ed at import and REDDED every chart gate for what is purely a speed change. Closed by `tests/unit/chart_yaml.py` — one `FAST_LOADER` that degrades to `yaml.SafeLoader`, with `test_the_chart_gate_parses_with_libyaml.py` asserting the fast path is the one in use, so a lost libyaml reports as ONE failing test rather than as a suite that silently got three times slower. **STILL OPEN, and deliberately not taken in the same pass**: `pytest-xdist` is still not a dependency, and the parallel-unsafe suites the probes named — the `lance.audit` process-global logger, `configure_audit`'s level, the registry CAS markers — need grouping (`--dist loadfile`) before `-n` is safe. A separate zero-code lever was measured too: the 19 non-unit trees run **238.9 s sequentially against 72.8 s concurrently**, every tree green. **THE RESIDUAL, measured AFTER the fix by an adversarial refuter, and the ordering is now inverted**: helm subprocess is 43 % of `test_invariants.py` and YAML 17 %, where before the parse dominated. **44 files touch helm; 16 share `_rendered_docs` and 17 roll their own `subprocess.run` render** — 105 uncached calls for 162 tests, and even inside `test_invariants.py` 61 calls are only 40 unique, because `_helm_template` is deliberately left uncached one level BELOW the cache for the three callers that need a real subprocess each time. Residual ≈ 76 redundant renders × 0.445 s ≈ **34 s**. **CONVERGING THEM IS NOT A PURE SPEED CHANGE, and that is the reason it has not been done in the same pass.** Reading the 17: **13 are cleanly convertible**, **4 need a real subprocess** (`check=False` in three, `CalledProcessError` in `test_invariants` itself) — and the baselines are **not** all the same. `test_chart_gitops_ready._render` OMITS `image.localImages=true` ON PURPOSE, because its subject is that a bare image name must be REFUSED; `test_prod_ha_posture` renders with `-f chart/values-prod.yaml`. A shared helper that normalised those would silently change what two gates assert. So the convergence has to make the differing baselines VISIBLE at each call site — one cached `render(*flags)` keyed on the verbatim flag tuple, with the identity flags as a named constant callers compose — rather than hide them behind one default. **REFUTED so nobody re-diagnoses them**: the autouse `_reset_pooled_ray_client` fixture costs 0.2 s of 728 s; the AST gate tests cost 17.2 s (2.3%) and assert genuinely different properties; disabling the 10 vendored subcharts saves only 16% of the parse. And the CLUSTER is not the problem — `kubectl` answers in 0.08-0.26 s, scheduling is +0.0 s, 0 CrashLoopBackOff, no `pending-upgrade` |
| ~~Q17-40~~ | ~~Distributed compaction is configured ON and denied at the router, so every rewrite silently falls back in-pod~~ | ~~high~~ | **FOUND, FIXED AND OBSERVED 2026-09-09** (`235d43c3`). § E2 bootstrapped the sweep as a `maintainer` rather than a `writer`; distributed compaction calls `compaction_plan` and `compaction_commit`, and NEITHER was named in `fga_deps`' rung maps, so `_action_relation` fell them to the WRITER rung. Two rules individually right and jointly denying — the router refused before the endpoint ran. **It surfaced only because § Q17-29 turned on `vendWriteCredentials`, which renders `MAINTENANCE_CATALOG_URL` — the value all THREE of maintenance's catalog calls need. Before that the URL was empty, so this defect was hidden behind a second one**; fixing the silent root-key fallback is what made the silent compaction fallback visible. **BEFORE, measured**: `maintenance.distributedCompaction = True` in the running pod, `mode: in_pod` for **4,274 of 4,274** dataset outcomes in 20 minutes, and **1,818** `can_write_data` DENY for `service-maintenance` in the same window (~5,200/hour) carrying no `tier` attribute — the marker that says router gate rather than vending door. Nothing red: the sweep compacted in-pod and reported success, so the only symptom was the memory ceiling the feature exists to remove. **AFTER, at `main-235d43c3`**: `mode: distributed` **728** against `in_pod` 1,079 in an 8-minute window spanning the rollout, and `can_write_data` denials **GONE** — the action is absent from the grouped result, leaving only the unrelated `can_maintain` 56 / `can_read_data` 41 of the per-warehouse coverage gap. Same class as `changes` and `history` arriving from the other side: a MAINTENANCE door demanding a rung the estate deliberately took away. `can_maintain` outright rather than an alternative rung, because unlike `credentials` these two doors have no audience but maintenance; the fail-closed default is untouched and pinned |
| ~~Q17-41~~ | ~~Every derived tier declares its PARENT's canonical name, so its maintenance provenance is filed against the wrong dataset~~ | ~~high~~ | **FOUND, FIXED AND OBSERVED 2026-09-09** (`fb8e9023`). `lineage.dataset_id` is schema METADATA, and metadata survives every column operation `stamp_stage` performs — `set_column`, `append_column`, `drop_columns` all carry it forward — so a stage that said nothing about it handed the child its upstream's identity. **MEASURED before the fix**, through the real stamp: a bronze table declaring `acme$bronze` produced a stamped silver table declaring `acme$bronze`, and the empty schema the DISTRIBUTED lane creates its destination with (`ray_stage_job._target_schema`) declared `acme$bronze` too — both lanes, one defect. `maintenance.core.lineage_emit.declared_table_id` reads exactly that key to decide which dataset a maintenance run is about, so silver's compactions and silver's per-dataset FAIL events — the estate's ONLY per-dataset maintenance failure surface — were being written against bronze's node. **Attestation O12 passed throughout**, and that is the reusable part: it asks whether a name is stamped, never WHOSE, so a presence check over an inheritable field certifies the defect it was written to catch. **The gap was a READER, not a writer**: `WorkOrder.to_env()` has always shipped `RASK_DEST_TABLE` and the job never read it. Closed by making the stamp own the question — `stamp_stage(..., dataset_id=...)` re-declares the destination and DROPS an inherited id when unwired, which is the rule it already applied one line up to the `lineage` document (the parent's document describes the parent's run; the parent's id names the parent's dataset). Unwired now means NO name, so `declared_table_id` returns `None` and the sweep falls back to the URI derivation: a weaker answer, but a true one, where an inherited name is a confident lie. The constant had FOUR definitions (stamp / medallion / maintenance / attestation) and a test whose whole job was asserting two of them still agreed; it is defined once beside the stamp that writes it and imported by the other three, so disagreement is unrepresentable rather than pinned. **THE FIRST FIX WAS INERT ON THE LIVE ESTATE AND THE MEASUREMENT IS WHY IT WAS CAUGHT**: `merge_insert` carries ROWS and not schema metadata (pylance 10.0.0 — a dataset created declaring `acme$bronze` still declared it after a full-sync merge of a table declaring `acme$silver`; only an `overwrite` moved it), and the cascade's steady-state write IS that merge, deliberately, because an overwrite re-mints every `_rowid`. So the stamp alone would have landed only on tiers created after it. `ensure_declared_dataset_id` closes it with a metadata-only commit — one key, other producers' keys preserved, no data rewritten, no row id re-minted, idempotent by a read — called on every write path including the `add_columns` fast path that returns before all the others. **The estate therefore SELF-HEALS on the next cascade run over each dataset rather than needing a backfill pass**; what stays an owner decision is only a tier the cascade will never touch again. OBSERVED in the deployed pod at `main-bf141737`: bronze `acme$bronze` → stamped-unwired `None` → stamped-wired `acme$silver` → after merge still `acme$bronze` → repair wrote True → `acme$silver` → repair again False |
| ~~Q17-42~~ | ~~The vend scope probe shipped with its IO half and its authorization gate untested — a pure reducer at 100% beside the body it summarises at 0%~~ | ~~high~~ | **FOUND BY THE VERIFICATION WORKFLOW AND FIXED 2026-09-09** (`4bb23982`). Measured: `catalog/api/v1/endpoints/warehouses.py` **47%**, with `988-1042` (the whole body of `_run_scope_probe`) and `967-978` (the whole body of `validate_warehouse_credentials`) in the missing list, while `catalog/services/vend_probe.py` — the 23-statement pure reducer beside them — read **100%**. `grep` over `tests/` and `tests/e2e-py/` for either symbol: no hits. **THE SHAPE IS THE FINDING, not the row.** A helper at 100% next to the IO body it summarises at 0% reads as coverage and is not, and the untested half was the one the module's own docstring calls *"THE WHOLE STORAGE POSTURE"* — including the `write_outside_refused` step, which is the actual security check, over a failure mode the module itself records as *"UNDETECTABLE AFTER THE FACT"*. Closed with 11 tests over both branches of the question the probe exists to answer (a store that HONOURS the session policy, and one that accepts and IGNORES it — every other check green in the second, which is exactly why a probe that only asked *did the writes work* would report a healthy warehouse on a store handing out the whole bucket), the three shapes that must read UNKNOWN rather than healthy (no credential, vendor raising, first write refused), and the gate: denied and missing raise the byte-identical error (no-existence-oracle — this door writes to a tenant's bucket), nothing is probed before the gate answers, warehouses-off is Unsupported and never reaches the registry, and the probed root comes off the REGISTRY RECORD rather than the request. Both bodies now fully covered. **The general rule, worth more than the fix**: when a change lands a pure helper beside an IO shell, coverage of the helper is not evidence about the shell — and a `--cov` line naming the module is what tells them apart |
| ~~Q17-43~~ | ~~`make registry-gc` would delete every image the live release is running, and its own comment argued it was safe~~ | ~~high~~ | **FOUND AND FIXED 2026-09-09** (`965f59b0`), while acting on two audits that recommended running it. The script kept the **lexically-last** N tags per repository, justified in its own comment as *"epoch-suffixed tags are same-width, so lexical == chronological"* — true of a tag scheme the estate no longer uses. Its tags are `main-<git-sha>` / `zone-<sha>` / `fix-<sha>`, and lexical order over hex is arbitrary. **MEASURED against the running release before touching anything**: the newest-2-lexically for `web-lakehouse` were `zone-75b5a141` / `zone-d5080599`, for `lance-rest-catalog` `verlist-215945` / `wire-32ff50cb`, for `ray-lance` `main-b791c0d7` / `wire-32ff50cb` — so the reclaim would have deleted `main-90b8d0ed`, `main-bf141737` and `h14-ce91f68b`, **every pin the estate was serving**, across all 16 repositories it names. **THE FAILURE MODE IS WHY THIS IS HIGH AND NOT TIDINESS**: k3s serves running pods out of containerd's own cache, so nothing goes red at reclaim time. The estate fails at the next restart or reschedule, with no event connecting the ImagePullBackOff to the sweep that caused it — and `make dev-gc` is documented in `CLAUDE.md` as safe, with 289 GB of pressure making it attractive. Rewritten around two rules: **a tag any running workload references is never a candidate** (read off the live cluster across pods, controller templates and the Ray CRs, plus every tag in `chart/values-live-pins.yaml` — because a pin not yet rolled out would `ImagePullBackOff` the next `make k3s-up`), and what is left is ranked by **the image's own `created` time from its config blob**, since a registry exposes no push time and the tag string carries none. The ranking half is extracted to `scripts/registry_gc_plan.py` and pinned by `tests/unit/test_a_reclaim_never_deletes_what_the_estate_is_running.py` — a rule with a silent failure mode belongs where a test can reach it. `RASK_REGISTRY_DRY_RUN=1` prints the plan, because a safety argument that rests on a protection list deserves a way to read the list. **VERIFIED on the live registry**: 16 repositories protected, 1 151 stale manifests identified, all three measured casualties SAFE. The 289 GB reclaim is now the owner's to run |
| Q17-44 | TWO Kueue installations reconcile the same CRDs, and one of them is not chart-owned | low | **PARTLY MEASURED 2026-09-09; recorded rather than fixed because the second install is not this repo's.** A verification-workflow refuter reported the k3s journal running **63 % Kueue error spam** — 4 856 of 7 752 lines in a 12.5-minute rollout window, 4 668 of them `tls: failed to verify certificate: x509: certificate signed by unknown authority ... 'kueue-ca'` against the CRD conversion webhook, continuing at ~5.8 lines/s. **I COULD NOT CONFIRM THE SPAM** — `journalctl -u k3s` returned zero lines for me, so that half stands on the refuter's measurement and not mine, and it is written here as theirs. **WHAT I DID CONFIRM, and it is the likely cause**: there are TWO Kueue controller managers on this cluster — `default/rask-kueue-controller-manager` (chart-owned: the CRDs carry `app.kubernetes.io/managed-by: Helm`, `helm.sh/chart: kueue-0.18.1`, and their conversion webhook points at `rask-kueue-webhook-service` in `default`) and `kueue-system/kueue-controller-manager`, 43 days old and owned by nothing in this repo. Two operators reconciling one set of CRDs is the estate's recurring shape — *a convention one path holds and another does not* — and a CA bundle written by whichever reconciled last is exactly what produces that TLS line. The CRs themselves answer (`clusterqueues`, `localqueues`, `workloads` all list), so this is log pressure and a latent reconcile fight rather than a broken queue. **NOT ACTED ON**: the second install arrived with the foreign `htr-batch` namespace (the same namespace carrying the estate's only ImagePullBackOff pods, none of them chart-owned), so removing it is a cluster-operator decision about somebody else's workload, not a repo change |
| ~~Q17-45~~ | ~~The deploy half's 12-minute web-zone stall was diagnosed as an image-pull/layering problem by two independent audits, and it was neither~~ | ~~med~~ | **STRUCK 2026-09-09 with the measurement that refutes it, and the refutation is the finding.** Two verification probes independently reported that five web zones did not start their container until +696…707 s while `home` and `compute` started at +0 s, and both concluded IMAGE PULL — one going further and naming a root cause: `.docker/frontend.dockerfile:91` copying the whole workspace `node_modules` into each zone, seven byte-identical 206 MB copies under seven different layer digests, ~12 min × 5-7 zones per deploy. I was one step from acting on it. **IT WAS A MISSING SECRET.** An adversarial refuter read the journal rather than the timestamps: 110 lines of `CreateContainerConfigError: secret "rask-infra-credentials" not found` for `web-lakehouse` in that window and **zero** of `ImagePullBackOff` / `ErrImagePull` / `Pulling image`. The ExternalSecret was created 06:51:38Z and reached `SecretSynced` at 07:03:14Z — 696 s — and the first container started 07:03:14–07:03:25Z. All five zones blocked on the same object and started within 11 s of each other, so it is **ONE shared 11 m 36 s stall, not five 12-minute pulls**. The pull hypothesis is independently impossible: backend pods pulling from that same registry that morning recorded `totalImagesPullingTime` of 4.70 / 5.11 / 5.29 s, and 707 s for a 0.30 GB image would be 2 MB/s from a same-host registry. The proposed root cause is false too — the zone images share 7 layers, union 1.369 GB against 1.798 GB summed, so they ARE sharing a base. **AND THE CAUSE WAS MINE, DRAINING.** Helm revision 116 landed 08:51:28 local = 06:51:28 Z, the minute the ExternalSecret was recreated: it is the § Q17-30 ESO repair (the `ray-compute-*` properties an ExternalSecret read and nothing wrote, which failed the object WHOLE and took every consumer with it). The 696 s is ESO's backoff draining after the seed landed — a one-off tail of a fix, not a standing defect. **IT WAS A ONE-OFF IN FACT AND A STANDING RACE IN MECHANISM, and the workflow's synthesis got the second half right and the first half wrong.** It concluded *"it recurs on every upgrade (revisions 116-120 landed in a single morning)"*. Measured: every web-zone pod still carries `creationTimestamp: 06:51:38Z` — they were recreated ONCE, at revision 116, and revisions 117-121 left them untouched, so four upgrades cost nothing. But the MECHANISM is a standing risk and no hook orders it: `chart/templates/openbao.yaml`'s seed Job and `chart/templates/external-secrets.yaml` carry **no `helm.sh/hook` annotation between them**, so an upgrade that ADDS a property to an ExternalSecret's data list races the seed that writes it — and `creationPolicy: Owner` plus ESO's whole-or-nothing rule means the estate loses the ENTIRE Secret, not the one new key, for as long as ESO backs off. Revision 116 was exactly that upgrade (§ Q17-30 added `ray-compute-*`). Recorded as § Q17-47 rather than folded in here, because the repair is a chart ordering change with its own verification. **THE LESSON IS GENERAL AND WORTH MORE THAN THE ROW**: a pod-timestamp delta says a pod waited, never what it waited FOR, and two independent audits reading the same timestamps produced the same wrong cause and a confident remediation for it. Read the journal, not the deltas — and note that a third audit, from the same measurements, recommended a registry reclaim that would have deleted every live image (§ Q17-43) |
| Q17-46 | A third of `tests/unit` is single-component tests that belong in a service's own directory, which is why a one-service change drags the estate suite in | med | **OWNER QUESTION 2026-09-09, MEASURED RATHER THAN ASSERTED** — *"Should the test not be in each microservice then? Tests correctly structured or wrong?"*. Classified all **329** files in `tests/unit` by how many components each actually imports: **53 render the chart** (correct here — no single service owns a cross-service render), **119 import two or more components** (correct here — a seam between services is exactly what a per-service directory cannot hold), **46 import none** (repo-shape and invariant gates, correct here), and **111 import EXACTLY ONE** — 33 catalog, 25 service-kit, 19 annotator, 14 medallion, 9 maintenance, 6 lineage, 2 ingest, 2 search, 1 storage. **Those 111 are the answer to the owner's question and the cost is concrete**: `services/catalog/tests` holds 72 files while 33 more catalog-only files sit in the shared directory, so a catalog change cannot be verified against catalog tests alone and the per-commit selection is dishonest by construction. **WHAT RELOCATION WOULD NOT HAVE FIXED, stated so the move is not oversold**: the § Q17-42 red that shipped this same day lived in `tests/integration` and was a genuinely cross-service assertion about the catalog's authorization gate. No layout change moves it, and the lesson there is about the SELECTION (the per-commit run must always include the invariant and integration layers), not the tree. **NOT DONE — one owner call and one read first.** It is 111 file moves plus import fixes across every service's testpath at once, and 25 of the 111 are `service_kit` files that may be shared FIXTURES rather than misplaced tests; those get read before they move. The gate is meanwhile down to `make test` = 8 m 36 s (§ Q17-39), so this is now about honest selection rather than wall clock |
| Q17-47 | An upgrade that adds a property to an ExternalSecret races the OpenBao seed that writes it, and the estate loses the WHOLE Secret meanwhile | high | **MEASURED 2026-09-09.** `chart/templates/openbao.yaml`'s seed Job (`bao kv put …`) and the three ExternalSecrets in `chart/templates/external-secrets.yaml` carry **no `helm.sh/hook` annotation**, so one `helm upgrade` applies them together and their order is arbitrary. ESO reads a property the seed has not written yet, fails the object — and because it syncs a Secret **whole or not at all** with `creationPolicy: Owner`, what is lost is not the new key but the entire Secret, taking every consumer with it into `CreateContainerConfigError`. **COST, measured on revision 116** (the § Q17-30 upgrade that added `ray-compute-*`): the ExternalSecret was created 06:51:38Z and the Secret it owns only appeared **07:03:14Z — 696 s later**, and five web zones sat in `CreateContainerConfigError` for that whole 11 m 36 s. It is one shared stall rather than five, and two independent audits misread it as an image pull (§ Q17-45). **NOT EVERY UPGRADE**, and the workflow's synthesis was wrong to say so: every zone pod still carries `creationTimestamp: 06:51:38Z`, so revisions 117-121 cost nothing. The trigger is specifically a CHANGE to an ExternalSecret's data list — which is exactly what happens whenever the estate adds a scoped identity, and the zero-trust goal adds them by design. **THE REPAIR, and the reason it is not a one-liner**: the seed must complete BEFORE the ExternalSecrets reconcile, which is a `helm.sh/hook: pre-upgrade` on the seed Job — `pre-upgrade` and NOT `pre-install`, because on a first install OpenBao does not exist yet and the Job's own `until bao status` wait would hang the release; on an upgrade the previous revision's OpenBao is still serving, which is precisely what makes the ordering work. A fresh install keeps today's behaviour, where the race is harmless because nothing is serving yet. **AND THE OBVIOUS VERSION OF THIS FIX WAS ALREADY TRIED AND REVERTED**, which is why it is written down rather than shipped: `openbao.yaml:129` says *"NOT a hook — see BOOTSTRAP JOBS in _helpers.tpl"*, and `_helpers.tpl:481-495` records the live proof (2026-07-28) — a post-install hook runs only AFTER `--wait` has already given up on the very pods that need the secret, and a pre-install hook runs before any release manifest exists. **That ruling is about INSTALL-time circularity and does not reach pre-UPGRADE**, where the previous revision's OpenBao is still serving, so the distinction is real rather than a loophole; but a future reader will meet the ruling first and must be told why it does not apply. **TWO THINGS MAKE THIS AN OWNER CALL RATHER THAN A COMMIT.** (1) A resource is either a hook or a release resource, so the pre-upgrade copy is a SECOND Job — and the seed is a ~120-line template of conditionals over every scoped identity in the estate, so it has to be extracted into a named template first, in the chart's most load-bearing file. (2) A `pre-upgrade` hook that fails ABORTS the release, and the Job's own `until bao status` wait is unbounded, so it needs an `activeDeadlineSeconds` or a sealed OpenBao hangs `helm upgrade` forever. Bounded, it turns today's silent 12-minute Secret outage into a loud refusal to deploy — which I think is the better trade and is not mine to make |
| Q17-48 | A published delta cannot express a RETRACTION, and the cascade retracts | med | **FOUND BY MINING THE pylance 11.0.0 RELEASE, then measured against live code (2026-09-09).** `PublicationResult`'s contract (D-R3, `publication.py`) is that a consumer resolves the delta between two published versions with `_row_created_at_version > from AND <= to` and keeps no bookmark. That filter finds rows CREATED in the range and is structurally blind to rows DELETED in it. **The gap is live, not theoretical:** `medallion/services/compute.py:380` executes `merge_insert("id").when_matched_update_all().when_not_matched_insert_all().when_not_matched_by_source_delete()`, so a silver/gold row whose parent vanished from the source IS deleted on the next run — and every consumer that resolved the delta keeps serving it forever. That is the exact failure ruling D1 exists to prevent, one layer out: impact analysis answers confidently and wrongly, this time by OMISSION. **v11 supplies the missing half:** `LanceDataset.delta(begin_version, end_version).get_deleted_row_ids()` streams the row ids deleted in a range (new in 11.0.0 — `get_inserted_rows`/`get_updated_rows`/`list_transactions` already existed on 10.0.0, `get_deleted_row_ids` is the addition, confirmed by diffing the two API surfaces symbol by symbol). **DRIVEN, not inferred:** a real dataset seeded `{1,2,3}` then merged with the cascade's exact call against a source that lost `id=3` retracts it, and `delta(begin, end).get_deleted_row_ids()` answers `{'_rowid': [2]}` while `get_inserted_rows()` answers 0 rows — so today's contract reports NOTHING about a retraction the tier actually performed. NOT YET BUILT: the fix is a contract change to what a publish notification carries, so it needs a decision about whether the delta is served on demand from the door or stamped into the event |
| ~~Q17-49~~ | ~~The orphan scan deserializes every manifest of every dataset, on every tick, and its own cost ceiling VOIDS the answer~~ | ~~med~~ | **`referenced_paths_of` (`maintenance/services/orphans.py:192`) calls `ds.versions()`, which reads and deserializes EVERY manifest** — for every dataset, in every warehouse bucket, on every reconcile tick, when all it uses is `v["version"]`. The cost is not incidental: it is why `_MAX_VERSIONS` exists, and hitting that ceiling sets the `note` that makes the referenced set INCOMPLETE, so the scan reports NOTHING as an orphan for that dataset. A cost control that turns into a coverage hole. **pylance 11.0.0 adds `LanceDataset.version_refs()` — *"Unlike `versions()`, this does not read or deserialize every manifest"*.** Measured on a 120-version local dataset: `versions()` 9.3 ms, `version_refs()` 0.5 ms, **18.4x**, identical version list. On S3 each of those manifest reads is a network round trip, so the local number is the floor. `dataplane.py:649` is the same shape — it filters `int(version_info["version"]) > read_version` on the CAS probe that decides whether a run already committed. Two call sites, both version-numbers-only. The other six `versions()` callers read `timestamp` and must stay | **FIXED 2026-09-09.** Both call sites now read `version_refs()`. Re-measured through the real function rather than the accessor alone: `referenced_paths_of` over a 100-version dataset spends 7.2 ms in `versions()` against 0.5 ms in `version_refs()` — 14.3x of the walk's per-dataset overhead, and that is the LOCAL floor, since in production each of those manifest reads is an S3 round trip. Equivalence checked on the three states this scan actually meets (fresh, tagged, and after a cleanup that removed versions — a tag-pinned version survives in both). **THE FIX ALSO EXPOSED THE DOUBLES' ROT, which is the more useful half:** three hand-written stand-ins for `lance.dataset` listed their methods explicitly and answered `AttributeError` for the new one — and in the orphan suite that error was REPORTED AS A REFUSAL, so `test_a_dataset_using_overlays_is_refused` went green for the wrong reason. Caught by reading the refusal text, not the exit code. The overlay double now forwards to a real dataset for everything EXCEPT the two seams the test fakes (a blanket forward hands back a real handle whose fragments carry no overlay, which is how the false pass happened); the two catalog doubles track the accessor the code under test actually calls |
| ~~Q17-50~~ | ~~Compaction is bounded at the READ BATCH because that was the only bound Lance offered; v11 bounds the PASS~~ | ~~low~~ | **The estate already reasoned its way to this control and could not have it.** `maintenance/core/config.py:70-80` records incident #93 in its own words — *"Rows are not a unit of memory: against ~1.8 MB bronze page-image rows (measured) that is ~15 GB per compute thread"*, against a pod inheriting a **512Mi limit**, so the first blob-tier tick OOM-killed the sweep and a maintenance pod in CrashLoopBackOff maintains nothing. The repair was two knobs whose PRODUCT is the memory (`scan_batch_size` x threads), bounding the read batch because `compact_files` exposed nothing else. **pylance 11 adds four planning-level bounds**, verified present in the installed signature: `max_source_bytes`, `max_source_rows`, `max_source_fragments` and `excluded_fragment_ids`. `max_source_bytes` is the bound the note was reaching for — a unit of MEMORY applied to the pass rather than to the batch — and it composes with the existing read bound rather than replacing it. `excluded_fragment_ids` is the separate half: a sweep that failed on a fragment currently retries the same one every 120 s tick with no way to set it aside. LOW because the read bound is in place and holding, so this buys defence in depth and a cheaper failure mode, not a fix for something broken today  **STRUCK 2026-09-10.** `max_source_bytes` threaded end to end — `MaintenanceSettings` (256 MiB default against the pod's inherited 512Mi limit), `DatasetPlan` so it crosses the broker with the unit, `MaintenancePolicy` for per-tier override, and into `size_kw` on BOTH compaction attempts. It COMPOSES with the read bound rather than replacing it: `scan_batch_size` caps a read chunk in ROWS, and the settings note that introduced it says why that is a proxy — "Rows are not a unit of memory". This is the half that survives a tier whose rows are larger than whoever set the count assumed, which is exactly how incident #93 happened. Gated by a spy asserting the kwarg REACHES `compact_files` (a dropped kwarg looks identical) plus a settings test that the default is not None. **SCOPE CUT, stated:** `excluded_fragment_ids` stays open — setting a failed fragment aside needs cross-tick memory of which fragment failed, and the sweep is stateless per tick. |
| ~~Q17-51~~ | ~~The Ray lane hand-rolls `storage_options` in the ONE spelling that fails in every pod, and cannot express a vended credential at all~~ | ~~high~~ | **FOUND BY READING THE RUNNING POD (2026-09-09), which is what the goal asks for.** `service_kit.lakehouse.objectfs.lance_storage_options` is the estate's builder and EVERY service uses it — ingest (`objectstore.py:245`), lineage (`config.py:269`), medallion (`config.py:461`), catalog (`vending.py`). The baked Ray job scripts do not: `scripts/ray_stage_job.py:72-80` — the per-stage cascade transform, baked into BOTH ray images — builds the dict by hand, and so do `ray_lance_job.py:38` and `ray_train_job.py:64`. **TWO CONSEQUENCES, and the second is the one that blocks the goal.** (1) The hand-rolled copies use the BARE keys (`access_key_id`, `secret_access_key`) where the builder uses `aws_`-prefixed, and the builder's own docstring records why that matters, measured in-cluster 2026-09-03: object_store BLENDS the ambient `AWS_*` environment with bare keys, so a request signs *"with a pair belonging to neither identity"* and fails `403 SignatureDoesNotMatch` — *"doubly hidden"*, because it reads as an expired credential and **no test process has an ambient AWS_\* environment, so the spelling that fails in every pod passes every unit test**. It does not bite today only because the running head carries no `AWS_*` (surveyed: `S3_ENDPOINT`, `S3_KEY`, `S3_SECRET`, `S3_REGION`, `RAY_DEDUP_LOGS`, four lineage tokens, nothing else). (2) **The hand-rolled dict has NO session-token field, so the Ray lane is structurally unable to accept an STS credential** — the builder's docstring names exactly this: *"a builder that cannot express one forces every vended-credential caller to hand-roll the dict … or to sign as a different identity"*. That is a prerequisite for § Q6-1, not a tidiness item: no amount of vending work on the submitting side can land while the receiving side cannot hold the third leg of the triple. IN SCOPE: `ray_stage_job.py` (the cascade) and `ray_lance_job.py`. OUT: `ray_train_job.py` — the TRAIN lane, struck by the 2026-09-07 scope ruling, and named here so its omission reads as the ruling rather than as a miss | **FIXED 2026-09-09.** Both in-scope scripts now call `lance_storage_options`, and `ray_stage_job._reset_dataset` builds its filesystem with `s3_filesystem` instead of a hand-built `S3FileSystem` — which also removes the second half of the same hazard, since that copy forwarded no session token and pyarrow falls back to the pod's own credential chain for anything it is not given (`objectfs`: *"a half-forwarded vended credential can sign with the POD's own role — broader rights than the catalog scoped, not narrower. Dropping it fails OPEN"*). `pafs` became a dead import and went with it. Every `so: dict[str, str]` is now the estate's `StorageOptions` alias. **Pinned RED-first** by `test_storage_options_come_from_the_SHARED_builder_in_the_aws_spelling`, which asserts IDENTITY (the module must reference the shared function object, so a reintroduced copy fails) and the SPELLING — the half a behavioural test cannot reach, since no test process carries an ambient AWS_* environment. **Verified in the DEPLOYED image, not just locally:** `service_kit` reaches the job scripts as `/home/ray/jobs/service_kit` on `sys.path[0]`, and `python -c` from that directory in the running head imports `objectfs` and returns the `aws_`-prefixed keys — so the new import cannot become the exit-2 class. `ray_train_job.py` keeps the copy: the TRAIN lane is struck by the scope ruling
| ~~Q17-52~~ | ~~The dummy-lane e2e rides a PROJECT-ADMIN bearer on the one field an unauthenticated surface echoes~~ | ~~high~~ | **MEASURED ON THE LIVE ESTATE 2026-09-09, by probing the running Ray head rather than by reading code.** `GET http://ray-lance-head:8265/api/jobs/` answers **200 with no credential at all** (`ray.auth.enabled: false` is the chart default and this release does not override it, so the `rask-ray-auth-token` ExternalSecret correctly does not render and there is no token to present). The response echoes each job's full `runtime_env.env_vars`; across the 7 jobs it returned, the echoed key set includes **`LINEAGE_TOKEN` on 5 of them**. **`tests/e2e-py/test_dummy_lane_e2e.py:395` is what puts it there: `"LINEAGE_TOKEN": ADMIN_TOKEN`, and `ADMIN_TOKEN` is `LANCE_E2E_ADMIN_TOKEN` — a PROJECT-ADMIN OIDC bearer.** **NOT LEAKED HERE, and the distinction is the whole reason this is a row and not an incident:** every echoed value measured was EMPTY (len 0, sha256 of the empty string), because that variable is unset for these runs. The exposure is what happens on an estate where the suite runs as intended — the credential it is designed to carry lands in a field any unauthenticated reader can fetch, and stays for the job's retention (the 7 jobs read here include runs from previous days). **THE ESTATE ALREADY FIXED THIS EXACT SHAPE AND MISSED THIS CALLER**, which is the recurring anti-pattern: `medallion/services/ray_submit.py` carries the ruling in its own comments — *"NO CREDENTIAL RIDES THIS BODY … The Jobs API echoes `runtime_env` on `GET /api/jobs/<id>`, an unauthenticated dashboard published at the edge, so anything here is readable by anyone"* — and was cleaned of the S3 secret (2026-08-28), then the S3 key id (2026-08-30). Both production submit paths now send only `LINEAGE_SERVICE_ID`. **THE FIX IS THE ONE PRODUCTION ALREADY USES**, and the receiving side already supports it: `runners/dummy/src/dummy_runner/lineage.py:152-171` resolves the bearer as `LINEAGE_SERVICE_ID` -> `RASK_LINEAGE_TOKEN_<IDENTITY>` (pod-mounted; the running head carries four of them via secretKeyRef) before falling back to `LINEAGE_TOKEN`. The e2e uses the fallback. Sending the identity instead of the bearer needs care — the suite asserts attribution, and `RASK_ORIGINATOR` (the admin subject) is a separate field from the bearer, so it stays | **FIXED 2026-09-09.** The e2e sends `LINEAGE_SERVICE_ID: "service-bronze-to-silver"` and no bearer — the value the live bronze-to-silver runner actually presents (`MEDALLION_FGA_SERVICE_IDENTITY`, read off the deployment), so the lane emits as the service it is instead of borrowing an operator's rights. `RASK_ORIGINATOR` is untouched: it names WHO the run is for and stays the admin subject, because attribution and authentication are different questions. **VERIFICATION LIMIT, stated rather than glossed:** the suite skips without `LANCE_E2E_ADMIN_TOKEN`, so the change is type-clean, lint-clean and collects, but has NOT been driven end-to-end. **CORRECTION 2026-09-09 — I called that "owner-blocked" and it is not.** A bearer IS mintable from the estate's own Dex: `rask-dex` has been up 34 days, its discovery document lists the `password` grant, and the chart's static client plus its static user (`dex.clientId` / `dex.staticUser`, the demo pair the e2e itself uses) returns a 690-byte `id_token` from inside the cluster. My earlier 401 was a username that does not exist, not a disabled flow. What the bearer does NOT buy is a free run: alice holds no grant on an arbitrary table, so a live `exists` probe answers **403 PermissionDenied** — the gate working — and reaching the 200 path means CREATING a table, i.e. adding exactly the estate residue § Q8-15 exists to stop. The behaviour stays pinned by the RED-verified unit test rather than bought with residue. **AND IT MAY SURFACE A GRANT GAP, which is the point rather than a regression:** `seed_medallion_fga.sh:94` grants `service-bronze-to-silver` a WAREHOUSE writer rung that cascades to linked tables, but the suite's own failure message already records that this lane needs a link the seed does not write (`namespace:<p>-silver -> table:<p>-silver$dummy`). Under the admin bearer that gap was invisible; under the service identity a missing link fails loudly with the message that names the fix
| Q17-53 | A `_probe.json` nobody writes makes the drift report INCOMPLETE on every tick | low | **MEASURED on the live estate 2026-09-09, on pylance 11:** the reconcile report carries `incomplete: 61`, and one of them is `registry:projects` — *`lance-catalog/_projects/_probe.json` is unreadable (JSONDecodeError: Expecting value: line 1 column 1 (char 0))*. Char 0 means empty or non-JSON from the first byte. **Nothing in the tree writes that file** (grepped for `_probe.json` across `*.py`/`*.sh`), so it is estate residue like the lineage `advz…` datasets § Q8-15 traced. `_list_json_records` takes every `*.json` directly under `_projects/` and REPORTS what it cannot parse, which is correct and deliberate — its docstring: *"a record this cannot read is a tenant it cannot see, and a category computed without it may name a live object as drift"* — so the reader is not the defect, the file is. **Consequence, stated honestly:** `incomplete` blocks `report_is_clean` and therefore the #79 purge, but it is NOT the sole blocker — the same report carries 13 real drift findings (`ghost_projects` 1, `unbound_namespaces` 2, `orphan_buckets` 12, `orphaned_annotation_tasks` 3), so removing this file alone would not open the gate. LOW for that reason. **NOT ACTED ON:** deleting an object from the owner's live control root is not a change to make unasked, and this service is deliberately report-only about exactly this class. The other 60 `incomplete` rows are `depth limit reached` on the dataset-discovery walk across five buckets — a separate, pre-existing coverage bound (`optimize.py:115` already records `max_depth` as unmaintained), and neither class is caused by the pylance bump |
| Q17-54 | KubeRay's operational surface is unadopted — events, history server, RayJob CRD, Kueue gang/priority | med | **OWNER-CAPTURED 2026-09-09, FOR THE COMPUTE PHASE — deferred by instruction, not worked.** Verbatim: *"Don't need to work on this just need to actually just capture it and add it to the todos to do when we start focusing on compute after we are finish with the lakehouse"*. Seven KubeRay guides, each measured against the live estate rather than assumed. **THE FACT THAT REFRAMES ALL SEVEN: there are ZERO RayJob, RayCluster and RayService CRDs.** `kubectl get rayjobs,rayclusters,rayservices -A` answers *No resources found*; the running `ray-lance-head` is a hand-applied Deployment (§ the cascade-on-a-hand-applied-head note), and the estate submits through the Jobs API to it. So most of this surface is not misconfigured — it is unreachable, because KubeRay is not managing anything. **(1) k8s Events** — nothing to emit about until a CRD exists. **(2) History server** — not configured, and the need was demonstrated accidentally the same day: a host `ray stop --force` restarted the head and its dashboard went from 7 jobs to 0. Job history today lives only in the head's memory, so every restart erases the record a failure investigation would need. **(3) RayJob sidecar submitter restart** — N/A until RayJob is adopted. **(4) RayJob quick start** — this is the actual decision the other six hang off, and it is already half-built: the chart templates `medallion-rayjob-rbac.yaml`, and `services/medallion/services/rayjob_executor.py` exists but is the DEAD adapter § Q17-2 asks the owner to keep or delete. Adopting RayJob and answering Q17-2 are the same question. **(5)+(6) Kueue gang + priority scheduling** — Kueue IS installed with two ClusterQueues (`rask`, `htr-batch-cq`), so the substrate is present and neither policy is configured. **Measured live and worth a look on its own: `htr-batch-cq` reports 6 PENDING workloads, and the cause is visible** — 8 `htr-batch` workloads exist, all created 8 DAYS ago, of which exactly 2 are admitted and hold the queue's whole GPU quota (`nvidia.com/gpu: 2`). The ClusterQueue itself is healthy and says so (`Active/Ready`, *"Can admit new workloads"*), so this is not a Kueue misconfiguration: two long-dead admitted workloads never release, and six wait behind them forever. That is a WORKLOAD-plane observation (`htr-batch` is a sealed runner's namespace), captured here rather than worked. **(7) Batch inference** — the cascade already runs Ray Data batches inside the sealed runners, so this one is a pattern the estate has rather than a gap. IN SCOPE for the compute phase by the 2026-09-07 ruling (`services/compute`, ray-kit, the `compute` zone), NOT for the lakehouse pass in flight |
| ~~Q17-55~~ | ~~SIX seams answered "is this Lance target ABSENT?" — including TWO shared classifiers that disagreed~~ | ~~high~~ | **FIXED 2026-09-09, RED-verified, and found by re-measuring § Q6-5 rather than by reading.** The estate's recurring shape — a convention one path holds and another does not — on the one question whose wrong answer is destructive. Four seams asked it with four different marker lists: `catalog/dataplane._ABSENCE_MARKERS` (`not found`, `does not exist`, `no such`), `catalog.plan_compaction` (a local `"not found"`, not using its own module's list), `ingest._reads_as_absent` (`"not found"`), and `lineage.read_storage_version` — the ONLY correct one, which had written down exactly why: *"a loose 'not found' would also match S3's 'Bucket x not found', so a misconfigured endpoint would go on reading as a deleted dataset"*. **MEASURED on pylance 11.0.0 against a stub S3 endpoint returning each error verbatim, because the wording is the only signal pylance gives** (it exports no typed error; the object-store layer flattens absence into a bare `ValueError`/`OSError`). A 403 and a 500 were already re-raised correctly by all four. **A 404 was not: it read as ABSENT everywhere but lineage** — `"not found"` matches the HTTP STATUS LINE `404 Not Found` that pylance embeds in every non-2xx object-store error, and `"does not exist"` matches `NoSuchBucket`'s body verbatim, so neither says anything about whether the table was written. **WHAT EACH SEAM DOES WITH "ABSENT" IS WHY THIS IS HIGH**: `ingest` routes it to `create_empty` — its own comment calls that *"the single operation that must never run over a table that does exist"*; `catalog._already_committed` routes it to "this run has not committed", after which the caller appends the same rows again, against a guard whose stated contract is that it *"must not assume"* what it cannot prove. **THE BLAST RADIUS, STATED HONESTLY AND NOT INFLATED**: for a bucket that is genuinely gone the following write fails too, so no duplicate landed and no live table was overwritten — the measured cost is a WRONG DIAGNOSIS, and it is live: § Q8-15 measured 79 datasets registered into `lakehouse`/`landing`, buckets that do not exist, every one of which reads as a table nobody ever wrote. The sharp edge is the per-version branch of `_already_committed`, where a 404 on ONE version's transaction skips the version that may carry the run's own marker — the docstring names that exact regression as one it had already fixed once. **THE FIX IS THE PRECEDENT THE ESTATE SET FOR THE SAME PROBLEM ONE QUESTION OVER**: `service_kit.lancekit.commit_verdict` exists because two planes classified commit failures differently and drifted; `service_kit.lancekit.absence` now holds the absence vocabulary the same way, and all four seams ask it. The list is NARROW and fails closed — `was not found`, `manifest was not found`, `no such file` (the space is load-bearing: `nosuchbucket` does not match it), `must already exist unless`, plus `FileNotFoundError`. Verified by measurement that every genuine-absence shape still matches: an absent dataset, an absent version, an absent local path. **A FIFTH SEAM WAS FOUND AND DELIBERATELY NOT MERGED**: `ray_kit/prune.py` cited `dataplane._ABSENCE_MARKERS` in its own comment, but it asks Ray about a JOB ID, where no object store is in the sentence — so it keeps `does not exist` and the comment now says why the vocabulary is not shared. **ONE KNOWN SEAM LEFT UNTOUCHED, BY THE SCOPE RULING**: `annotator/annotations/versions.py:184` carries the same loose `"not found"`; annotator is out of scope (owner, 2026-09-07) and is recorded here rather than worked. Gated by `packages/service-kit/tests/test_one_absence_vocabulary_for_every_plane.py`, which pins the 404/403/500 trio and the status-line trap on its own **AND THE SIXTH SEAM WAS A SECOND SHARED CLASSIFIER, which is the same defect one level up.** `service_kit.lancekit.errors.is_not_found` already existed for exactly this question on the SERVING side (the registry's `table_dataset` and the reader's `_at_version`, which translate it to a 404), and it was created for the same reason — *"as two inline copies they had already drifted"*. So the estate had two shared vocabularies for one question, with OPPOSITE widths: `is_not_found` matched `("not found", "does not exist")`, widened at 9.0.0 for *"the wording object stores actually produce for a missing path"*. **RE-MEASURED ON 11.0.0 AGAINST A REAL S3 SERVER, that justification does not survive**: every genuine absence — a missing table, a missing version, a missing local path — says `was not found`; nothing said `does not exist` except S3's own bucket error. The wide list was answering MISSING for two conditions that are not, and one of them is worse than the bucket case: **a DELETED DATA FILE** (`Not found: t/data/<uuid>.lance`, measured by deleting one object and reading) is a CORRUPT dataset, and answering 404 "no such table" is how data loss gets reported as a typo. Both now fall through to the raw error. `is_not_found` is a thin alias over `reads_as_absent`, so the two call sites keep their 404 for the two conditions that deserve it. **ONE MARKER WAS KEPT RATHER THAN DROPPED**: `"object at location"`, the precise half of the 9.0.0 wording `Object at location <uri> does not exist` — not reproducible on 11 but previously measured, and unlike the bare `does not exist` it cannot match a bucket body. **A TEST WAS PASSING FOR THE WRONG REASON and was corrected in the same commit**: `test_reader_still_translates_not_found` drove the invented literal `"version 9 not found"`, a wording pylance does not produce, so it proved only that the classifier matched the phrase the test had written. It now uses the measured `Dataset at path t/_versions/9.manifest was not found`, and a new sibling pins the corruption case. **BUILT WITH DAGGER, DEPLOYED AND OBSERVED 2026-09-09.** `lance-rest-catalog:main-88b628a0` published to the dev registry and rolled onto ALL ELEVEN deployments that run it, which also cleared a three-tag drift measured on the way in (catalog/maintenance on `main-3b47bb2c`, annotator/search on `main-28e7f31b`, the rest on `main-cc36396a`). Observed: the running catalog image carries `lancekit/absence.py` and no `errors.py`; the deployed classifier answers `False` for a 404 NoSuchBucket and `True` for an absent dataset, driven in the pod; `plan_compaction` against `s3://lakehouse/gold/catalog` — a bucket § Q8-15 proved does not exist — answers **503 STORAGE fault** against the LIVE object store where it used to answer 404 "never written"; and the maintenance sweep drove `POST /compaction_plan` to **200 OK** on real tables through the changed door, with 0 tracebacks across catalog, lineage, ingest and maintenance in the window. **VERIFICATION LIMIT, stated rather than glossed**: the 404 half could not be isolated in-cluster, because a fresh in-pod process does not run the app lifespan and so holds no Dapr-resolved storage secret — an unauthenticated read cannot prove absence, which is the classifier being right. That path is pinned by the suite and was driven locally. **ONE OPERATOR ERROR, recorded because it is a trap and not a one-off**: `kubectl set image deploy/x "*=IMG"` replaces INIT containers too, so lineage's `wait-age` init went from `busybox:1.36` to the service image, which ships no `nc` — the pod sat in `Init:0/1` looping `sh: 1: nc: not found`. No outage: the old ReplicaSet kept serving 2/2 until the init image was set back by name. Lineage is the only deployment in the estate with an init container, so nothing else was touched |
| ~~Q17-56~~ | ~~A catalog DENIAL was answered by doing the work with a WIDER credential~~ | ~~high~~ | **FOUND BY READING THE LIVE SWEEP 2026-09-10, FIXED, DEPLOYED AND OBSERVED — it was in no row.** Both maintenance->catalog clients classified on `status_code >= 400`, so a 403 took the OUTAGE branch, and every caller answers that branch by doing the work in-pod under the ambient credential — which reaches every bucket in the estate. Measured in one 60-minute window: 20 `compaction_plane_unavailable_falling_back` against 352 successes, and the vending log naming eight refused tables (`lakehouse$gold`, `m2proof_silver$…`, `blobtab_vaud1ns$vblob2`, five more), each then rewritten under the root key. Silent by construction: the fallback logs at INFO because it was written for the outage it could not tell apart from a denial. **Fixed at three seams** — the sweep's vend, the compaction plan, and the index-build door (SUCCESS not RETRY: a denial does not change on redelivery, and an index build writes under the table's own prefix). `MaintenanceDenied` is deliberately NOT a subclass of `CompactionPlaneUnavailable`; had it been, every existing `except` would have kept falling back and the fix would have been invisible — pinned by a test asserting they are siblings. A 503 still degrades, because reclaiming disk through a brief catalog outage is why the fallback exists. **OBSERVED on the deployed release `main-a25688d2`:** 6 `maintenance_vend_denied`/`maintenance_rewrite_denied` at WARNING in four minutes, **0** of the old `credential vending unavailable (403)` bypass, and 165 scoped vends unaffected. None of the eight denied tables is a live cascade tier — `lakehouse$*` sit in a bucket that does not exist (§ Q8-15's residue) and the rest are e2e probes — so the refusal costs no real reclamation and surfaces the residue instead of rewriting it under root |
| ~~Q17-59~~ | ~~Two sibling readers classify the SAME refusal differently, so one edge warns every tick forever~~ | ~~med~~ | **FOUND BY DRIVING THE LIVE CASCADE 2026-09-10, FIXED.** `rask-medallion-producer` logged 17 `cascade_lag_edge_unreadable` WARNINGs in three hours, and that path is NOT the memo's steady state — an invisible edge raises `EdgeNotMeasurable` and logs nothing by design, so every one of these was counted `failed`. **Driving all 261 declared edges in the running pod isolated it to ONE:** 245 invisible, 15 read, and `('silver->gold', 'advref31')` answering **403 on `GET /datasets/advref31-gold$catalog/producers`**. **THE CAUSE IS THE ESTATE'S RECURRING SHAPE, INSIDE ONE FILE:** `published_reader` translates 403/404 into `EdgeNotMeasurable` with the reasoning written out — the catalog collapses "not yours" and "does not exist" deliberately, so claiming either would be a fabricated answer — and `consumed_reader`, thirty lines below it, called `raise_for_status()` bare. The identical refusal was a silent steady state on one side and a WARNING plus a permanent `failed` count on the other. **PROBED BEFORE COPYING THE RULE, because the catalog's reason is not automatically lineage's**: lineage collapses them the same way — an unknown dataset and a forbidden one BOTH answer 403 — so "not visible to this subject" is the honest reading of either. **A 500 still RAISES**, pinned by its own case: this is a translation, not a blanket catch, and swallowing a real fault would turn a broken lineage service into a cascade that looks idle. **SECOND EFFECT, and it is the better one**: the memo only records absence on `EdgeNotMeasurable`, so that edge now stops being PROBED — which stops the probe writing a false `access_denied` into the compliance trail, which is the whole reason § Q5-4's memo exists. **DEPLOYED AND OBSERVED on `main-625e3b06`:** a tick RAN (`cascade_lag_tick`, `POST /medallion-cascade-lag-cron` 200 at 08:13:54, cron is `@every 300s`) and produced **zero** `cascade_lag_edge_unreadable`, where every tick before it produced one. The tick was checked for explicitly rather than reading absence-of-warning as success — with a 300s cron, a quiet window is what a cron that did not fire also looks like |
| Q17-60 | Every lineage RESTART logs ~125 ERRORs saying provenance was lost, for events already in the graph | med | **MEASURED AND EXPLAINED 2026-09-10 — not a loss, but an ERROR that fires routinely is one nobody reads.** The live service showed 126 `lineage_event_unauthorized` (WARNING) and 125 `dapr_dead_letter_parked` (**ERROR**, whose own comment says *"WHOSE provenance was lost. This is terminal loss until someone replays the stream"*) in two hours, plus 783 `ingest_run_mutation_denied` in 45 minutes. **IT IS A RESTART ARTIFACT, and the rate is what proves it:** 783 in the 45-minute window and **0** in the last 15. The pod was 35 minutes old and the burst runs 07:44:37 -> 07:48:58 — four minutes from startup, then silence. The consumer is an ephemeral `deliverPolicy=all` subscriber, so a restart REPLAYS the retained stream; the module already states that (*"lineage's real recovery story stays replay-from-stream"*). Those events were ingested when they were first delivered — nothing is lost. **THE CHAIN, traced rather than assumed:** a replayed event whose subject no longer holds the grant is refused by `enforce_bus_authz` -> `_DROP` -> and `_DROP` is `{"status": "DROP"}`, which Dapr routes to the DEAD-LETTER topic rather than acking, so the dead-letter handler logs ERROR. `lineage_authz_unavailable` (the RETRY path) fired **0** times, which is what ruled out a real delivery failure. **THE DECISION IS THE OWNER'S**, because both repairs touch security-relevant behaviour: (a) leave it — the replay is deliberate and the DLQ is the visibility it was added for; (b) skip re-authorizing a replayed event the graph already holds, which means authz is evaluated at first delivery and never again; (c) tell a replay from a live delivery and log it lower. Recorded rather than guessed, because chasing this cost twenty minutes and would cost it again |
| Q17-61 | Governed silver and gold tables across the estate DECLARE their source tier's table id, so maintenance names the wrong table in lineage and plans compaction against it | high | **FOUND 2026-09-10 BY READING THE LIVE ESTATE — and findable only because the diagnostics tail now renders (§ the `extra=` fix).** `declared_table_id` reads the `lance.dataset_id` schema-metadata stamp a producer writes; `maintenance_dataset_outcome` prints it beside the URI, and for 20+ governed datasets the two disagree: `s3://acme-bucket/e41135a5_acme-silver$features` declares `acme-bronze$events`; so do `acme-gold$agnostic`, `lakehouse-silver$features`, `lakehouse-gold$catalog`, and the same pair under `bind86-`, `c6t115034-`, `lens222127-`, `med215609-`, `vaud31a-`, `advref31-`. `bind86-gold$catalog` declares the tenant-less `bronze$events`. `tov15140-gold$catalog` declares its SOURCE one tier up (`tov15140-silver$features`) rather than bronze, so the stamp is CARRIED FORWARD at each write rather than set once. **The control is in the same measurement**: `research-silver$features`, `research-gold$catalog` and `tov15140-silver$features` each declare themselves, so this is data, not a parsing artefact. **THREE CONSEQUENCES, and the third is the one that can move bytes.** (1) PROVENANCE DOES NOT SURVIVE THE WRITE — `sweep._emit_*` prefers `result.declared_table_id`, so every maintenance event for `acme-silver$features` enters the lineage graph named `acme-bronze$events`, which is the goal's own clause failing silently. (2) ZERO TRUST — the declared id feeds `credentials.write_options_for(..., declared_table_id=...)`, so a credential can be requested for the bronze table and used to rewrite the silver one. (3) DISTRIBUTED COMPACTION PLANS AGAINST THE WRONG TABLE — `plan_via_catalog(table_id, ...)` was observed returning `read_version=166` for `acme-bronze$events` while the bytes to be rewritten were `acme-silver$features`'s. Nothing executed in the observed tick (`compaction_distributed_nothing_to_do`, empty plan), so this is a latent path rather than a recorded corruption — and it is latent only because the plan happened to be empty. **THE CODE IS NOT THE DEFECT, which is why this is a data row.** `stage_stamp.ensure_declared_dataset_id` exists precisely for this (`merge_insert` carries rows, not schema metadata, so a stamp on the source table reaches the dataset only on the create branch) and is called at all four write sites — `medallion/services/compute.py:242,359,383` and `scripts/ray_stage_job.py:591`. It landed in `2e2786f6` (2026-09-09 13:42), and the running Ray head (`ray-lance:main-06e31013`, built 22:44 the same day) has it — verified with `git merge-base --is-ancestor`. The correction is metadata-only and idempotent, so it self-heals ON A WRITE; every table above is one nothing has written since. **MEASURED 2026-09-10 AGAINST THE LIVE STORE, and the answer changes what this row asks for.** Read in the maintenance pod through the sanctioned Dapr path (`apply_dapr_secrets` — a fresh in-pod process holds the env placeholder and 403s `SignatureDoesNotMatch` without it): `acme-silver$features` carries `{b'lineage.dataset_id': b'acme-bronze$events'}` at **version 288, last written 2026-09-10 09:12** — twenty hours AFTER the fix landed. That looks like a live code defect and is not one: `get_transactions()` shows every write since the fix is `Update` or `CreateIndex`, and **no `Merge`**. The cascade has not written this tier since 2026-09-09 13:42, so the correction has never been exercised on it. The dormant tables agree — `acme-gold$agnostic` last written 2026-08-25, `tov15140-gold$catalog` 2026-08-31, the latter stamped at CREATE with its source's identity across all three `lineage.*` keys (`lineage.namespace: tov15140-silver`). **THE VERDICT: not a code defect, and the SELF-HEAL CLAIM IS NARROWER THAN ITS DOCSTRING SAYS.** `ensure_declared_dataset_id` says it is *"safe on every cascade tick and the estate self-heals rather than needing a backfill pass"*. It heals a tier the cascade still WRITES. A tier that has gone quiet — which is most of the estate's silver and gold, since these are e2e tenants nothing re-runs — keeps its parent's name forever, and the maintenance and index writes that do still land on it correctly do not re-stamp, because they do not own the tier's identity. So the estate does not converge on its own, and the three consequences above stay live for every quiet tier. **WHAT IS LEFT IS A DECISION, not a diagnosis:** either a backfill pass over declared ids (the correction is a metadata-only, idempotent commit that rewrites no data and re-mints no row id), or accept stale names on dormant tiers and narrow the docstring so nobody reads convergence into it. Driving a cascade tick would prove the write path but repairs only the one tier it touches |
| Q17-62 | The diagnostics tail is deployed on the nine lakehouse services and on none of the others, so the estate now renders `extra=` on one plane and drops it on the other | low | **INTRODUCED BY THE FIX ITSELF 2026-09-10 (`8ef4c1cc`), recorded rather than left invisible.** `DiagnosticFormatter` lives in `service_kit.app.setup_logging`, so every Python service gets it on its next image build. It shipped in `lance-rest-catalog` `main-d6dc0e3c` and `ingest` `main-d6dc0e3c`; `compute`, `gateway`, `notifications`, `controlplane` and `flows` build from their own dockerfiles and still run older images, so their `extra=` diagnostics are still dropped. That is the estate's own recurring shape — a convention one path holds and another does not — and it is here because a shared-package change lands per image, not per commit. **IN-SCOPE REMAINDER IS EXACTLY ONE: `compute`** (`services/compute`, named by the owner's scope ruling). `gateway` and `notifications` are fleet services the ruling does not name; `flows` is struck outright; `controlplane` is not a lakehouse service. **VERIFIED ON THE NINE THAT DID ROLL**: zero `Traceback` and zero `--- Logging error ---` across all nine in a 40-minute window, which is the negative that matters — a formatter raising on a record prints the latter to stderr, so its absence proves the tail is safe on every record shape these services actually emit. Not a correctness risk on the un-rolled services either: they behave exactly as they did yesterday. The cost is only that a diagnostic is readable on one plane and not the other, which is worth one image build and is held for the owner rather than taken |
| Q17-57 | The SANCTIONED delivery path was handing the Ray lane the tenant ROOT, while a hand-applied Secret carried the scoped one | high | **FOUND AND FIXED IN THE CHART 2026-09-10; NOT DEPLOYED — the rollout rotates a live credential.** The goal names ESO as path 2 for a sidecar-less pod, and the Ray head is exactly that. **Measured live:** `rask-ray-compute-s3` is NOT an ExternalSecret — its only annotation is `kubectl.kubernetes.io/last-applied-configuration`, and the estate materialises just two ESO targets (`rask-infra-credentials`, `rask-observability-s3`). So the head's `S3_SECRET` reached it as hand-applied Secret -> `secretKeyRef` -> env: none of the three sanctioned paths. **AND THE TRAP IS THE INVERSE OF WHAT IT LOOKS LIKE.** `rask-infra-credentials` DOES carry `ray-compute-access-key`/`ray-compute-secret-key` — and both decoded live to **`rustfsadmin`**, the tenant root, because `chart/values.yaml` shipped `rayComputeAccessKey: ""` and `infra-credentials.yaml:36` reads `| default .Values.rustfs.accessKey`. The values file says so in its own words: *"EMPTY = the root credential … stays inert until an identity is named"* — medallion and maintenance were named by § Q17-17 and this third one never was. **So pointing the head at ESO to "fix the delivery path" would have silently downgraded the Ray lane from a scoped identity to root** — the opposite of what that change reads as. **PROVEN AT THE RENDER, both ways:** with the shipping value the OpenBao seed writes `ray-compute-secret-key=rustfsadmin` and the derived scoped secret appears 0 times; with the identity named it seeds the derived `0ea0cae5…` and `rask-ray-compute` renders 5 times (the scoped-user Job now provisions the policy and user, where it rendered 0 before). **THE FIX IS BOTH HALVES OR NEITHER**: naming the identity alone rotates the RustFS user onto the derived secret while the head still mounts the hand-applied `Gm8gbZ7k…`, which is `SignatureDoesNotMatch` on every cascade call — the § Q17-17 outage shape, predicted here rather than caused. `deploy/ray-lance-demo.yaml` now reads `rask-infra-credentials/ray-compute-secret-key`. **DEPLOY SEQUENCE, and it carries a recorded trap:** `helm upgrade` first (rotates the RustFS user and updates the ESO target), then re-apply the head — but that manifest also pins the head's IMAGE, and re-applying it has reverted the head to `ray-lance:dev` and silently broken the cascade before, so the image must be preserved across the apply. Gates: `make prod-render-check` green, 369 chart/secret invariants pass |
| ~~Q17-26~~ | ~~The cascade-lag memo shipped without an observation, and the estate's deny stream is the instrument that settles it~~ | ~~med~~ | **OBSERVED LIVE 2026-09-09, and it is the first real query the audit trail has been asked.** `e7aa8aff` (2026-09-08 18:32) added `AbsentEdgeMemo` because `declared_edges` is a cartesian product — every declared lane x every project in the warehouse registry — so most cells name a table nobody created, each refusal is correct on both sides, and each writes a `lance.audit` record. Its own commit measured the before (1,464 refusals to 96 answers in thirty minutes, ~2,900/hour) and could not measure the after. **The after is now measured, from GreptimeDB rather than from the code**: denials attributed to `service-medallion-producer` ran at a FLAT 2,928/hour for fourteen consecutive hours, then fell — 2443, 1464, 732, 732, 976, 488, 732, 488 — settling around 490-730/hour, with periodic spikes to 1,464 and 1,708. **The spikes are the memo working, not leaking**: its docstring promises it "re-probes EVERYTHING every `TICKS_BEFORE_REPROBE` ticks" so a lane created after the memo learned about it is picked up rather than lost forever, and a flat post-fix rate would have meant the forgetfulness had failed. ~76% fewer refusals, and the shape matches the design. **THE SCALE IS WHY THIS MATTERED**: over the 8.4-day retained window the producer was refused on 252 distinct tables and allowed on 31 — 245,469 denials against 19,391 allows, i.e. 86% of every `deny` on the estate came from this one cron. An audit trail that is six-sevenths one detector being correctly refused is the § Q17-22 failure again in a different instrument: a stream nobody can read is the same as no stream. Both halves are now true — the trail is armed and queryable (§ Q17-23), and the noise that would have made it useless is gone
...
| ~~Q17-15~~ | ~~Root create is open by default (§F2-11)~~ | ~~med~~ | **DONE 2026-09-07** (`e6f4ce37`). THE SHIPPED DEFAULT WAS THE DEFECT, not its value: `hasKey` finds a key whether or not anyone chose it, so `values.yaml`'s `lockRootCreate: false` beat any derivation, and the control could only ever be armed by an operator who already knew to arm it. The key is deleted and `services.yaml` derives the value from `rask.isRealDeployment` — ONE helper, now shared with `prod-credentials.yaml`, so the credential guard and the root-create lock cannot answer "is this a real deployment?" differently. Rendered in all three directions rather than reasoned about: local loop `false` (self-serve preserved), real deployment `true` with nobody arming it, and an explicit `auth.lockRootCreate` still winning in BOTH directions. Gated by `test_root_create_is_locked_on_a_real_deployment.py`, whose second test refuses a `values.yaml` default so the opt-in cannot come back. **The original measurement, kept because it is what the fix answers**: **CONFIRMED LIVE 2026-09-07, not merely a chart default**: the running catalog carries `LANCE_FGA_LOCK_ROOT_CREATE=false`, so on THIS estate any authenticated subject may mint a top-level namespace. `config.py:245` defaults the field False and `fga_deps.py:290` gates the root-object check on it, so an absent lock reads as open self-serve rather than as missing configuration. `lockRootCreate: false`, and `fga_deps.py` reads a missing lock as "open top-level create" — any authenticated subject may mint a top-level namespace | **STRUCK 2026-09-09 — the row said DONE 2026-09-07 and was never marked, the 'already fixed and unstruck' class the goal names. Verified on all three axes rather than taken on the note's word:** (1) the gate exists and passes — `test_root_create_is_locked_on_a_real_deployment.py`, 5 tests; (2) the `lockRootCreate` key is GONE from `values.yaml` (grep finds it only in `services.yaml`, where `hasKey .Values.auth "lockRootCreate"` now falls through to `include "rask.isRealDeployment"`), which is what stops a shipped default beating the derivation; (3) the live catalog still reports `LANCE_FGA_LOCK_ROOT_CREATE=false` — and that is CORRECT here rather than the original defect, because this is the local loop, where self-serve is the intended answer. The defect was that a REAL deployment got the same `false` with nobody arming it, and the derivation is what fixes that |
| Q17-16 | No image signing, no SBOM (§F2-12) | low | **CONFIRMED 2026-09-07, and there is a naming trap in the build that makes it easy to believe otherwise.** `.dagger/images.go` has a helper called `provenance()`, and it emits exactly three OCI LABELS — `BUILD_DATE`, `VCS_REF`, `VERSION` — passed as build args. No SBOM, no signature, no in-toto/SLSA attestation. A reader checking "do we have provenance?" finds a function by that name on the publish path and can reasonably conclude yes. The labels are worth having and are not the control this row names. Scanners only (osv-scanner, trivy, trufflehog). Lakekeeper lacks this too and even disables provenance, so this is parity, not a regression — but it is the last row of the zero-trust list  **THE SBOM HALF IS DONE 2026-09-10; SIGNING DELIBERATELY IS NOT.** `dagger call sbom --name=<stem>` and `zone-sbom --zone=<zone>` emit CycloneDX from the SAME tarball seam the scanner reads, so the inventory describes the exact bytes that were scanned. **Driven, not asserted**: `dagger call sbom --name=gateway` produced a 326 KB document — `bomFormat: CycloneDX`, `specVersion 1.7`, **179 components**, `metadata.component.type: container`. **The first run was WRONG in a way that mattered and was fixed before landing**: trivy takes the scan TARGET as `metadata.component.name`, so a tarball mounted at `/img.tar` produced an SBOM that did not say WHICH image it described — fine for a pass/fail gate, useless for an inventory somebody files and returns to when a CVE lands. It mounts under the image's own name now and the document reads `/gateway.tar`. **WHY THIS SHIPS WHILE SIGNING DOES NOT** (§ Q17-26): an SBOM is an ARTEFACT, useful the moment someone asks which images carry a package, and it needs no verifier to have value — a signature nothing verifies is a control that cannot fire. `--exit-code 0` on purpose: the image you most need an inventory for is the one with findings, so producing it must not depend on the image being clean. Exports are gitignored — 326 KB of reproducible output at the repo root would enter every Dagger build context, which is the trap this estate already carries one instance of |
...
| ~~Q17-22~~ | ~~The lineage seed registers datasets in buckets that do not exist, and the drift report can never clear them~~ | ~~med~~ | Measured 2026-09-07. `lineage/seed.py:51-54` HARDCODES `s3://lakehouse/bronze/events`, `s3://lakehouse/silver/features`, `s3://lakehouse/gold/catalog` and `s3://landing/raw/events` while the configured bucket is `LINEAGE_S3_BUCKET`, which is `lance-catalog` on this estate. **Neither `lakehouse` nor `landing` exists**: the live RustFS volume holds 106 buckets — `lance-catalog`, `rask-observability` and ~104 runtime-minted `*-wh` — and no `lakehouse`. So every dataset carrying a seed-shaped location is permanently `unreadable` to the reconciler, which reports it each tick as `NoSuchBucket ... prefix=bronze/events/_versions/`. Seen in the wild: a reconcile sweep over `checked: 389` datasets came back with a long `storage_loss` list and an `unreadable` map led by `bronze$events_e2e12bc3bbc`, `e2e3367167a-bronze$events` and `e2e3367167a-gold$catalog` — all pointing at `s3://lakehouse/...`. **STRUCK 2026-09-09 — THE SOURCE IS ALREADY CLOSED, and the row's verb is what makes it read otherwise.** `lineage/seed.py` REGISTERS nothing: `build_events` and `events_as_dicts` are called from `tests/unit/test_lineage.py` and from nowhere else — no production path, no startup hook, no HTTP door, no chart Job, no e2e suite (grepped estate-wide). The hardcoded `s3://lakehouse/...` and `s3://landing/...` are a TEST FIXTURE's URIs, not an ongoing writer, so no new residue enters the graph from here. **What the row actually describes is EXISTING graph residue, which is § Q8-15's territory and is converging**: retention landed 2026-09-08, one driven tick pruned 285 runs and 117 orphan datasets, and `unreadable` has gone 79 → 2 with the maintenance sweep now logging neither warning. **The cost is the EVIDENCE, not the data**: the drift report is the estate's own instrument for spotting a real divergence, and one dominated by entries that can never clear is an instrument nobody reads — the same failure as a count that treats one defect recorded three times as three items. **ATTRIBUTION, kept honest**: the seed HARDCODES those URIs and `tests/e2e-py/test_lineage_e2e.py:142` asserts the spelling (`source_uri == "s3://lakehouse/silver/features"`), so the convention is real and exercised; whether the seed also runs on a production estate is NOT measured here and must not be assumed. The demo endpoint next to it does the right thing — `demo.py:181` composes `f"s3://{bucket}/{path}"` from the CONFIGURED bucket — so the fix is the one-line shape already used ten lines away. The residue already registered needs deregistering separately |
| ~~Q17-18~~ | ~~The cascade needs settling time after a multi-service rollout, and the suite cannot tell that from a defect~~ | ~~med~~ | Measured 2026-09-07: rolling catalog + lineage + medallion-producer together, then running `test_governed_union_e2e` immediately, gave 3 failed / 2 passed — `lance_ray_ingest` and `embed_features` both `None` for the first drive. Every tuple checked out (`can_create_table`, `owner`, `can_promote` all True) and the one 403 in the stage runner log was test 2's own deny window working. A re-run minutes later: 5 passed, unchanged code. So the suite reports a settling estate and a broken one identically, and the only way to tell them apart today is to run it twice — which is how a real regression gets waved through as "probably just settling". Either the drives need a readiness precondition (the stage runners' Dapr subscriptions re-registered, the workflow runtime resumed) or the runner needs to wait for one  **STRUCK 2026-09-10 — a settle gate, driven through a real restart rather than asserted.** `scripts/e2e_live.sh` gains a `3.5/4` step between discovery and pytest: for the producer and all three stage runners, wait for the deployment to converge, then poll the SIDECAR'S OWN answer — `/v1.0/metadata`, whose `subscriptions` are the topics the runtime has actually registered, plus `actorRuntime.hostReady`. Verified live: `bronze-to-silver` answers `['dlq.bronze-to-silver','medallion.bronze']`, hostReady true. Not a sleep, which would be slower on a settled estate and still wrong on a slow one. It FAILS rather than warns, because proceeding produces a red run whose cause is the clock. **THE FIRST VERSION HAD A HOLE, FOUND BY DRIVING THE NEGATIVE CASE**: mid-rollout `kubectl exec deploy/<name>` can land on the OLD pod, which is still serving and still settled — a restart reported "ready" after **1 second** while the new pod had registered nothing. `kubectl rollout status` now runs FIRST, so the pod the probe reaches is the pod the suite will talk to; re-driven through a real restart it waited **11s** for genuine convergence. On this estate the sidecar registers before Ready, so the rollout wait is the load-bearing half and the metadata probe covers the inverse case the row measured |
| ~~Q17-23~~ | ~~No authorization-DECISION event: the estate events the grant, never the check~~ | ~~med~~ | **STRUCK 2026-09-09 — I WROTE THIS ROW WRONG AND THE ESTATE REFUTED IT WITHIN THE HOUR.** The claim was that a rask denial "is a 403 and a log line, subscribable by nothing and retained by nothing", derived from the `ControlAction` vocabulary carrying `grant_added`/`grant_revoked` and no decision verb. That vocabulary reading is correct and the conclusion drawn from it is not: the decision trail is not a control event and never was, it is `service_kit.governed.audit`'s dedicated `lance.audit` stream, and **it is armed and working on the live estate.** Measured at the DESTINATION rather than at the call site, which is the only measurement that settles it — a `SELECT` against GreptimeDB's `opentelemetry_logs` where `scope_name = 'lance.audit'` returns real denials from the running catalog: `audit.action=can_get_metadata`, `audit.outcome=deny`, `audit.resource=table:vv6bfc7e9b-silver$features`, `audit.subject=service-medallion-producer`, carrying `trace_id` and `span_id`. `LANCE_AUDIT_ENABLED=true` in the pod, 1504 records in a 4000-line window, and the catalog's own `fga_deps.py` imports `ALLOW, DENY, FAILURE, audit` with 24 references. **TWO TRAPS ON THE WAY, both worth keeping.** (1) A fresh `python -c` inside the container reports the audit logger at level NOTSET — because `configure_audit` runs in the SERVER's process, which a new interpreter never entered; that reading proves nothing about the running app and nearly became the finding. (2) The stdout rendering IS bare — every record prints as the single word `audit`, because the fields live in `extra` and the console formatter names no extra key (`otel.py:161` records exactly this) — so a reader who greps the pod log concludes the trail is empty while the OTLP path is carrying every field. **AND THE ONE DELTA I THOUGHT REMAINED IS ALSO NOT ONE:** Lakekeeper's `determined_by` (which policy decided) is documented in their own source as *"Empty when the authorizer produces no per-decision diagnostics (`AllowAll`, OpenFGA)"* — it is a Cedar-only field, so their OpenFGA deployment records no more than ours does. What genuinely differs is shape, not coverage: theirs is a CloudEvent on the bus, ours is a trace-correlated log stream in GreptimeDB that vmalert can already alert on. Neither is a gap. Related: § Q17-14, refuted 2026-09-07 for the same field on the same records
| Q17-24 | The event actor is a string where it should be a closed union | low | **MEASURED 2026-09-09.** Lakekeeper's CloudEvent actor is a sum type — `Anonymous \| Principal(UserId) \| Role { principal, assumed_role }` (`service/events/publisher.rs`) — so an assumed role records BOTH the human behind it and the role assumed, and neither can be spelled as the other. rask stamps one `author.sub` string. **The consequence is already a documented live failure mode**, not a hypothetical: `.claude/skills/rask-notifications` warns that a producer stamping a role literal in `author.sub` targets nobody and the event is still acked SUCCESS, so the miss is reported by nothing. A closed union at the emit boundary makes that unrepresentable rather than merely warned about |
| ~~Q17-25~~ | ~~S3 remote request signing — the per-object authorization Lakekeeper has and rask does not~~ | — | **STRUCK 2026-09-09: not a design we declined, a mechanism the format's client does not offer.** Lakekeeper implements remote signing (`crates/lakekeeper/src/server/s3_signer/sign.rs`): the client sends an UNSIGNED S3 request, the catalog resolves the tabular **by location**, authorizes `Read`/`Write`/`Delete` on it, and returns signed headers — no credential ever reaches the client and every object operation is individually authorized, which is strictly stronger than a 900 s STS session that re-checks nothing inside its window. **But that is an Iceberg-CLIENT capability** (`S3V4RestSignerClient` in the Iceberg FileIO), and Lance has no equivalent hook: `lance_docs/guide.md:2380-2420` enumerates the whole S3 `storage_options` surface — `access_key_id` / `secret_access_key` / `session_token`, `endpoint`, `region`, virtual-hosted style, S3 Express, SSE/KMS — and `lance_docs/` contains **zero** matches for a signer, a presigned URL or a request-signing callback. The session token IS the strongest mechanism a Lance reader can consume, so `vending.build_session_policy` (bucket + prefix, 900 s) is the ceiling of the format rather than a weaker choice within it. Re-open only if Lance's object_store grows a signer hook |
