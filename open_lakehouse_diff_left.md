...
| ~~High~~ **CLOSED 2026-09-04** | ~~No cascade reconciler and no re-run verb.~~ Both exist: the medallion carries a `bindings.cron` reconciler (`medallion-cascade-lag-cron.yaml`, `b0dec4f0`) and `POST /api/movers/stages/rerun` re-drives ONE edge, gated on that edge's own rung. `open_cascade_repair.md` records the two rules that did not survive contact — the 409 is dropped because the Ray listing it needed has already OOM-killed this pod, and the forward went with it | `open_estate-verification.md` row 35 (D), closed with it |
...
| ~~High~~ **CLOSED 2026-09-04** | ~~Maintenance compaction runs in a 512Mi pod while the distributed seam has no executor.~~ M1 split the planner from the workers; M2 consumes the protocol — plan and commit at the catalog under its own key, every byte in the worker under a vended table-scoped credential. It DEGRADES rather than fails, and `DatasetResult.compaction_mode` records which path ran so the fallback is a count. M3 (BYO compute via `RayJob`) stays deferred behind §7.4 steps 3-4 | Found and closed 2026-09-04 |
