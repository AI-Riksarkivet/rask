...
| Q17-5 | Per-workload storage identities (§F2-1) | high | THE ONE MISSING CONTROL of 19, and NARROWER than §F2 states — measured against the live estate 2026-09-07 rather than read off the sweep. Two consumers are ALREADY scoped and working: `rask-maintenance` holds `MAINTENANCE_S3_ACCESS_KEY_ID=rask-maintenance` and the movers' Ray lane holds `MEDALLION_RAY_S3_ACCESS_KEY_ID=rask-ray-compute`, provisioned by `chart/templates/rustfs-scoped-users.yaml` (a Job that ran here 2 days ago) with its three hard-won policy lessons — bucket-scoped `s3:ListBucket`, the `medallion*` prefix without a trailing slash, and no bucket-NAME allowlist because tenant buckets are minted at runtime. **What is actually left is three identities**: `rask-catalog`, `rask-medallion-producer` and the movers' OWN (non-Ray) writes, all still `rustfsadmin`. So this is EXTENDING a proven pattern, not building one — a policy, a user, the values pair, the OpenBao seed, the infra-credentials key and the deployment's `$scoped` ternary, per identity. The chart DEFAULTS are still empty, so a fresh install comes up on the root user regardless (Q17-13) |
