...
| Medium | **The `Transform` CRD is DEFERRED to `rask-operator`, not abandoned** — reasoning in `docs/DECISIONS.md`, "The compute plane is decoupled". — `open_compute-decoupling.md` §7.4 step 5. A CRD without its controller renders unreconciled CRs as objects stuck mid-provision (`docs/DECISIONS.md` 2026-08-16, re-verified live at `open_estate-verification.md` row 21), so it must not ship in this chart. What it would buy is the declaration living in git with the catalog record as a projection | Carried out of `open_compute-decoupling.md` when that file closed |
