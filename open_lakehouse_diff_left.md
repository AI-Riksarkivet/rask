...
| Q3-36 | `MED-013` | low | Two handler seams typed `Any` with an ANN401 suppression — **RE-MEASURED AT HEAD 2026-09-09 (§ Q12-1's re-run): FOUR, not two**, and the row understates it. `inprocess_executor.py:144` (`result()` -> a WriteResult), `:153` (`_lineage_of` -> a LineageDoc), `workflow.py:810` (`_run_async`), `cascade_lag_readers.py:44` (`declared_edges(settings)`). Each carries its own reason on the line, which is the estate's rule for a suppression — so this is a COUNT correction, not a newly discovered breach |
