...
- `packages/` — reusable libraries, **no entrypoints**. uv + Bun workspace members.
  ...
  - `packages/component-lib` — Svelte 5 + Bits UI + Tailwind 4 component library (`@your-repo/oxen`) w/ Storybook 10 (`@storybook/svelte-vite`). The shared design system; apps consume it via `workspace:*`. Exports button/card/dialog/utils today (more being promoted in — see `docs/architecture/frontend-microfrontends.md`).
