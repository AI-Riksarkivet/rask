...
- `runners/` — **sealed model environments, NOT workspace members. This is the ONLY place a modality may live.** One directory per workload, each holding its Ray Data pipeline (`src/runner`) *and* its model actors in one project with its **own `pyproject.toml` and own `uv.lock`**. Matched by no glob, which is the point: a runner's heavy ML stack (torch, ultralytics, transformers, …) never enters the fleet's resolution (root lock 200 → 145 packages; fleet tests ~32 min → ~6 s). `storage` is a **path** dep. A runner's tests are invisible to the root pytest — `make test` runs them separately; its images build from **its** lock; it carries its own ruff config (ruff resolves the nearest pyproject). Ray entrypoint: `uv run --project runners/<workload> runner`, overridden in-cluster by `RASK_RUNNER_CMD=runner`. A second workload is a second sibling directory, and needs no change anywhere else. **A workload's awkward dependencies are ITS problem — never solved by fattening a shared image.** The chart's Ray cluster currently runs one workload's CUDA image, which is why the medallion lane ships off. **How that resolves is OPEN.** A 2026-08-17 ruling said to move those deps into the runner's own Ray `runtime_env`; that was SUPERSEDED 2026-08-23 — `runtime_env` is not a dependency-provisioning mechanism in production here. Do not reach for it to install a workload's stack, and do not treat the older ruling as standing. Note the distinction, because the word is overloaded and the codebase uses it correctly elsewhere: `runtime_env.env_vars` IS the production mechanism for scoping ENVIRONMENT to one Serve application or job (`chart/templates/rayservice.yaml`, `ray_submit.py`) and is unaffected. What is rejected is shipping DEPENDENCIES through it. Naming the replacement — per-workload baked images, a second Ray cluster, or something else — is an owner decision that has not been made, so no file should assert one.
