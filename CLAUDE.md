...
- `projects/<name>/pyproject.toml` — **deployable composition only, no code**. One per deployable: `gateway`, `core-api`, `orchestrator`, `volumes-api`, `search-api`, `ray-api`, `runner`. (There is no `projects/viewer` — it was deleted when viewer dissolved.)
...
- **Observability (optional, `observability.enabled`):** Vector → GreptimeDB (on RustFS S3 bucket `rask-observability`) → Perses; fleet + Ray export OTLP/gRPC to `rask-greptimedb-standalone:4001` via `service_kit.setup_otel`. Standard OTLP throughout (not OTel-Arrow).
