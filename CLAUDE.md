...
make dev-micro         # the fleet: gateway :8888 + ray :8804 + controlplane :8820 +
...
- `packages/` — reusable **Python** libraries, **no entrypoints**. uv workspace members.
  ...
  - `packages/ray-kit` — Ray Job SDK + dashboard wrapper (schemas, `build_client`, `RAY_TRANSIENT_ERRORS`, the dashboard service). Used by the `ray` service.
...
- `services/` — runnable **Python** code. **The old monolithic `viewer` service is gone**, and so is the whole batches/orchestrator plane (P7a compute-plane cutover — see `docs/architecture/lance-ns-merge.md` P7):
  - `services/gateway` — reverse proxy on `:8888` (the frontend's proxy target). Path-routes `/api/*` longest-prefix-first: the `ray` rows (`/api/ray`, `/api/serve`), `/api/projects` → controlplane, plus the lance-plane rows (`/api/catalog`, `/api/lineage`, `/api/produce`, `/api/train`, `/api/media/*`); owns no state; **no `/api` catch-all** — unmatched `/api/*` 404s. Upstreams are env-overridable (`RASK_RAY_URL` :8804, `RASK_CONTROLPLANE_URL` :8820, `RASK_MEDIA_VIEWER_URL` :8101, …).
  - `services/ray_api` — the **`ray` service** (`:8804`): Ray dashboard introspection (`/api/ray/*`) + the `/api/serve/*` proxy (thin shell over `ray-kit`); no DB. Its k8s/dapr/image/gateway name is `ray` (R20); the uv member stays `ray-api` / import `ray_api` because a Python package named `ray` would shadow PyPI ray. (`core`/`core_api`/`search_api`/`volumes_api` died in the R6/R20 media wave — the S3 object browser now lives in the lance `viewer` at `/api/media/object*`; lines/EAD FTS re-land as catalog-governed Lance tables behind `/api/media/search`.)
...
Deployables are just workspace members with a dockerfile: `.docker/<name>.dockerfile` runs `uv sync --frozen --package <name>` against the **root** `uv.lock` (the deployable set is `gateway`, `ray`, `runner` — the `.docker/ray.dockerfile` image is the ray service; the Ray cluster image is `.docker/ray-cluster.dockerfile`).
