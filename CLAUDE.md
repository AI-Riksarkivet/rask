...
- **No backward compatibility — the estate is pre-production.** No migration shims, dual read paths,
  deprecation windows or "keep the old key working": change the thing and move every consumer in the
  same commit. Current state is test data.
- **Lance-idiomatic, answered from `lance_docs/`, never guessed.** A question about Lance or
  lance-namespace behaviour is settled by citing `lance_docs/` (`ns_catalog/spec.yaml` is authoritative;
  `namespace.md` is an older bundle that contradicts it in places) or by measuring on the installed
  pylance — never from memory.
- **Lakekeeper is the reference, not the stack.** For how zero trust, authn/authz, governance,
  provenance, events and secrets are solved well, read Lakekeeper (`github.com/lakekeeper/lakekeeper`
  and `lakekeeper-charts`). rask translates those answers onto ITS stack — Lance + lance-namespace, Dapr,
  NATS JetStream, OpenFGA, Dex — and imports neither Iceberg nor Lakekeeper's own stack choices.
...
- `packages/` — reusable **Python** libraries, **no entrypoints**. uv workspace members.
  ...
  - `packages/lineage-kit` — the OpenLineage emission kernel used by the medallion producer/stage runners
...
- `services/` — runnable **Python** code. **The old monolithic `viewer` service is gone**, and so is the whole batches/orchestrator plane (P7a compute-plane cutover — see `docs/architecture/lance-ns-merge.md` P7):
  - `services/gateway` — reverse proxy on `:8888` (the frontend's proxy target). Path-routes `/api/*` longest-prefix-first: the `compute` rows (`/api/ray`, `/api/serve` — the URL namespace names the Ray cluster, not the service), `/api/projects` → controlplane, plus the lance-plane rows (`/api/catalog`, `/api/lineage`, `/api/explorer/*`, and the medallion producer's `/api/produce`, `/api/train`, `/api/trains`, `/api/promotions`, `/api/stage-runners`, `/api/cascade`); owns no state; **no `/api` catch-all** — unmatched `/api/*` 404s. Upstreams are env-overridable (`RASK_COMPUTE_URL` :8804, `RASK_CONTROLPLANE_URL` :8820, `RASK_EXPLORER_VIEWER_URL` :8101, …).
  ...
  - `services/medallion` — the lakehouse cascade (producer + stage runners). R23: raw is the external world, never a governed tier — the medallion is exactly bronze→silver→gold. The producer exposes **exactly three write doors, all root-mounted (NOT under `RASK_API_PREFIX`) and all token-guarded**: `POST /produce` seeds the bronze dataset and emits the ONE bronze-write OpenLineage event — this is the **cascade head**, and dropping that emit means the whole bronze→silver→gold run silently never happens; `POST /ingest-media` lands external media as bronze blobs and drives the media chain; `POST /train` submits training. `/bronze-arrival` subscribes to the bronze-write event and publishes the `medallion.bronze` trigger, so the cascade is driven by the ARRIVAL event rather than by the ingest call. (Those three are the whole INGEST surface — there is no protocol-specific ingest door, and adding one would make that protocol privileged. They are not the whole ROUTER surface: beside the probes, produce, ingest-media and train, `producer.py` mounts `promotions` (the quality gate's third answer: a stage runner that HOLDS a promotion publishes it here and a `can_promote` holder answers on `/promotions/*`, gateway-routed as `/api/promotions`), `stage_runner_ops` (the cascade's operator door at `/stage-runners/*`, gateway-routed as `/api/stage-runners`, which authorizes and forwards workflow terminate to the stage runner that hosts the instance), `rerun` (`POST /stage-runners/stages/rerun`, the edge-addressed repair verb, on `stage_runner_ops`' own prefix) and `cascade_lag_read` (`GET /cascade/stalled`, gateway-routed as `/api/cascade/stalled`) — and registers the `/bronze-arrival`, train-trigger and promotion Dapr routes on top. The extra routers are human-facing control, not ingest, which is why they do not weaken the three-doors rule.)
...
- **A Lance APPEND never names a file version.** A create names `data_storage_version="2.2"` and
  `enable_stable_row_ids=True` (both create-time-only); an append inherits the table's version. On
  pylance ≥ 12 an append or fragment commit whose files are at another version is COMMITTED and stamps
  reader flag 256 (mixed data file versions), which no operation removes: maintenance refuses the table
  and pylance 11 / lancedb 0.34 cannot open it (`lance_docs/PROVENANCE.md`). The catalog's `/commit`
  door, the ingest lander and the register door refuse such a set
  (`service_kit.lakehouse.features.describe_foreign_data_file_versions`), and the maintenance counter
  `compaction.datasets.mixed_file_versions` pages on one.
...
- **The estate is AUTHENTICATED, and this bullet used to say the opposite.** It read *"No auth, no app
  ...
  harmless: read as policy, "trusted network" made an unguarded service look deliberate rather than
  unfinished. Owner ruling 2026-08-26: authenticated is the truth, the ungated services get the door
  their siblings share, and body caps + rate limiting land as one seam in
  `service_kit.middleware.register_middleware` rather than per route. That work landed: the tracking
  file, `open_fastapi-audit.md`, was **DRAINED AND DELETED 2026-08-28** — all 55 findings fixed
  RED-first, every closure adversarially re-verified (the drain's own commits carry the record).
  **`compute` and `controlplane` are gated too** — verified 2026-09-09: both mix in
  `GovernedAuthSettings`, both ship a `security.py` whose `require_read` resolves a `CurrentSubject`
  plus an FGA checker, and both gate the WHOLE router (`dependencies=[Depends(security.require_read)]`
  on `/ray` and `/projects`). Measured live: `/api/ray/jobs` answers 401 with no bearer. The frontend
  hits `/api/*` on the **gateway** (`:8888`), which path-routes to the per-domain services; `/api/ray/*`
  and the `/api/serve/*` proxy are served by the **compute** service (over `ray-kit`) — the URL
  namespace names the Ray cluster, not the service (R22). SSR `load`/remote functions reach the gateway server-side via an absolute base URL (`RASK_GATEWAY_URL`); client code uses the relative `/api/*` proxy. The gateway sits **behind** the SvelteKit Bun server (it does not serve the SPA shell).
- **State surface:** the lakehouse (Lance datasets on RustFS S3) governed by the catalog, plus the chart-managed **lineage (AGE)** and **OpenFGA** Postgres databases — served by the `rask-age` **StatefulSet**, NOT by CloudNativePG. Measured live 2026-09-07: **zero** `cluster.postgresql.cnpg.io` objects exist, and `age.cnpgCluster.enabled` defaults **false** because the AGE-on-CNPG cutover needs a built extension image (`.docker/cnpg-age-ext.dockerfile`), K8s 1.33+ and CNPG >= 1.27 — the two paths are mutually exclusive and `age-cluster.yaml` fails the render if both are on. The CNPG **operator** IS installed (`cnpg.enabled: true`, `rask-cloudnative-pg` running) with nothing to reconcile, which is exactly what made this line easy to believe: the name is present and the thing it implies is not. **The app's own relational DB is gone** (P7a): no batches table, no Alembic, no `DATABASE_URL` in the fleet. **No Redis**; events ride Dapr pub/sub on NATS JetStream. (`.docker/` still carries 8 `docker-compose*.yml` files — auth/dex, governance, lineage, rustfs, demo, local. They are side stacks for local bring-up, not the deploy path; the Helm chart is.) The Helm chart in `chart/` is the single deploy artifact for both local k3s and production — in-cluster CloudNativePG (`Cluster`), RustFS operator (`Tenant` → `rask-rustfs-io:9000`), and KubeRay are gated by `cnpg.enabled`/`rustfs.enabled`/`ray.enabled` values toggles; each toggle gates the operator subchart. **The CUSTOM RESOURCE has its own gate and they differ** — `cnpg.enabled: true` installs the operator while `age.cnpgCluster.enabled: false` creates no `Cluster`, so an enabled operator toggle is not evidence that the resource exists. Local deploy: `make k3s-install` (one-time) → `make k3s-build` → `make k3s-import` → `make k3s-up`; tear down with `make k3s-down` / `make k3s-purge`. See `docs/architecture/deployment.md` and `chart/README.md`.
