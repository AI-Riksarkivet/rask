# open_ingest — pre-bronze ingest as a platform service

**THE one plan doc for the ingest wave.** Study/audit + critique, 2026-08-01; consolidated
2026-08-03 (this file absorbed its own first generation — see "Lineage of this file"
below — and a readiness audit's corrections landed with it). Committed as the
implementation spec; implementation itself has not started (§6e is the /goal-ready
condition for driving it).
...
**One name, everywhere: `ingest`** (§7.1, re-resolved 2026-08-03). uv member, import,
app-id, image, chart key, gateway row `/api/ingest`, stream `INGEST`, DLQ
`dlq.ingest.tasks`, this filename. There is no `etl` token on any surface; "ETL" survives
in this document only as prose and as the name of Phase-2 options.

## Lineage of this file

Until 2026-08-03 the ingest wave had **two** root plan docs, and they contradicted each
other in three blocker-grade ways — the only such pair the repo has ever carried. The
older `open_ingest.md` (2026-07-29, the owner's R1/R2/R3 ruling) directed a reader to
build ingest **on Ray inside `compute`**, to give **every tier an opaque payload**, and to
do **R3 first** — all three now wrong, and all three stated without a supersession marker,
because the marker lived in the file the reader hadn't opened. Per the repo's own
convention (fold-then-delete, as `open_dockview`/`open_fga`/`open_anno`/`open_label` each
retired), that file's unique content was folded into `OPEN-WORK.md` — its shipped ledger
as the **eighth wave** (per-store endpoint/credentials/TLS, attach-a-bucket) and the
`compute` OOMKill→1536Mi rationale appended to **G2** — and the file was deleted. What it
ruled, and what became of each:
...
- **R1** (medallion = bronze→silver→gold and nothing else) — **stands**; operationalized
  at §5 point 7 and gate A12.
- **R2** ("ingest is an ETL job; ETL and query live in `compute`, on Ray") — **replaced**
  by the ruling above: ingest is its own service plane, no Ray (§5, §7.1).
- **R3** (every tier carries a generic opaque payload) — **revised to R3′** in §3.1: an
  opaque payload destroys pushdown, schema evolution and catalog governance; a tier is a
  stage over many typed datasets, and the envelope fields become required governance
  columns. (R3 was never adopted anyway — `schemas/tier.py`'s `TIER_COLUMNS` has zero
  importers, and it says `source_rowid` where the ruling said `source_uri`.)
- The **"two buses is the expensive mistake"** deferral — **answered** in §2/§4: right as
  instinct, wrong as frame; the control plane and the data plane are separate questions.

## 0 · Readiness audit — corrections that change the plan (2026-08-03)

Before consolidation this document's claims about the codebase were re-verified against the
code (six parallel lanes: Phase-0 infra, package seams, service seams, dependencies +
`lance_docs/` citations, the doc pair, and the process gates). **Most claims held and are
line-exact** — 202-but-synchronous, overwrite-clobbers-siblings, the non-deduplicating
`Idempotency-Key`, the missing FAIL lineage, the driver-side `ray.get`, the M3 lane drop,
the per-process single-flight lock, quality-runs-after-commit, and zero consumers for both
`tracker` and `validate`. What follows is only what was **wrong**, because that is what
changes work. Each item is evidence-backed; where a gate as written cannot pass, the gate
text below has been corrected rather than left aspirational (§3.4's own rule, applied to
this document).

**C1 · Phase 0's M4 condition is unachievable as written — re-plan required (hard blocker).**
The condition says "upgrade Dapr to a version whose `resiliencies` CRD accepts the
exponential retry fields". **The pinned and live versions cannot** — this half is
independently reproduced twice: the vendored `chart/charts/dapr-1.18.1.tgz`'s
`crds/resiliency.yaml` carries only `duration`/`maxInterval`/`maxRetries`, and the live
cluster CRD accepts exactly `{duration, matching, maxInterval, maxRetries, policy}`, while
`chart/templates/dapr-resiliency.yaml:33-39` sends `initialInterval`/`multiplier`/
`randomizationFactor` — `kubectl apply --dry-run=server` returns `strict decoding error`
today. The stronger claim — that **no** version accepts them, released or master, because
the Go type `Retry` itself lacks the fields (`types.go:120-126`, decoded through
`resiliency.go:442-449`, so they could never reach dapr/kit's Config) — rests on network
fetches verified once and **not** re-verified; treat it as very likely, and re-check
upstream before concluding a bump is impossible. Either way the ~4s effective retry window
is real and a bump is not the plan. Re-plan M4 as one of: constant-policy duration
arithmetic; moving the window to the pub/sub component's own `backOff` metadata; or dropping
the resiliency CR for pub/sub. **Three artifacts move in lockstep** —
`dapr-resiliency.yaml:33-39`, `.dagger/charts.go`'s `resiliencyGate` (which render-asserts
`initialInterval: 30s` / `multiplier: 2` / `randomizationFactor: 0` / `! grep duration:` and
the 400≤w≤500 arithmetic), and the Phase 0 `/goal` text. Note CI is currently **green over a
CR the cluster can never accept**: the gate proves the render, never an apply.

**C2 · "The nine IIIF files" is twelve, and only two are deletable — A12 corrected.**
`rg -l -i iiif services/medallion` returns **12**: the nine named plus `services/ingest.py`,
`services/produce.py`, `services/transform.py`. Only `api/ingest_iiif.py` and
`services/iiif_produce.py` are IIIF-exclusive; the other ten are shared modules requiring
surgical edits — and `services/ingest.py` is simultaneously cited by §6e as "the verified
write shape" to **keep**. Step 6 is an edit campaign, not a file sweep.

**C3 · A13's literal gate can never pass — corrected.** `rg await_success` returns nothing
is unsatisfiable while this document names the string (it does, in A13 itself). The gate is
restated below as a *code-scoped* grep. Scope is also wider than stated: three copies of the
poll machinery exist (`ray-kit/submit.py`, `ratch/core/jobs.py`,
`medallion/services/ray_submit.py`) plus six tests.

**C4 · Three "already exists" seams do not exist.** (a) **The catalog's publication event
has no seam**: `catalog.control.v1` exists (`control_emit.py:57-99`, 22 call sites) but
carries **metadata actions only** — `commit_fragments` and every tag route emit nothing —
and movers subscribe to a whole Dapr *topic* (`api/events.py:36-40`), so subject-per-dataset
needs new actions, new NATS subjects, and per-dataset Dapr components in the chart.
(b) **A14's creation contract does not exist**: `register_table`/`declare_table` validate
nothing and the repo has no primary-key surface at all (the catalog *sets*
`enable_stable_row_ids=True` on its own writes, `dataplane.py:244`, but refuses nothing).
(c) **The I4 enforcement template does not exist**: ratch's write-funnel "grep gate" is a
docstring claim — `packages/ratch` has no tests directory and is not in `testpaths`. There
is no Python prior art for a seeded-violation gate; A10 must build the pattern, not copy it.

**C5 · The tracker step is not "small, standalone".** `TrackerProtocol` is entirely
**synchronous** (`protocol.py:19-49`, every member `def`) and its shared implementation is
SQLModel/SQLAlchemy-shaped (`_base.py:53-195`), so a JetStream-KV backend shares nothing
with it and forces an async-protocol-vs-thread-offload decision this document never names.
There is also **no shared conformance suite** to extend — `tests/test_tracker.py` hardcodes
`SqliteTracker` in all 11 tests. Both must be built.

**C6 · Dependency reality.** `dapr-ext-workflow` and `nats-py` are absent from every
`pyproject.toml` and from `uv.lock`; `duckdb` is dev-group only; there is **no NATS Python
client anywhere in the repo**. Both add cleanly (dapr-ext-workflow 1.18.3 requires only
unpinned `dapr`, matching the locked 1.18.3; nats-py 2.15.0 needs ≥3.7), and chart Dapr
1.18.1 clears the ≥1.15 Workflow-stable bar — but nothing in §6b's stack is installable
today.

**C7 · Open decision 9 is ANSWERED — and it constrains §5.4.** The DuckDB-Lance extension
was previously "blocked by the sandbox egress proxy"; it in fact loads here (duckdb 1.5.5,
extension `2f167ea`). Results: it **scans blob-v2 fine**; `COPY … TO … (FORMAT lance)`
works for record columns but **hard-fails on blobs** (`Conversion Error: DuckDB does not
support Strings over 4GB`); and it exposes **no `ENABLE_STABLE_ROW_IDS` option**
(`Unrecognized option`), so a DuckDB-*created* dataset can never satisfy D1's
creation-time-only precondition — and per `file_format.md:4012` it can never be turned on
later. **Ruling: DuckDB may land records into datasets the catalog created; it may never
create a governed dataset, and never touches the blob lane.** The pylance/DuckDB lane split
in §5.4 stands and is now grounded rather than provisional. (`tests/e2e-py/test_duckdb_lance_e2e.py`
already exists and covers READ only.)

**C8 · `lance_docs/ray.md` is stale against the installed lance-ray 0.5.0** — it documents
`namespace=` where 0.5.0 takes `namespace_impl`/`namespace_properties`, and omits
`enable_stable_row_ids`/`dataset_options`. This **refutes D1's own aside** that "lance_ray
exposes no stable-row-id/version-pinning surface": `io.py:157` has
`enable_stable_row_ids`, plumbed through `datasink.py`/`fragment.py`. D1's Ray-tasks-drop-to-core-scanners
workaround is therefore unnecessary as stated. (D5's claims — no merge/upsert mode, returns
no fragments — **do** hold.) Re-pin or annotate `ray.md` at 0.5.0. Every other spot-checked
`lance_docs/` citation was accurate; all seven cited files exist and are git-tracked.

**C9 · Scope and gate-runnability corrections.**
- **The frontend is in the blast radius and was not in scope:**
  `frontend/packages/api/src/ingest.ts:28` calls `/api/ingest-iiif` and pins the response
  with a valibot literal `status: 'ingested'` (`:10-15`) — a genuinely-async 202 breaks it.
  Added to the Phase 1 scope list.
- **`make check` is RED at HEAD**, so goal condition (6) cannot pass in-scope: 5 ruff errors
  (in-scope: `ray-kit/submit.py`, medallion) **plus** knip offenders that are committed files
  *outside* every scope bullet (`…/annotator/drive2.tmp.mjs`, `…/lakehouse/look.tmp.mjs`,
  committed in `a7cccf8`). Either widen the scope to delete those, or restate (6) as
  "`make check` clean **for the files this work touches**". `make test` **is** green
  (1960+28 passed).
- **A11's "tilt/k3s harness" does not exist.** The Tiltfile has no `local_resource` and no
  test resource; `scripts/tilt-verify.sh` is a live_update prover, not a data-lane harness.
  The only real in-cluster suite runner is **kind**-based (`scripts/e2e_stack.sh`,
  `make e2e-ci`). A11 must either build the harness or retarget to kind — and `Tiltfile` is
  in neither phase's scope list.
- **NATS chart keys:** there is no `replicaCount` and no `cluster:` key in the nats stanza
  (`chart/values.yaml:1031-1047`); the subchart key is `config.cluster.enabled/replicas`,
  and `values-prod`'s commented sketch points at *externalization* instead — pick one.
  Adding the stream requires editing **two** lists in `nats-stream-job.yaml` (the
  add-if-missing list and the `:96` drift-reconcile loop, which already omits
  `CATALOG_CONTROL`).
- **Helm is not installed in `.dagger/base()`**, so 13 chart-render invariants silently skip
  in CI — audit m5, reproduced. A10 depends on fixing this.
- **`service_kit.openlineage` has six production importers**, not one (`lineage`, `catalog`,
  `compaction`, `medallion`, …) plus the spec-conformance gate — retiring it is not a "drift
  fix in passing".
- **Behavior pins are wider than the `test_medallion*.py` glob:** all 7 tests in
  `tests/unit/test_iiif_ingest.py` are IIIF, plus ~12 across
  `test_medallion.py`/`test_medallion_cascade.py`, plus the chart's four `MEDALLION_IIIF_*`
  env rows pinned by `test_invariants.py:852`. Conversely **zero gateway tests** guard
  `/api/ingest-iiif` — the route move is untested in both directions.

**C10 · D6's creation path is refuted by the catalog's own door — the highest-risk
assumption in the plan, and it is salvageable.** D6 routes the lander through the catalog's
client-direct fragment-commit endpoint and states creation as "(creation: `Overwrite`,
`guide.md:1594-1601`)" — citing the pylance guide, never the door. **The door refuses it.**
`services/catalog/src/catalog/services/dataplane.py:614` hardcodes
`op = lance.LanceOperation.Append(frags)`, and its docstring at `:594-595` states the policy
outright: *"CREATE and OVERWRITE stay server-side to centralize it and to owner-govern the
destructive reset"*. A second bar: `api/v1/endpoints/data.py:349-350` refuses a client-direct
commit when the table has no object-store location. So "no byte transits the catalog" is
**false for the first run of every dataset** unless the plan says how creation happens.
**The escape hatch was tested and works** (pylance 9.0.0, the repo's own bronze schema —
`pa.field("id", int64)` + `blob_field("payload")` + `pa.field("source_uri", string)`):
`write_dataset(empty_table, mode="create", data_storage_version="2.2",
enable_stable_row_ids=True)` → version 1, 0 rows (**a blob-v2 column creates fine empty**);
then `write_fragments` → `LanceOperation.Append` commit with `read_version=1` → version 2,
`_rowid` `[0,1,2]`, `_row_created_at_version` `[2,2,2]`. **Write this two-step into D6
explicitly**: server-side empty create through the catalog, then client-direct
fragment-append. Creation is the catalog's; only appends are the lander's.

**C11 · A12's deletion trips a repo invariant in the opposite direction — the chart rows
must be cut in the SAME commit as the code.** `tests/unit/test_invariants.py:138`
(`test_no_dead_chart_env_vars`) fails once the four `MEDALLION_IIIF_*` envs
(`chart/templates/medallion.yaml:75, :167, :178, :182`) have no first-party reader — and
their only readers are `core/config.py:294-301` + `api/ingest_iiif.py:84`, exactly the code
A12 deletes. Meanwhile `test_invariants.py:844` fails if they vanish from the render while
the code still reads them. **Neither cut is green alone**; they must land together.

**C12 · A13 forces re-cutting the SURVIVING mover, which no goal condition names.**
`ray_kit.await_success` has two medallion call sites and they are not symmetric:
`services/ray_submit.py:163` is inside `submit_iiif_ingest_job` (`:117`) — the deleted IIIF
path — but `:111` is inside `submit_stage_job` (`:54`), whose only caller is
`services/transform.py:243`, **the mover path R1 explicitly keeps in the medallion**. The
...
repeats the pin; `docs/DECISIONS.md:71` (#16) points the other way. This document sides with
adoption (§4 "Durable orchestration") without addressing the pin. The *substrate* is fine —
placement + scheduler are load-bearing already (`values.yaml:1144-1153`), `actorStateStore:
"true"` exists (`dapr-statestore.yaml:62`) — what is missing is the **ruling**: either argue
the ingest run meets the reopen criterion (the fan-out + external-wait shape plausibly does)
and record it, or drop Dapr Workflow. Do not build against a contested pin.

**C14 · Phase 1's end state has no IMAGE, so nothing it builds is deployable — and A11 needs
a running pod.** Goal condition (1) requires a uv member "with a chart entry, gateway row,
stream, and DLQ route". A chart entry is not a deployment: the image chain is hand-listed in
**two** places inside one file (`.docker/rest-catalog.dockerfile:32-34` **and again**
`:42-44` both hardcode the seven `--package` names) plus `.dagger/images.go:56-60`
...
working tree, so `git switch -c <branch>` re-points the directory *for every process using
it* — the other session's `git pull --rebase --autostash origin main` (x3) and its commits
then landed on this plan's branch, and `--autostash` stashed and restored this work mid-edit
underneath it. Nothing was lost, but an unrelated local commit (`fix(shell)`) ended up filed
on the ingest branch and the branch stopped being a clean base. **A branch is not isolation;
a worktree is.** Phase 0/1 run on branch `ingest-plane` in the
`/home/blackwell/Desktop/rask-ingest` worktree (`git worktree add`), a mechanism the estate
already uses (`rask-dockview`, `.claude/worktrees/`). Before starting, run `git worktree
list` and check whether another agent holds the checkout.

**Already green, for the record:** the exponential retry fields are committed and the CI
charts gate already render-asserts the 450s arithmetic (it is the *CRD* that rejects them);
an actor state store with `actorStateStore: "true"` exists (`dapr-statestore.yaml:62`) — the
Dapr Workflow substrate; `stateStore.scopes` is already derived-from by netpol + secret-store
scoping, so adding `ingest` propagates for free; **a direct `nats-py` client needs no new
NetworkPolicy rule** — `chart/templates/network-policy.yaml:299-323` admits nats:4222 from
any intra-namespace pod, which the plan needs twice (pull consumer + KV tracker); the
fragment-parallel-write-plus-single-commit seam is **measured** working, not merely
documented; and the `write_fragments`+`Append` row of §7.11 is verified and dated
(§Empirical).

**Not evidence, do not cite:** `make e2e` is currently red (3 failed / 5 passed) **but
confounded** — tilt was running during the measurement, so live rollouts of
web-compute/studio/train produced `failed _app assets` 404s. A20 depends on this harness;
stop tilt, let rollouts settle, and re-baseline before treating any e2e result as signal.
...
The streaming question ("Kafka? Fluss? Flink?") is therefore two questions the retired first generation's
...
The first-generation rulings (R1/R2/R3, see "Lineage of this file") were audited, not just
implemented-against. Findings, hardest first.
...
R3 said the fifth column is `source_uri`, `schemas/tier.py:48-54` says
...
- **Two buses stays right as instinct, wrong as frame** — see §2's control/data split, which
  is the corrected version of R2-era deferral paragraph.
...
```sql
INSERT INTO for_landing        -- sink: nats subject ingest.out.av_catalog → lander → Lance
...
```
  ...
  {source: iiif|s3|…}│  • validate source spec                 • consume ingest.tasks.<run>│
 ────────────────────▶  • mint run_id, START lineage           • fetch bytes (bounded    │
  ...
  (status, progress) │    registry)                            • validate (pkg validate)│
                 ...
                 (observational ONLY:│   INGEST stream            (invisible until commit)
...
2. **Per-unit checkpointing = `packages/tracker`, finally consumed** (§1.2). The package
   ...
   re-publish. The first generation's "idempotency is the hard part, not the transfer" — answered.
...
8. **Gateway/chart mechanics** (audit checklist): `/api/ingest` row → bare app-id (don't repeat
   the `lance-ray` legacy app-id, audit m1); `.Values.services` entry with `frontDoor: true`,
   own resource tier; worker as a second deployment of the same image; new `INGEST` JetStream
   stream + per-app Dapr component via the existing loops; DLQ `dlq.ingest.tasks` parked to the
...
2. **Service skeleton**: new uv member via `make_service_app`, chart `.Values.services`
   entry (`frontDoor: true`, own tier), gateway row + `RASK_INGEST_URL`, `INGEST` stream +
...
| Object storage | RustFS | S3 wire protocol; per-store endpoint/creds already estate config (`OPEN-WORK.md` eighth wave; `service_kit/schemas/storage.py:55-71`) | any S3: HCP, MinIO, AWS |
...
- **D6 · The blob lane's landing IS uncommitted Lance fragments** — recipe verbatim in
  ...
  `read_version`. **Creation is a SERVER-SIDE two-step, not a client `Overwrite`** (§0 C10 —
  the earlier "(creation: `Overwrite`, `guide.md:1594-1601`)" cited pylance, not the door,
  and the door refuses it: `dataplane.py:614` hardcodes `LanceOperation.Append` and `:594-595`
  rules that "CREATE and OVERWRITE stay server-side"; `data.py:349-350` additionally refuses a
  client-direct commit on a table with no object-store location). The verified shape, measured
  on pylance 9.0.0 against this repo's own bronze schema: **(1)** the catalog creates the
  dataset EMPTY, server-side — `write_dataset(empty_table, mode="create",
  data_storage_version="2.2", enable_stable_row_ids=True)` → version 1, 0 rows; a blob-v2
  column creates fine with zero rows, and this is also where A14's creation-time flags are
  enforced. **(2)** workers then write fragments and the lander commits `Append` with
  `read_version=1` → version 2, `_rowid` `[0,1,2]`, `_row_created_at_version` `[2,2,2]`.
  So: **creation is the catalog's, appends are the lander's** — which keeps "no byte transits
  the catalog" true (the create carries zero rows) instead of false on every dataset's first
  run. Binding constraints now
...
- **A11** The dummy lane end-to-end in-cluster: fixture source (`LocalDirSource`,
  ...
  assertions over this same deployed lane. *(Corrected per C9: **no tilt-based data-lane
  harness exists** — the Tiltfile has no `local_resource` and no test resource, and
  `scripts/tilt-verify.sh` proves live_update, not data. The only in-cluster suite runner is
  **kind**-based: `scripts/e2e_stack.sh` / `make e2e-ci`. Decide explicitly: extend
  `e2e_stack.sh` (cheapest — it already stands the stack up in CI) or build the tilt
  resource; if the latter, `Tiltfile` must be added to the Phase 1 scope list, where it is
  absent today.)*
- **A12** *(corrected per C2 — it is twelve files, and only two are deletable.)* The two
  IIIF-exclusive files are **deleted** (`api/ingest_iiif.py`, `services/iiif_produce.py`);
  the other ten are **edited** until `rg -l -i iiif services/medallion/src` is empty —
  `core/config.py` (the four `MEDALLION_IIIF_*` settings, and the chart rows
  `test_invariants.py:852` pins), `producer.py`, `services/ray_submit.py`,
  `services/ingest_trigger.py`, `services/s3_harvest.py`, `schemas/events.py`,
  `schemas/htr.py`, `services/produce.py`, `services/transform.py` (incl. the `:109-123`
  lane-drop the move retires), and `services/ingest.py` — which keeps its verified write
  shape (§6e) and loses only its IIIF vocabulary. The medallion's tests pass without them:
  all 7 of `tests/unit/test_iiif_ingest.py` retire with the route, and the ~12 IIIF-touching
  tests in `test_medallion.py`/`test_medallion_cascade.py` are re-pointed. **The four
  `MEDALLION_IIIF_*` chart rows (`chart/templates/medallion.yaml:75, :167, :178, :182`) are
  cut in the SAME commit as the code** (§0 C11): `test_invariants.py:138` fails once they have
  no first-party reader, `:844` fails if they vanish while the code still reads them — neither
  cut is green alone.
- **A13 — no polling loops.** The run path contains zero polling loops: workers wait on
  ...
  survive the move in any form. *(Corrected per C3: the deletion gate is
  **`rg await_success -g '*.py'`** returns nothing — the unscoped form can never pass,
  because this document names the string. Three copies must go: `ray-kit/submit.py`,
  `ratch/core/jobs.py`, `medallion/services/ray_submit.py`, plus six tests. **And per §0 C12
  the deletion is NOT confined to the retiring IIIF path:** `ray_submit.py:163` sits in
  `submit_iiif_ingest_job` (deleted), but `:111` sits in `submit_stage_job` (`:54`), whose
  only caller is `transform.py:243` — the mover R1 explicitly KEEPS. A13 owns re-cutting it
  on the async pattern `submit_train_job` (`:169`) already documents at `:16-21`.)*
...
```text
/goal Phase 1 of open_ingest.md is implemented, on branch ingest-plane (inherited from
Phase 0; it lives in the /home/blackwell/Desktop/rask-ingest WORKTREE — never share a working
directory with another agent session, see §0 C15). End state, all demonstrated in-conversation: (1) services/ingest exists as a uv
workspace member (api + worker + lander modules) with a chart entry, gateway row `/api/ingest`
(inheriting the row `/api/ingest-iiif` vacates), INGEST JetStream stream, and dlq.ingest.tasks
route, AND a Dagger-built image that actually deploys — a chart entry is not a deployment
(§0 C14: the package list is hardcoded twice in .docker/rest-catalog.dockerfile:32-34 and
:42-44, plus .dagger/images.go:56-60 venvImages; the Deployment itself is a values edit since
fleet.yaml:11 ranges over .Values.services); (2) every acceptance condition A1–A20 in §6d has a named test:
the fast set green via `uv run pytest -m "not slow"`; the in-cluster set (A11) via the harness
A11 names (kind/e2e_stack.sh unless a tilt resource is built — §0 C9: no tilt data-lane harness
exists today) and the Playwright set (A20) via `make e2e` — slow markers are permitted for
A11/A20 but each must be demonstrated passing at least once in-conversation; (3) the grep-gate
tests for I3/I4 pass and fail on seeded violations — build the pattern, do NOT copy ratch's,
which is a docstring and not a test (§0 C4c) — including A13 as DELETION (`rg await_success
-g '*.py'` returns nothing; the unscoped form can never pass because this document names the
string — packages/ray-kit, packages/ratch and services/medallion are in scope for exactly this
deletion, three copies plus six tests); (4)
packages/tracker has a nats:// KV backend passing a NEW shared conformance suite — none exists
to extend, and TrackerProtocol is synchronous while JetStream KV is async, so the
async-vs-thread-offload decision is made explicitly and recorded (§0 C5); (5) the medallion's
two IIIF-exclusive files are deleted, the other ten edited, and `rg -l -i iiif
services/medallion/src` prints nothing (§0 C2 — it is twelve files, not nine); (6) `make check`
is clean FOR THE FILES THIS WORK TOUCHES — it is RED at HEAD and two of its knip offenders are
committed files outside this scope (§0 C9); state the pre-existing failures explicitly rather
than fixing out-of-scope files or claiming a clean run. Scope: services/ingest,
services/medallion, services/gateway, services/catalog (A7/A14/
A18 event emission — which has NO seam today and needs new actions, subjects and per-dataset
Dapr components, §0 C4a; plus the creation contract, which does not exist at all, §0 C4b; and
tag advance), packages/{tracker,service-kit,lineage-kit,
ray-kit,ratch,validate,storage}, runners/dummy, the frontend surfaces A20 names (via @rask/api)
**plus frontend/packages/api/src/ingest.ts — which calls /api/ingest-iiif and pins the response
with a valibot literal `status:'ingested'`, so a real async 202 breaks it (§0 C9)**,
...
```text
/goal Phase 0 of open_ingest.md is complete. End state, all demonstrated in-conversation:
(1) M4 closed — NOT by a Dapr bump: per §0 C1 the exponential fields exist in no released
chart, in dapr master's CRD, or in the Go type `Retry` itself, so no version accepts them.
Pick and implement ONE of: constant-policy duration arithmetic, the pub/sub component's own
backOff metadata, or dropping the resiliency CR for pub/sub — then `helm upgrade` applies
the committed CR with no strict-decoding error, `kubectl apply --dry-run=server` of the
rendered CR is clean against the LIVE cluster CRD, the effective retry window is shown to
total 450s by the mechanism chosen, and `.dagger/charts.go`'s resiliencyGate is rewritten to
assert that mechanism — with the chart-render invariant tests actually running in CI, which
requires installing helm in `.dagger/base()` (closing audit m5; 13 invariants silently skip
today). (2) NATS is a 3-node cluster with streamReplicas: 3 — note the subchart key is
config.cluster.enabled/replicas, NOT replicaCount, and values-prod's commented sketch points
at externalization instead: pick one and say which; `nats stream info` (or the render) shows
replicas=3 for every stream, and INGEST is added to BOTH lists in nats-stream-job.yaml (the
add-if-missing list and the :96 drift-reconcile loop, which already omits CATALOG_CONTROL).
...
RECORDED results as a dated table in open_ingest.md §7.11 for: RustFS conditional put
...
(5) The Dapr Workflow contradiction is SETTLED with a docs/DECISIONS.md entry (§0 C13):
docs/OPERATORS.md:95-99 pins "Dapr Workflow stays un-adopted", upheld 2026-07-28 with a reopen
criterion at :121-123, and chart/values.yaml:1142 repeats it — either argue on the record that
the ingest run meets that criterion (fan-out over N units + a durable external wait, which no
caller-chosen idempotency key expresses) and supersede the pin, or rule Dapr Workflow out and
say what orchestrates the run instead. Phase 1 must not build against a contested pin.
(6) `make check` is clean and `make test` passes. Constraints: changes only under chart/,
scripts/, .dagger/, .github/workflows (the helm CI job), tests/, docs/DECISIONS.md,
docs/OPERATORS.md, and open_ingest.md — no service code; no docker build; the
working branch is ingest-plane in the rask-ingest worktree (never claude-prefixed); commits carry no co-author
...
1. ~~Service name~~ **Re-resolved 2026-08-03 (owner ruling: best practice, easiest to maintain):
   `ingest` on EVERY surface — no ETL/ingest split.** uv member, import, app-id, image, chart key,
   gateway row `/api/ingest`, JetStream stream `INGEST`, DLQ `dlq.ingest.tasks`, and this document.
   *Supersedes the earlier resolution* (`ingest` internally, `/api/etl` + `ETL` + `dlq.etl.tasks`
   publicly, on the reasoning "the plane is EL-by-ruling, the public name says ETL"). That reasoning
   was true and still lost: it bought an accurate public label at the price of a permanent two-token
   mapping across the gateway's hand-maintained route table (`rask-services-fleet` invariant 3 — a new
   public prefix is a hand-written row, and longest-prefix-first with **no** `/api` catch-all means a
   mismatch 404s silently), the chart, the Dapr components, the stream-creation job, the DLQ route and
   every test that names any of them. One token is the maintainable choice, and it is this document's
   own "one name for one thing" rule (§6c) applied to itself. `/api/ingest` also inherits the row the
   retiring `/api/ingest-iiif` vacates, rather than adding a prefix beside it.
...
6. ~~Workflow engine threshold (§3.6)~~ **Resolved: Dapr Workflow** (stable since 1.15,
   ...
   redeliver-the-whole-message model suffices. Its explicit reopen criterion (`:121-123`):
   *"If a multi-step path appears whose steps cannot be made idempotent by any caller-chosen
   key … a workflow engine earns its dependency."* The honest argument for reopening is that
   the ingest run is exactly that shape — fan-out over N units with a durable external wait
   (`wait_for_external_event("drained")`) and exactly-one finalize, which no per-message
   idempotency key expresses. **Phase 0 must settle it with a `docs/DECISIONS.md` entry**
   (against #16 at `:71`, which already points the other way) — argue the criterion is met, or
   drop Dapr Workflow and orchestrate the run some other way. Do not build against a contested
   pin. The substrate is not the question: placement + scheduler are already load-bearing
   (`values.yaml:1144-1153`) and `actorStateStore: "true"` exists (`dapr-statestore.yaml:62`).
...
9. ~~Verify DuckDB-Lance blob-v2 support~~ **RESOLVED 2026-08-03 (§0 C7) — measured, not
   assumed.** The pylance side was already verified (see Empirical verification — append,
   merge-on-id, fragment-parallel commit all work on blob-v2). The DuckDB side is no longer
   blocked: the extension **loads** here (duckdb 1.5.5, extension `2f167ea`) and **scans
   blob-v2 fine**, but (a) `COPY … TO … (FORMAT lance)` **hard-fails on blob columns**
   (`Conversion Error: DuckDB does not support Strings over 4GB`, both for `SELECT *` and for
   the payload projected alone), and (b) it exposes **no `ENABLE_STABLE_ROW_IDS` option**
   (`Not implemented Error: Unrecognized option`; candidates are `max_rows_per_file` only) —
   so a DuckDB-*created* dataset can never satisfy D1's creation-time-only precondition, and
   per `file_format.md:4012` the flag can never be enabled afterwards. **Ruling: DuckDB may
   land record-shaped data into datasets the CATALOG created; it may never create a governed
   dataset, and it never touches the blob lane.** The §5.4 lane split is therefore grounded
   rather than provisional. (`tests/e2e-py/test_duckdb_lance_e2e.py` already exists — READ
   coverage only; the write path above is what it does not cover.)
