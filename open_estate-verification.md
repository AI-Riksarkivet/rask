...
| 19 | **Deletion semantics never verified live** — owner asked directly | **OPEN** | UNIT coverage is substantial (64 tests: project_delete 21, warehouse_delete 17, trash_purge 20, hierarchy_enforcement 6) but there is NO dedicated live e2e suite; `drop` appears only incidentally in the lineage/observability e2e. Unverified against a running estate: bottom-up refusal (a container 409s NAMING its contents), explicit `cascade`, warehouse-delete gating on the PROJECT's `can_administer`, project delete having NO cascade at all, `force=true` overriding `protected` and NOTHING else (the FGA gate must still run identically), bucket purge as a separate opt-in that must prove sole ownership (`projects_claiming_bucket` — deleting the work warehouse once wiped gold's data), the no-existence-oracle rule on destructive doors (PermissionDenied collapses to TableNotFound so ids cannot be enumerated), and the #96 recoverable CASCADE (detach + plural `undrop`). **Live estate runs `LANCE_TRASH_GRACE_DAYS=7`, so drops ARE recoverable here** — chart default is 0/permanent, so this estate is on the safer setting and the trash path is the one that actually needs proving |
