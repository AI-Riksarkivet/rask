...
| 11a | 2 lineage e2e failures — product defect or dirty-DB isolation? | **OPEN** | `test_medallion_ingest_and_lineage_queries`: `typed["embedding"]` is None where `array<float>` is expected, and `{p.author} == {"data_eng"}` is an EXACT set equality. The live AGE holds ~776 runs over 32 days from many authors, so both smell like test isolation against accumulated state rather than a defect — but that is a hypothesis, not a finding. Needs a clean lineage DB to distinguish |
