<script lang="ts">
	/** Shared card chrome for every flow node: a titled surface with a
	 *  run-status dot, stale/off chips, a per-node error banner, and
	 *  hover-revealed actions (run / duplicate / disable / delete).
	 *  Title falls back to the node's custom `label` when set. */
	import type { Snippet } from 'svelte';
	import { Copy, Eye, EyeOff, Play, X } from '@lucide/svelte';
	import { graph, STATUS_DOT, type RunStatus } from '$lib/flows/graph.svelte';
	...
	// Per-node error attribution: when this node's run failed, surface the
	// message right on the card (the red dot alone isn't enough to debug from).
	...
	<div class="border-border flex items-center gap-1.5 border-b px-3 py-1.5">
		...
		{#if stale}
			<span
				...
				title="This node's output is out of date (it was edited, rewired, or upstream re-ran) — press ▶ to refresh"
				>stale</span
			>
		{/if}
		<button
