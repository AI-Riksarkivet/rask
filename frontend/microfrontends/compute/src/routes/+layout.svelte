<script lang="ts">
	...
	// The read state: per SUBJECT for the rows the inbox holds, per TAB for the rest. `@rask/ui`'s
	// NotificationCenter has always exposed these two sets as bindable with `onseen`/`ondismiss`
	// documented as "the persistence seam"; this is the zone giving that seam its backend
	// (`services/notifications`, one inbox actor per subject, reached through this zone's own
	// RASK_GATEWAY_URL).
	...
	onMount(() => {
		feed = lineageFeed();
		// Seeded on mount, never during render, and for the same reason the feed is: the page must not
		// wait on the notification plane to paint.
