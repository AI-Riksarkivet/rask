<script lang="ts">
	...
	// The compute landing — the batches/HTR dashboard folded in from the retired overview zone
	// (R16: the overview surface is Ray-plane material, so it lives with the Ray views), merged
	// with this zone's own live cluster summary (count cards + events). Every link here is
	// in-zone (`base`-relative, soft nav) — the old cross-zone /compute/... hops became local.
	//
	// THE ONE PATTERN (see lib/remote/compute.remote.ts):
	//  - Initial reads (batches, chunks) are SSR-rendered remote queries — `await`ed in markup
	//    (experimental.async), no onMount waterfall — kept fresh by the mutation handlers below.
	//  - Live reads (events/cluster/jobs/actors/tasks/serve) are the SAME query objects the other
	//    compute views use, polled below via `.refresh().catch()` and read imperatively
	...
	// `runSync`/`handleSubmitChunk` refreshes. `await` is legal at the top level of an async-mode
	// component; first paint already has the data. With no backend these REJECT and the boundary
	// below renders the unreachable state (no infinite spinner).
	const payload = $derived(await batchesQuery);
	const chunks = $derived(await chunksQuery);

	// Polled Ray reads — imperative `.current` (undefined until first resolve). All six dashboard
	...
	// a clock is the only transport. The batches/chunks reads are deliberately NOT polled: they
	// change through the sync/submit mutations below, which refresh them in place. `.catch(() => {})`
	// is mandatory: an uncaught refresh rejection (one 500) evicts the query from cache and kills
	// the loop. Each query refreshes independently so one failure can't stop the others.
	onMount(() => {
	...
	async function handleSubmitChunk(id: number) {
		...
		try {
			...
			await Promise.all([jobsQuery.refresh(), clusterQuery.refresh()]);
		} catch (e: unknown) {
	...
	// The zone's live counts (the pre-merge compute landing) — each card soft-navs into the view
	// that explains it, so the batches dashboard above and the Ray views below share one index.
...
<main class="bg-background flex-1 overflow-auto">
	<div class="flex flex-col gap-4 p-6 text-sm">
		<!-- Live Ray cluster signal — outside the boundary so a dead gateway still shows
		     the honest "Ray offline" pill next to the unreachable-state card below. -->
		...
		<svelte:boundary>
			{#snippet pending()}
				<div class="text-muted-foreground py-6">Loading…</div>
			{/snippet}
			...
			{#snippet failed(boundaryError, reset)}
				...
				>
					<span>
						Backend unreachable — {boundaryError instanceof Error
			...
			<div class="flex flex-col gap-4">
				...
				<!-- Pipeline summary tiles (from the folded-in overview dashboard) -->
								...
								<div class="flex items-center gap-3 px-4 py-1.5 text-xs">
									...
									{#if j.batches.length}
										<span class="text-muted-foreground truncate">
											{j.batches.length} batch{j.batches.length === 1 ? '' : 'es'}: {j.batches.slice(0, 3).join(', ')}{j.batches.length > 3
						? ` +${j.batches.length - 3}`
						: ''}
										</span>
									{/if}
				...
				{#if chunks.length}
					<Card class="p-4">
						<div class="mb-2 flex items-center justify-between gap-3">
							<div class="text-muted-foreground text-[11px] font-medium tracking-wide uppercase">
								{chunks.length} chunks · ~{Math.round(chunks[0]?.expected_pages ?? 0).toLocaleString()} pages
								each
							</div>
							{#if chunkFilter !== 'all'}
						...
						{#if submitMsg}
							<div
								class="border-primary/30 bg-primary/10 mb-2 rounded-md border px-3 py-1.5 text-xs"
							>
								{submitMsg}
							</div>
						{/if}
						...
						<table class="w-full border-collapse text-xs">
							...
							<tbody>
								{#each filtered as b (b.batch_id)}
									{@const cid = b.chunk_id}
									<tr class="border-border/40 hover:bg-muted/40 border-b">
										<!-- Plain text, not a link: the discover viewer this used to open was
										     retired with its zone (the media plane owns viewing now). -->
										<td class="px-3 py-1.5 font-mono">{b.batch_id}</td>
										...
										<td class="px-3 py-1.5 text-right font-mono">
											{#if cid !== null}
												<button
													...
													onclick={() => (chunkFilter = cid)}>{cid}</button
												>
											{:else}—{/if}
										</td>
										...
										<td class="px-3 py-1.5 text-right font-mono tabular-nums">
											...
											<span class="text-muted-foreground">{pct(b.transcribed_pages, b.page_count)}</span>
										</td>
				...
				<!-- Cluster events (the pre-merge compute landing's feed) -->
				<Card class="overflow-hidden">
					<div
