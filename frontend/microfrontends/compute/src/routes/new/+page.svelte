<script lang="ts">
	...
	// The new-volume ingest form, folded in from the retired overview zone (R16). This app's
	// base is /compute; in-app links use `base`. Pure action page — one mutation on submit,
	...
	addFiles(e.currentTarget.files)}
		...
		{#if result}
			<div class="space-y-1 rounded border border-emerald-600 p-3 text-sm">
				...
				<!-- The retired overview page also linked the discover viewer here; that zone is gone
				     (the media plane owns viewing now), so the back-link is the only destination. -->
