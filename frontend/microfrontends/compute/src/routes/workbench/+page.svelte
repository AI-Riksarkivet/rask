...
<script lang="ts">
	...
	import { Boxes, ListTree, Server, ServerCog } from '@lucide/svelte';
	...
	import ServePanel from '$lib/dock/panels/ServePanel.svelte';
	...
	const panels: PanelRegistry = {
		...
		actors: {
			...
			label: 'Ray actors',
			icon: Boxes,
			keywords: ['ray', 'actors', 'workers', 'replicas'],
		},
		serve: {
			component: ServePanel,
			label: 'Serve apps',
			icon: ServerCog,
			keywords: ['serve', 'deployments', 'replicas', 'transcribe', 'htrflow', 'endpoints'],
		},
	};
	...
	function ready(dockApi: DockviewApi, restored: boolean): void {
		...
		// Serve tabs WITH actors: a Serve replica IS a Ray actor, so the two answer one question at
		// different altitudes ("which deployment" vs "which process"), and flipping between them is
		// exactly the move you make when a deployment goes unhealthy. A fourth region would shrink all
		// four below reading size on a laptop.
		dockApi.addPanel({
			id: 'serve',
			component: 'serve',
			title: 'Serve',
			position: { referencePanel: 'actors', direction: 'within' },
		});
	}
