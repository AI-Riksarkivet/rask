...
// SSR reads (remote query() via getRequestEvent().fetch) issue relative /api/*, which in prod
// resolves against the external ingress origin and hairpins back through it. Route them straight to
// the in-cluster gateway instead. Client-side fetches are untouched.
