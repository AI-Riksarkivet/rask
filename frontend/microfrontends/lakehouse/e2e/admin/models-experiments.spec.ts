...
// Hermetic /experiments coverage (#53), moved from e2e/models when the read became a remote function
// (`experiments.remote.ts`): the PromQL now runs FOR REAL on the zone server against raw Prometheus
// payloads seeded per bearer on the mock-observability server — these tests exercise the actual
// query URLs and series flattening instead of a pre-shaped dashboard stub.
//
// Two page.route-era states are consciously RETIRED, not translated: the 401 sign-in state (the
// auth-ON harness's login-first gate redirects a signed-out navigation before the page can render an
// in-page 401) and the 501 no-observability state (GREPTIME_API is always set on this server — the
// branch exists for un-provisioned deployments the hermetic harness deliberately does not model).
// Both renderings live in the component; the pipeline states that remain reachable are covered.
