// The FGA access surface's e2e fixtures, shared by the spec and the mock catalog. They lived inline in
// access.spec.ts while the surface read through page.route-able /capi routes; the remote-function
// migration moved every read/write SERVER-side, so the fixtures now feed `mock-catalog.ts` (which the
// zone server's CATALOG_API points at) and the spec asserts through `/__mock/access` instead of
