...
// The storage browser's backend seam (R18): `/api/explorer/**` rides to the rask GATEWAY with its
// path unchanged (KEEP_API_PREFIX strips nothing), and the gateway's `/api/explorer` row routes it to
// the media-plane viewer, which serves the ported S3 object browser (`/api/explorer/objects` →
