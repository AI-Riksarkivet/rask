...
// The FGA workbench's data plane, as REMOTE FUNCTIONS — the estate's dialect (every zone carries
// `remoteFunctions` + `experimental.async`, and this zone's own admin console already reads through
// `admin.remote.ts`). The functions run on the zone (SvelteKit/Bun) server, reach the catalog directly
// with the signed-in session's bearer, and PARSE at the wire boundary — which now lives here, not in
// the browser — so a contract drift is an `{ok:false}` the component renders honestly, never a cast.
//
 ...
 *  grant never renders a store it just changed. */
