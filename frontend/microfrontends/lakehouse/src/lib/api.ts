...
// `$lib/lineage/remote/lineage.remote.ts` (open_transport.md, area 2), which binds the SAME client to a
// server-side transport. What remains here rides the `/api/*` pass-through, whose deletion is blocked
// on the cross-zone workbench elements that consume it (open_transport.md caveat 2).
export const {
	fetchEstateGraph,
	fetchProducers,
	fetchReaders,
