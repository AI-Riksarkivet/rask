...
`ray.init(address="auto")` resolves):
    ...
    ray job submit --address http://127.0.0.1:8265 \
        --working-dir scripts -- python deploy_htr.py
...
class HTRFlow:
    def __init__(self):
        ...
        # A per-replica temp dir, not a fixed /tmp path: several replicas start
        # concurrently and a shared filename races (and is world-writable).
        cfg = pathlib.Path(tempfile.mkdtemp(prefix="htrflow-")) / "pipeline.yaml"
        cfg.write_text(PIPELINE_YAML)
        self.pipeline = Pipeline.from_config(str(cfg))
